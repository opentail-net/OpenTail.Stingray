...
public sealed class GgufModelTests : IDisposable
{
    ...
    [Theory]
    [InlineData(39u, DType.MXFP4, 32L, 17L)]
    [InlineData(40u, DType.NVFP4, 64L, 36L)]
    [InlineData(41u, DType.Q1_0, 128L, 18L)]
    [InlineData(42u, DType.Q2_0, 64L, 18L)]
    public void DTypeInfo_CurrentLlamaCppTypes_HaveCorrectStorageGeometry(
        uint rawType, DType expected, long elements, long bytes)
    {
        Assert.True(DTypeInfo.TryFromGgufType(rawType, out var dtype));
        Assert.Equal(expected, dtype);
        Assert.Equal(bytes, DTypeInfo.ByteSize(elements, dtype));
    }
    ...
    [InlineData(38u)]
    [InlineData(43u)]
    public void DTypeInfo_RetiredOrFutureLlamaCppTypes_AreRejected(uint rawType)
    {
        Assert.False(DTypeInfo.TryFromGgufType(rawType, out _));
    }
