...
{
    [Fact]
    public void TensorSourceFeatureProbe_DetectsAttentionBias()
    {
        var source = new ProbeTensorSource(
            new Dictionary<string, object>
            {
                ["general.architecture"] = "qwen2",
                ["qwen2.block_count"] = 1,
                ["qwen2.embedding_length"] = 64,
                ["qwen2.context_length"] = 512,
                ["qwen2.vocab_size"] = 128,
                ["qwen2.attention.head_count"] = 4,
                ["qwen2.attention.head_count_kv"] = 2,
                ["qwen2.attention.key_length"] = 16,
                ["qwen2.rope.dimension_count"] = 16,
    ...
    private sealed unsafe class ProbeTensorSource : IModelTensorSource
    {
        private readonly Dictionary<string, GgufTensorInfo> _tensors;

        public ProbeTensorSource(IReadOnlyDictionary<string, object> metadata, params GgufTensorInfo[] tensors)
        {
            Metadata = metadata;
            _tensors = tensors.ToDictionary(tensor => tensor.Name, StringComparer.Ordinal);
            Tensors = tensors;
        }
        ...
        public ReadOnlySpan<byte> GetTensorData(GgufTensorInfo tensor) =>
            throw new NotSupportedException("Feature detection does not read tensor bytes.");
