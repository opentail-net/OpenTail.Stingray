...
/// cleared <c>STINGRAY_CPU_MOE</c> (restored on dispose) so the env-var side effect is
...
public sealed class CpuMoeFlagsTests : IDisposable
{
    ...
    {
        Environment.SetEnvironmentVariable(Var, "preexisting"); // e.g. STINGRAY_CPU_MOE in the shell
