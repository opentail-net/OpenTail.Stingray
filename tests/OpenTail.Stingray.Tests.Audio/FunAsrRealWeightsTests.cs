...
public sealed class FunAsrRealWeightsTests
{
    ...
    [Fact]
    public void SenseVoice_RealModelFile_LoadsAndValidates()
    {
        string? modelPath = FindModelPath("sensevoice-small.int8.onnx");
        if (modelPath is null) return;

        var fi = new FileInfo(modelPath);
        Assert.True(fi.Length > 100 * 1024 * 1024, "SenseVoice ONNX file must be > 100MB");
