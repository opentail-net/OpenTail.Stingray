...
Dumped: 2026-06-08, via `opentail-llm-cli list-metadata` / `list-tensors` on the real file.
...
`_opentailllm.has_post_attn_norm=true`, `_opentailllm.has_post_ffw_norm=true`,
`_opentailllm.has_qk_norm=true`, **`_opentailllm.has_layer_output_scale=true`**.
There is **no** `_opentailllm.has_ple` (PLE absent — confirmed).
