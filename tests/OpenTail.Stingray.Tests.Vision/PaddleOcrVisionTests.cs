...
public sealed class PaddleOcrVisionTests
{
    [Fact]
    public void ImagePreprocessor_GeneratesTargetGrid()
    {
        ...
        var pre = PaddleOcrImagePreprocessor.Preprocess(rgb, w, h, patchSize: 14);
        Assert.NotNull(pre);
        Assert.True(pre.TargetWidth % 14 == 0);
        Assert.True(pre.TargetHeight % 14 == 0);
