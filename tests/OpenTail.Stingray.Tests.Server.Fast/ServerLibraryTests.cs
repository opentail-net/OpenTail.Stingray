...
public sealed class OpenTailStingrayServerOptionsTests
{
    ...
    public void Defaults_AreSafeForOutOfTheBoxBoot()
    {
        ...
        Assert.Equal(0, opts.CpuThreads);
...
public sealed class OptionsConfigurationBindingTests
{
    ...
    public void BindsBackendAndHardwareFields()
    {
        ...
        {
            ["OpenTail.Stingray:Backend"]      = "Cuda",
            ["OpenTail.Stingray:NGpuLayers"]   = "-1",
            ["OpenTail.Stingray:ContextSize"]  = "4096",
            ["OpenTail.Stingray:TurboQuant"]   = "true",
            ["OpenTail.Stingray:KvType"]       = "q8_0",
            ["OpenTail.Stingray:MinBatchBlas"] = "32",
            ["OpenTail.Stingray:CpuThreads"]   = "6",
        }).Build();

        var s = new ServiceCollection();
        s.AddOpenTailStingray(cfg);

        var opts = s.BuildServiceProvider().GetRequiredService<IOptions<OpenTailStingrayServerOptions>>().Value;
        ...
        Assert.Equal(6, opts.CpuThreads);
    }
    ...
    public void BindsMoeAndSpecAndSamplingNestedTrees()
    {
        ...
        {
            // MoE knobs
            ["OpenTail.Stingray:MoeWarmPin"]          = "4",
            ["OpenTail.Stingray:MoeWarmPinAfter"]     = "1024",
            ["OpenTail.Stingray:MoePredictPrefetch"]  = "false",
            ["OpenTail.Stingray:ExpertStatsPath"]     = "/tmp/stats.json",
            ["OpenTail.Stingray:CpuMoe"]              = "true",

            // Spec decode
            ["OpenTail.Stingray:SpecType"]            = "Mtp",
            ["OpenTail.Stingray:SpecDraftNMax"]       = "2",
            ["OpenTail.Stingray:SpecDraftPMin"]       = "0.75",

            // Nested sampling defaults
            ["OpenTail.Stingray:Sampling:Temperature"]   = "0.7",
            ["OpenTail.Stingray:Sampling:TopK"]          = "40",
            ["OpenTail.Stingray:Sampling:TopP"]          = "0.95",
            ["OpenTail.Stingray:Sampling:MinP"]          = "0.05",
            ["OpenTail.Stingray:Sampling:MaxNewTokens"]  = "2048",
        }).Build();
...
public sealed class ServerMetricsTests
{
    ...
    [Fact]
    public void RecordOverloadRejection_IncrementsByOne()
    {
        var m = new ServerMetrics();
        m.RecordOverloadRejection();
        m.RecordOverloadRejection();

        Assert.Equal(2, m.OverloadRejections);
    }
    ...
    [Fact]
    public void ServingTiming_RendersPrometheusHistograms()
    {
        var m = new ServerMetrics();
        using (var timing = m.BeginServingRequest())
        {
            timing.MarkAdmitted();
        ...
        string rendered = m.RenderServingLatencyMetrics();
        Assert.Contains("# TYPE opentail_llm_queue_latency_seconds histogram", rendered);
        Assert.Contains("opentail_llm_time_to_first_token_seconds_count 1", rendered);
        Assert.Contains("opentail_llm_generation_duration_seconds_count 1", rendered);
        Assert.Contains("opentail_llm_request_generation_duration_seconds_count 1", rendered);
    }
...
public sealed class InferenceEngineLoaderTests
{
    ...
    {
        var opts = new OpenTailStingrayServerOptions { ModelPath = null };
    ...
    {
        var opts = new OpenTailStingrayServerOptions { ModelPath = "" };
    ...
    {
        var bogus = Path.Combine(Path.GetTempPath(), $"opentail-llm-nonexistent-{Guid.NewGuid():N}.gguf");
        var opts = new OpenTailStingrayServerOptions { ModelPath = bogus };
        var ex = Assert.Throws<InvalidOperationException>(() => InferenceEngineLoader.Load(opts));
        Assert.Contains("Model file not found", ex.Message);
        Assert.Contains("STINGRAY_MODEL", ex.Message);
    }
}

public sealed class ModelCompatibilityTests
{
    [Theory]
    [InlineData("llama")]
    [InlineData("qwen3")]
    ...
    [InlineData(DType.IQ2_S, true)]
    [InlineData(DType.IQ2_XXS, true)]
    [InlineData(DType.IQ3_XXS, true)]
    [InlineData(DType.IQ3_S, true)]
    [InlineData(DType.IQ1_S, true)]
    [InlineData(DType.IQ1_M, true)]
    [InlineData(DType.MXFP4, true)]
...
{
    private const string Var = "STINGRAY_CPU_MOE";
    ...
    private static OpenTailStingrayServerOptions OptsNoModel(bool? cpuMoe) =>
    ...
    {
        const string sentinel = "preexisting"; // e.g. an operator's directly-exported STINGRAY_CPU_MOE
...
/// <summary>DI registrations produced by <see cref="ServiceCollectionExtensions.AddOpenTailStingray"/>.</summary>
public sealed class ServiceCollectionExtensionsTests
{
    [Fact]
    public void AddOpenTailStingray_RegistersExpectedServices()
    {
        var s = new ServiceCollection();
        ...
        Assert.NotNull(sp.GetService<IOptions<OpenTailStingrayServerOptions>>());
    }

    [Fact]
    public void AddOpenTailStingray_RegistersAllServicesAsSingletons()
    {
        var s = new ServiceCollection();
        s.AddOpenTailStingray();
    ...
    [Fact]
    public void AddOpenTailStingray_InlineConfigureCallbackRuns()
    {
        var s = new ServiceCollection();
        ...
        var bound = sp.GetRequiredService<IOptions<OpenTailStingrayServerOptions>>().Value;
    ...
    [Fact]
    public void AddOpenTailStingray_RendererUsesArchitectureFromOptions()
    {
        var s = new ServiceCollection();
        s.AddOpenTailStingray(opts => opts.Architecture = "llama");
    ...
    [Fact]
    public void AddOpenTailStingray_PreRegisteredFakes_AreNotOverridden()
    {
        // TryAddSingleton semantics: tests that wire a fake IInferenceEngine BEFORE
        // calling AddOpenTailStingray should keep the fake.
    ...
    [Fact]
    public void AddOpenTailStingray_EngineFactoryOption_BypassesModelPath()
    {
    ...
    [Fact]
    public void AddOpenTailStingray_RegistersJsonSourceGenContextAtIndexZero()
    {
        // Endpoints serialize via OpenTailStingrayJsonContext.Default; this guards that the
        ...
        Assert.Same(OpenTailStingrayJsonContext.Default, jsonOpts.SerializerOptions.TypeInfoResolverChain[0]);
    }

    [Fact]
    public void AddOpenTailStingray_WithConfiguration_BindsOpenTailStingraySection()
    {
        // The IConfiguration overload binds the "OpenTail.Stingray" section onto the options.
        var cfg = new ConfigurationBuilder()
            .AddInMemoryCollection(new Dictionary<string, string?>
            {
                ["OpenTail.Stingray:ModelPath"]     = "/tmp/foo.gguf",
                ["OpenTail.Stingray:MaxBatchSize"]  = "16",
                ["OpenTail.Stingray:Architecture"]  = "llama",
            })
            .Build();
    ...
    [Fact]
    public void AddOpenTailStingray_MmprojPath_ConfigurableViaConfigKeyAndConfigure()
    {
        // Issue #253: the vision projector must be settable through OpenTailStingrayServerOptions
        // — both the OpenTail.Stingray:MmprojPath config key and the inline Configure callback —
        // not only the STINGRAY_MMPROJ environment variable.
        var cfg = new ConfigurationBuilder()
            .AddInMemoryCollection(new Dictionary<string, string?>
            {
                ["OpenTail.Stingray:ModelPath"]  = "/tmp/model.gguf",
                ["OpenTail.Stingray:MmprojPath"] = "/tmp/mmproj.gguf",
            })
            .Build();

        var fromConfig = new ServiceCollection();
        fromConfig.AddOpenTailStingray(cfg);
        using var spConfig = fromConfig.BuildServiceProvider();
        Assert.Equal("/tmp/mmproj.gguf",
            spConfig.GetRequiredService<IOptions<OpenTailStingrayServerOptions>>().Value.MmprojPath);

        var fromConfigure = new ServiceCollection();
        fromConfigure.AddOpenTailStingray(opts => opts.MmprojPath = "/code/mmproj.gguf");
        using var spConfigure = fromConfigure.BuildServiceProvider();
        Assert.Equal("/code/mmproj.gguf",
            spConfigure.GetRequiredService<IOptions<OpenTailStingrayServerOptions>>().Value.MmprojPath);
    }

    [Fact]
    public void AddOpenTailStingray_InlineConfigure_OverridesConfigurationValues()
    {
        ...
        var cfg = new ConfigurationBuilder()
            .AddInMemoryCollection(new Dictionary<string, string?>
            {
                ["OpenTail.Stingray:MaxBatchSize"] = "2",
                ["OpenTail.Stingray:Architecture"] = "llama",
            })
            .Build();

        var s = new ServiceCollection();
        s.AddOpenTailStingray(cfg, opts => opts.MaxBatchSize = 32);
...
/// <summary>
/// End-to-end smoke test for <see cref="EndpointRouteBuilderExtensions.MapOpenTailStingray"/>:
...
{
    [Fact]
    public async Task MapOpenTailStingray_RegistersAllEndpointGroups()
    {
        using var host = await new HostBuilder()
            .ConfigureWebHost(b => b
                ...
                .ConfigureServices(s =>
                {
                    s.AddRouting();
                    s.AddOpenTailStingray();
                    // Override IInferenceEngine so MapOpenTailStingray() doesn't trigger a model load.
                    s.AddSingleton<IInferenceEngine>(new FakeInferenceEngine("ut-model"));
                })
                .Configure(app =>
                {
                    app.UseRouting();
                    app.UseEndpoints(e => e.MapOpenTailStingray());
                }))
            .StartAsync();
    ...
    [Fact]
    public void MapOpenTailStingray_ReturnsTheSameBuilder_ForChaining()
    {
        // Fluent return — common ASP.NET Core idiom: app.MapOpenTailStingray().MapHub<...>().
        var builder = WebApplication.CreateSlimBuilder();
        builder.Services.AddOpenTailStingray();
        builder.Services.AddSingleton<IInferenceEngine>(new FakeInferenceEngine("x"));
        var app = builder.Build();

        IEndpointRouteBuilder ret = app.MapOpenTailStingray();
        Assert.Same(app, ret);
    }
}

/// <summary>Public-surface sanity check on <see cref="OpenTailStingrayJsonContext"/>.</summary>
public sealed class OpenTailStingrayJsonContextTests
{
    [Fact]
    public void Default_CoversCoreRequestAndResponseTypes()
    {
        ...
        var ctx = OpenTailStingrayJsonContext.Default;
    ...
    public void Default_UsesSnakeCaseLowerNaming()
    {
        ...
        var json = JsonSerializer.Serialize(req, OpenTailStingrayJsonContext.Default.ChatCompletionRequest);
    ...
    public void Default_OmitsNullFieldsWhenWriting()
    {
        ...
        var json = JsonSerializer.Serialize(content, OpenTailStingrayJsonContext.Default.AContent);
