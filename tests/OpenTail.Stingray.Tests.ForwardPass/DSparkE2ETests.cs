...
/// (model.safetensors + config.json) is absent, or when STINGRAY_SKIP_E2E=1 —
...
public sealed class DSparkE2ETests
{
    ...
    private static string? FindDSparkHeadDir()
    {
        ...
        string[] absolute =
        {
            ...
            $@"C:\p\opentail-llm\models\{HeadDirName}",
        };
    ...
    // SnapKV pinned off: an inherited STINGRAY_SNAPKV_BUDGET would flip
    // SupportsHiddenTaps (and BatchVerify usability) to false on the CPU pass —
    // same pinning as CudaSpecBatchVerifyTests.NewFwd.
    private static OpenTail.Stingray.Engine.ForwardPass NewFwd(
        GgufModel model, CpuBackend cpu, ModelHyperparams hp, int ctx = 512)
    {
        ...
        try { return new OpenTail.Stingray.Engine.ForwardPass(model, cpu, hp, maxContextLength: ctx); }
