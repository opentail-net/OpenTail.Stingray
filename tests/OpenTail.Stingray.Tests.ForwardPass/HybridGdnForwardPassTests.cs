...
public sealed class HybridGdnForwardPassTests : HeavyTestBase
{
    ...
    /// file lives in <c>C:\p\opentail-llm\models</c> (17 GB), not <c>E:\models</c>.
    /// </summary>
    ...
    /// the same decode with <c>STINGRAY_DISABLE_MTP=1</c> on the same model and prompt.
    /// </summary>
    /// <remarks>
    /// <para>
    ...
    /// disabled via <c>STINGRAY_DISABLE_BATCH_VERIFY=1</c> for this test. That path
    ...
    /// MTP wiring bugs that the self-parity test (which compares opentail-llm
    /// to opentail-llm) cannot detect — concat order, eh_proj orientation,
    ...
    public async Task MtpDecoder_GreedyParity_LlamaCpp()
    {
        ...
        // model differing between opentail-llm and llama.cpp.
        ...
        // differently between opentail-llm's JinjaChatTemplate (which honours the
        ...
        // opentail-llm vs llama.cpp diverges at the very first emitted token for this
        // model — opentail-llm's logits rank `<|im_end|>` highest while llama.cpp ranks
        // a newline highest, producing an extra `<|im_end|>\n<|im_start|>assistant\n`
        // prelude before the model enters thinking mode. This is a MAIN-FORWARD
        // parity bug (opentail-llm-MTP and opentail-llm-noMTP outputs are bit-identical, so
        ...
        Assert.True(actualAnchor >= 0,
            $"opentail-llm output never produced the `<think>` anchor; cannot align for parity. " +
