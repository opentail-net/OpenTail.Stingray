...
/// <c>STINGRAY_KV_DTYPE=bf16</c> the K/V cache is stored half-width; kernel
...
public sealed class CudaForwardPassKvDtypeTests
{
    ...
    {
        var prevKv = Environment.GetEnvironmentVariable("STINGRAY_KV_DTYPE");
        ...
        var prevGrouped = Environment.GetEnvironmentVariable("STINGRAY_SPLIT_DECODE_GROUPED");
        Environment.SetEnvironmentVariable("STINGRAY_KV_DTYPE", kvDtype);
        Environment.SetEnvironmentVariable("STINGRAY_SNAPKV_BUDGET", "0"); // isolate the KV dtype
        // null → leave the flash-decoding split-KV default (on); "0" forces the single-block path.
        if (splitDecode is not null) Environment.SetEnvironmentVariable("STINGRAY_SPLIT_DECODE", splitDecode);
        // null → auto (grouped only for fp32 ≥24K); "1" forces grouped, "0" forces per-head (#237).
        if (groupedDecode is not null) Environment.SetEnvironmentVariable("STINGRAY_SPLIT_DECODE_GROUPED", groupedDecode);
        try
        {
        ...
        {
            Environment.SetEnvironmentVariable("STINGRAY_KV_DTYPE", prevKv);
            ...
            Environment.SetEnvironmentVariable("STINGRAY_SPLIT_DECODE_GROUPED", prevGrouped);
        }
    }
    ...
    /// split-on vs split-off (<c>STINGRAY_SPLIT_DECODE</c>). A long prompt (≈5000 tokens,
    ...
    public void Gemma4_E4B_SplitKv_GraphReplayCrossesBoundary()
    {
        ...
        Environment.SetEnvironmentVariable("STINGRAY_SPLIT_DECODE", "1");
        try
        {
            using var model = GgufModel.Open(path);
            var hp = ModelHyperparams.FromGgufMetadata(model.Metadata, model);
    ...
    // for all three thunks). We FORCE grouped (STINGRAY_SPLIT_DECODE_GROUPED=1) so a feasible
