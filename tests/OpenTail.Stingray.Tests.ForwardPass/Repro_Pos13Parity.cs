...
/// by default; runs only when STINGRAY_REPRO_POS13=1.
///
...
/// auto-inserts <c>&lt;think&gt;\n</c> after <c>&lt;|im_start|&gt;assistant\n</c>, so the model is
/// trained to predict the FIRST WORD OF THINKING CONTENT at this position
/// (e.g. opentail-llm 2026-05-26: token 8160 = "Here" @ logit 23.10 — confident).
/// The prior 13-token form `[..., 248045, 74455, 271]` was MALFORMED: token
/// 271 = `\n\n` (BPE-merged double newline) is a sequence the chat template
/// never produces, so the model was in OOD state and both opentail-llm and llama
...
public sealed class Repro_Pos13Parity
{
    private static int[] PromptTokens
    {
        get
        {
            var alt = Environment.GetEnvironmentVariable("STINGRAY_REPRO_TOKENS");
    ...
    public void DumpPos13Logits()
    {
        ...
        // Redirect stderr to a file so the STINGRAY_TRACE_LAYERS=1 trace ends up
        // somewhere we can compare offline. xUnit otherwise captures stderr
        // and only surfaces it on test failure.
        var traceFile = Environment.GetEnvironmentVariable("STINGRAY_TRACE_LAYERS") == "1"
            ? new StreamWriter(@"C:\p\opentail-llm\tmp\opentail-llm_layer_trace.txt") { AutoFlush = true }
            : null;
        ...
        var bypassTag =
            (Environment.GetEnvironmentVariable("STINGRAY_BYPASS_GDN")  == "1" ? "_nogdn"  : "") +
            (Environment.GetEnvironmentVariable("STINGRAY_BYPASS_ATTN") == "1" ? "_noattn" : "") +
            (Environment.GetEnvironmentVariable("STINGRAY_BYPASS_MOE")  == "1" ? "_nomoe"  : "");
        int predPos = PromptTokens.Length - 1;
        var outPath = Path.Combine(@"C:\p\opentail-llm\tmp", $"opentail-llm_pos{predPos}_logits{bypassTag}.txt");
        ...
        w.WriteLine($"# opentail-llm pos-{predPos} logits (predicts pos {predPos + 1}) on Qwen3.6-27B-MTP-Q4_K_M");
        w.WriteLine($"# prompt_tokens = [{string.Join(",", PromptTokens)}]");
        w.WriteLine($"# vocab_size = {logits.Length}");
        w.WriteLine($"# bypass: GDN={Environment.GetEnvironmentVariable("STINGRAY_BYPASS_GDN")} " +
                    $"ATTN={Environment.GetEnvironmentVariable("STINGRAY_BYPASS_ATTN")} " +
                    $"MOE={Environment.GetEnvironmentVariable("STINGRAY_BYPASS_MOE")}");
        ...
        // 248045 = `<|im_start|>`, 198 = `\n`, 271 = `\n\n` (BPE-merged),
        // 8160 = "Here" (opentail-llm's typical first-thinking-token), 31248 = "Okay".
