...
/// streams are compared. fp32 KV is pinned on the CUDA oracle (STINGRAY_KV_DTYPE=fp32) so the KV
...
public sealed class VulkanHybridGdnE2ETests
{
    ...
    {
        string[] absoluteRoots = { @"E:\models", @"C:\p\opentail-llm\models" };
    ...
    public void VulkanHybridGdn_Dense27B_MatchesCudaArgmax()
    {
        ...
        {
            Environment.SetEnvironmentVariable("STINGRAY_KV_DTYPE", prevKvDtype);
        }
    }
    ...
    /// <c>STINGRAY_VULKAN_BATCHED_PREFILL=1</c> (the batched trunk) and once with <c>=0</c> (the
    ...
    public void VulkanHybridGdn_BatchedPrefill_MatchesSequential()
    {
        ...
        {
            string? prev = Environment.GetEnvironmentVariable("STINGRAY_VULKAN_BATCHED_PREFILL");
            Environment.SetEnvironmentVariable("STINGRAY_VULKAN_BATCHED_PREFILL", gateVal);
            try
            {
                VulkanBackend gpu;
                try { gpu = new VulkanBackend(); }
            ...
            {
                Environment.SetEnvironmentVariable("STINGRAY_VULKAN_BATCHED_PREFILL", prev);
            }
        }
    ...
    /// <c>STINGRAY_VULKAN_GDN_CHUNKED_PREFILL</c> (1 = the chunk-parallel GdnChunkedPrefill, unset =
    ...
    public void VulkanHybridGdn_ChunkedPrefill_MatchesFusedScanArgmax()
    {
        ...
        {
            string? prev = Environment.GetEnvironmentVariable("STINGRAY_VULKAN_GDN_CHUNKED_PREFILL");
            Environment.SetEnvironmentVariable("STINGRAY_VULKAN_GDN_CHUNKED_PREFILL", gateVal);
            try
            {
                VulkanBackend gpu;
                try { gpu = new VulkanBackend(); }
            ...
            {
                Environment.SetEnvironmentVariable("STINGRAY_VULKAN_GDN_CHUNKED_PREFILL", prev);
            }
        }
    ...
    /// layers in VRAM, so both auto-select (and we pin via STINGRAY_CPU_MOE=1) the CPU-MoE
    ...
    public void VulkanHybridGdn_Moe35B_MatchesCudaArgmax()
    {
        ...
        string? prevKvDtype = Environment.GetEnvironmentVariable("STINGRAY_KV_DTYPE");
        string? prevCpuMoe = Environment.GetEnvironmentVariable("STINGRAY_CPU_MOE");
        Environment.SetEnvironmentVariable("STINGRAY_KV_DTYPE", "fp32");
