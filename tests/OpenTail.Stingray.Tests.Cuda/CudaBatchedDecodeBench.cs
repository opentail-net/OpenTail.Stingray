...
public sealed class CudaBatchedDecodeBench
{
    ...
    // Profiler ergonomics (#197 follow-up): STINGRAY_BENCH_BATCH_N="8" runs only those batch
    // sizes (comma list; default 1,2,4,8), STINGRAY_BENCH_SINGLE=0 skips the single-user
    // baseline, and STINGRAY_BENCH_STEPS shrinks the timed loop (ncu's launch interception
    ...
    private static bool SingleUserEnabled =>
        Environment.GetEnvironmentVariable("STINGRAY_BENCH_SINGLE") != "0";
    ...
    public void Decode_Throughput_SingleUser_vs_Batched()
    {
        ...
        foreach (int n in BatchSizesToRun)
        {
            ...
            try
            {
                ...
                // #205/#206: STINGRAY_BENCH_ARGMAX=1 measures the on-device argmax tail
                ...
                bool useArgmax = Environment.GetEnvironmentVariable("STINGRAY_BENCH_ARGMAX") == "1";
    ...
    ///
    /// Opt-in: $env:STINGRAY_BENCH_BATCH=1; dotnet test ... --filter
    ...
    public void Decode_Throughput_SnapKvEvicted_Ragged_vs_PerSeq()
    {
        ...
        var prevWindow = Environment.GetEnvironmentVariable("STINGRAY_SNAPKV_WINDOW");
        var prevSlots = Environment.GetEnvironmentVariable("STINGRAY_PREFIX_SLOTS");
        Environment.SetEnvironmentVariable("STINGRAY_SNAPKV_BUDGET", "512");
        Environment.SetEnvironmentVariable("STINGRAY_SNAPKV_WINDOW", "32");
        Environment.SetEnvironmentVariable("STINGRAY_PREFIX_SLOTS", null);
        CudaForwardPass fwdTmp;
        try { fwdTmp = new CudaForwardPass(model, gpu, hp, maxContextLength: 2048); }
        finally
        {
            Environment.SetEnvironmentVariable("STINGRAY_SNAPKV_BUDGET", prevBudget);
            Environment.SetEnvironmentVariable("STINGRAY_SNAPKV_WINDOW", prevWindow);
            Environment.SetEnvironmentVariable("STINGRAY_PREFIX_SLOTS", prevSlots);
        }
        using var fwd = fwdTmp;
