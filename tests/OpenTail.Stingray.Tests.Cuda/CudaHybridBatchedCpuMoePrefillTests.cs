...
/// With <c>STINGRAY_HYBRID_BATCHED_MOE=1</c> the batched-trunk prefill replaces the per-token
...
///
/// <c>STINGRAY_CPU_MOE=1</c> is pinned for both arms so the pass deterministically selects
...
public sealed class CudaHybridBatchedCpuMoePrefillTests : IDisposable
{
    ...
    /// per-token reference (on non-AVX-512 boxes STINGRAY_Q4K_Q8K auto-enables and would
    ...
    private static IDisposable PinQ8KOff() => new EnvPin(
        ("STINGRAY_Q3K_Q8K", "0"), ("STINGRAY_Q8_0_Q8K", "0"), ("STINGRAY_Q4K_Q8K", "0"));
    ...
    /// (STINGRAY_Q4K_Q8K=1 etc.), the batched CPU-MoE prefill routes the gate/up (and,
        ...
        try
        {
            ...
            float[] Run(string q8k)
            {
                using var pin = new EnvPin(
                    ("STINGRAY_Q3K_Q8K", q8k), ("STINGRAY_Q8_0_Q8K", q8k), ("STINGRAY_Q4K_Q8K", q8k));
                using var fwd = TryConstruct(model, gpu, hp, placement);
                if (fwd is null) return Array.Empty<float>();
                var logits = fwd.Prefill(tokens).ToArray();
                Assert.True(fwd.LastPrefillWasBatched && fwd.LastPrefillUsedBatchedCpuMoe,
