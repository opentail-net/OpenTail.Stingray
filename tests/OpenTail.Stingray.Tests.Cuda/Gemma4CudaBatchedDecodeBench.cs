...
/// <b>Q4_K 12B</b> (<c>STINGRAY_BENCH_MODEL=gemma-4-12b-it-Q4_K_M.gguf</c>) so the decode-MMQ tile
...
///   $env:STINGRAY_BENCH_BATCH=1; dotnet test ... --filter "FullyQualifiedName~Gemma4CudaBatchedDecodeBench"
///   # decode-MMQ vs WS A/B on the Q4_K 12B (#283): force WS everywhere with STINGRAY_BATCH_DECODE_MMQ=0
///   $env:STINGRAY_BENCH_BATCH=1; $env:STINGRAY_BENCH_MODEL="gemma-4-12b-it-Q4_K_M.gguf";
///   $env:STINGRAY_BENCH_BATCH_N="2,4,5,6,8"; dotnet test ... (default = MMQ on, then re-run with
///   $env:STINGRAY_BATCH_DECODE_MMQ="0" = WS everywhere)
///   # old all-GEMM routing (#195 baseline):
///   $env:STINGRAY_BENCH_BATCH=1; $env:STINGRAY_BATCH_DECODE_GEMM=1; dotnet test ... (same filter)
...
public sealed class Gemma4CudaBatchedDecodeBench
{
    ...
    private static string ModelFile =>
        Environment.GetEnvironmentVariable("STINGRAY_BENCH_MODEL") is { Length: > 0 } m
            ? m
    ...
    private static bool BenchEnabled => Environment.GetEnvironmentVariable("STINGRAY_BENCH_BATCH") == "1";

    private static int[] BatchSizesToRun =>
        (Environment.GetEnvironmentVariable("STINGRAY_BENCH_BATCH_N") ?? "1,2,4,8")
        .Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries)
        .Select(int.Parse).ToArray();
    private static int DecodeSteps =>
        int.TryParse(Environment.GetEnvironmentVariable("STINGRAY_BENCH_STEPS"), out int s) && s > 0 ? s : 128;
    // The decode loop only touches ~prompt+steps positions, so a smaller ctx is fine and lets the
    // bigger-KV Q4_K 12B fit N=8 caches in 12 GB (STINGRAY_BENCH_CTX, default 2048 — unchanged for E4B).
    private static int MaxCtx =>
        int.TryParse(Environment.GetEnvironmentVariable("STINGRAY_BENCH_CTX"), out int c) && c > 0 ? c : 2048;
    ...
    {
        // STINGRAY_BENCH_MODEL may be an absolute path; honor it directly.
        if (Path.IsPathRooted(ModelFile)) return File.Exists(ModelFile) ? ModelFile : null;

        string[] absolute = { $@"E:\models\{ModelFile}", $@"C:\p\opentail-llm\models\{ModelFile}" };
    ...
    public void Gemma4_E4B_Decode_Throughput_Batched()
    {
        ...
        Environment.SetEnvironmentVariable("STINGRAY_SNAPKV_BUDGET", "0");
        CudaForwardPass fwdTmp;
        try { fwdTmp = new CudaForwardPass(model, gpu, hp, maxContextLength: MaxCtx); }
        finally { Environment.SetEnvironmentVariable("STINGRAY_SNAPKV_BUDGET", prevSnap); }
        using var fwd = fwdTmp;

        bool gemm = Environment.GetEnvironmentVariable("STINGRAY_BATCH_DECODE_GEMM") == "1";
        bool mmqKill = Environment.GetEnvironmentVariable("STINGRAY_BATCH_DECODE_MMQ") == "0";
        string routing = gemm
