...
public sealed class CudaSnapKvBatchingTests
{
    ...
    {
        string[] absolute = { $@"C:\p\opentail-llm\models\{ModelFile}", $@"E:\models\{ModelFile}" };
    ...
    {
        return new EnvScope(
            ("STINGRAY_SNAPKV_BUDGET", budget.ToString()),
            ("STINGRAY_SNAPKV_WINDOW", window.ToString()),
            ("STINGRAY_PREFIX_SLOTS", null)); // ensure multi-slot doesn't suppress the budget
    }
