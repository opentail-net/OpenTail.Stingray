...
public sealed class CudaHybridGdnForwardPassTests
{
    ...
    /// <summary>
    /// CPU-MoE experiment (STINGRAY_CPU_MOE=1): keep attention on GPU and route
    ...
    public void CudaHybridGdnForwardPass_Qwen35Moe_CpuMoeMode_MatchesCpuBaseline()
    {
        ...
        try
        {
            ...
            Assert.True(produced == referenceFirstToken,
                ...
                "the STINGRAY_CPU_MOE branch.");
        }
        finally
        {
            Environment.SetEnvironmentVariable("STINGRAY_CPU_MOE", prev);
        }
    }
    ...
    public void CudaHybridGdnForwardPass_Qwen35Mtp_Bf16KvCache_GreedyMatchesFp32()
    {
        ...
        finally { Environment.SetEnvironmentVariable("STINGRAY_KV_DTYPE", prev); }

        // Candidate: bf16 KV cache (the new default).
        Environment.SetEnvironmentVariable("STINGRAY_KV_DTYPE", "bf16");
        int bf16Top1;
        float[] bf16TopK;
        try
        {
    ...
    public void CudaHybridGdnForwardPass_SplitKv_MatchesSingleBlock()
    {
        ...
        {
            var pKv = Environment.GetEnvironmentVariable("STINGRAY_KV_DTYPE");
            var pMoe = Environment.GetEnvironmentVariable("STINGRAY_CPU_MOE");
            var pSp = Environment.GetEnvironmentVariable("STINGRAY_SPLIT_DECODE");
            Environment.SetEnvironmentVariable("STINGRAY_KV_DTYPE", "bf16");
            Environment.SetEnvironmentVariable("STINGRAY_CPU_MOE", "1");
            Environment.SetEnvironmentVariable("STINGRAY_SPLIT_DECODE", split);
            try
            {
                using var model = GgufModel.Open(path);
                var hp = ModelHyperparams.FromGgufMetadata(model.Metadata, model);
            ...
            {
                Environment.SetEnvironmentVariable("STINGRAY_KV_DTYPE", pKv);
                Environment.SetEnvironmentVariable("STINGRAY_CPU_MOE", pMoe);
                Environment.SetEnvironmentVariable("STINGRAY_SPLIT_DECODE", pSp);
            }
        }
