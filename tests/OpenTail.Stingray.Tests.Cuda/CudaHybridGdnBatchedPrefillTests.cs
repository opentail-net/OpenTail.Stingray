...
public sealed class CudaHybridGdnBatchedPrefillTests : IDisposable
{
    ...
    public void BatchVerifyCpuMoe_BitwiseMatchesPerToken_Carnice()
    {
        ...
        // loop it replaces. Toggle ONLY STINGRAY_MTP_BATCHED_MOE_VERIFY between the two
    ...
    public void BatchedAttnWave_BitwiseMatchesPerPosition_Over4096_Carnice()
    {
        ...
        Environment.SetEnvironmentVariable("STINGRAY_SPLIT_DECODE", "0");
        ...
        try
        {
            ...
            float[] RunWith(bool batchedAttn, string? budgetMb)
            {
                ...
                Environment.SetEnvironmentVariable("STINGRAY_ATTN_WAVE_BUDGET_MB", budgetMb);
                using var fwd = new CudaHybridGdnForwardPass(model, gpu, hp, placement);
        ...
        finally
        {
            ...
            Environment.SetEnvironmentVariable("STINGRAY_CPU_MOE", prevCpuMoe);
            Environment.SetEnvironmentVariable("STINGRAY_ATTN_WAVE_BUDGET_MB", prevBudget);
            Environment.SetEnvironmentVariable("STINGRAY_SPLIT_DECODE", prevSplit);
        }
    }
    ...
    /// (<c>STINGRAY_BATCHED_TRUNK=0</c>). Both run under batched prefill; only the trunk
    ...
    public void BatchedTrunk_BitwiseMatchesSequentialTrunk_Carnice()
    {
        ...
        try
        {
            ...
            Assert.True(firstDiff < 0,
                ...
                "TrunkLayerBatched must be bit-identical to TrunkLayerSequential (STINGRAY_BATCHED_TRUNK=0).");
    ...
    /// bit-identical to the per-token FFN fallback (<c>STINGRAY_BATCHED_FFN=0</c>). Both arms
    ...
    public void BatchedFfn_BitwiseMatchesPerTokenFfn_Dense27BMtp()
    {
        ...
        try
        {
            ...
            Assert.True(firstDiff < 0,
                ...
                "BatchedGpuDenseFfn must be bit-identical to the per-token GpuDenseFfn loop (STINGRAY_BATCHED_FFN=0).");
    ...
    /// <c>STINGRAY_BATCHED_CPU_FFN=1</c>) must be bit-identical to the per-token
    /// Download → <c>CpuDenseFfn</c> → UploadInto loop it replaces. Both arms run under
    /// batched prefill + batched trunk with <c>STINGRAY_DENSE_FFN_GPU_MARGIN_MB</c> set
    ...
    public void BatchedCpuDenseFfn_BitwiseMatchesPerTokenCpuFfn_Dense27BMtp()
    {
        ...
        var prevMargin = Environment.GetEnvironmentVariable("STINGRAY_DENSE_FFN_GPU_MARGIN_MB");
        Environment.SetEnvironmentVariable("STINGRAY_DENSE_FFN_GPU_MARGIN_MB", "1000000");
        try
        {
            using var model = GgufModel.Open(path);
            var hp = ModelHyperparams.FromGgufMetadata(model.Metadata, model);
        ...
        finally
        {
            ...
            Environment.SetEnvironmentVariable("STINGRAY_DENSE_FFN_GPU_MARGIN_MB", prevMargin);
        }
    }
    ...
    /// to the sequential per-token <c>Forward</c> loop (<c>STINGRAY_BATCHED_PREFILL=0</c>) for
    ...
    /// With a long-enough prompt and a small <c>STINGRAY_SNAPKV_BUDGET</c> the batched path must
    ...
    public void BatchedTrunkGpuFfn_SnapKvActive_BitwiseMatchesSequential_Dense27BMtp()
    {
        ...
        Environment.SetEnvironmentVariable("STINGRAY_SNAPKV_BUDGET", "128");
        bool prevBatched = CudaHybridGdnForwardPass.BatchedPrefillEnabled;
        try
        {
            using var model = GgufModel.Open(path);
        ...
        {
            CudaHybridGdnForwardPass.BatchedPrefillEnabled = prevBatched;
            Environment.SetEnvironmentVariable("STINGRAY_SNAPKV_BUDGET", prevSnap);
        }
    }
    ...
    /// <c>STINGRAY_BATCHED_PREFILL=1</c>) against the sequential per-token <c>Forward</c> loop
    ...
    /// MoE model forced onto the on-GPU routed-expert path with <c>STINGRAY_CPU_MOE=0</c>
    ...
    public void BatchedTrunkGpuFfn_BitwiseMatchesSequential_GpuSlruMoe()
    {
        ...
        var prevCpuMoe = Environment.GetEnvironmentVariable("STINGRAY_CPU_MOE");
        Environment.SetEnvironmentVariable("STINGRAY_CPU_MOE", "0"); // force GPU-SLRU MoE
        bool prevBatched = CudaHybridGdnForwardPass.BatchedPrefillEnabled;
