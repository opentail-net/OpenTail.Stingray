...
public sealed class PipelineTests
{
    ...
    {
        var cache = new OpenTail.Stingray.Pipeline.ExpertCache<int>(capacity: 4);
    ...
    {
        var cache = new OpenTail.Stingray.Pipeline.ExpertCache<string>(capacity: 4);
    ...
    {
        string? evicted = null;
        var cache = new OpenTail.Stingray.Pipeline.ExpertCache<string>(capacity: 2, onEvict: v => evicted = v);
        cache.Put(0, 0, "a");
        cache.Put(0, 1, "b");
    ...
    {
        var p = new OpenTail.Stingray.Pipeline.ExpertAccessProfiler(numLayers: 2, numExperts: 4);
    ...
    {
        var p = new OpenTail.Stingray.Pipeline.ExpertAccessProfiler(numLayers: 1, numExperts: 2);
        p.RecordMiss(0, 0);
    ...
    {
        var cache = new OpenTail.Stingray.Pipeline.ExpertCache<int>(capacity: 8);
        cache.Put(0, 0, 100);
        cache.Put(1, 0, 200);
    ...
    {
        var cache = new OpenTail.Stingray.Pipeline.SlruCache<int, string>(probationaryCapacity: 2, protectedCapacity: 2);
    ...
    {
        var profiler = new OpenTail.Stingray.Pipeline.ExpertAccessProfiler(numLayers: 2, numExperts: 4);
    ...
    {
        var profiler = new OpenTail.Stingray.Pipeline.ExpertAccessProfiler(numLayers: 1, numExperts: 4);
    ...
    public void SlruCache_FrequencyAware_EvictsLeastAccessed_NotLruTail()
    {
        ...
        var cache = new OpenTail.Stingray.Pipeline.SlruCache<int, string>(
    ...
    public void SlruCache_FrequencyAware_HotKeysSurviveSkewedChurn()
    {
        ...
        var lru = new OpenTail.Stingray.Pipeline.SlruCache<int, string>(probationaryCapacity: 2, protectedCapacity: 2);
        var freqAware = new OpenTail.Stingray.Pipeline.SlruCache<int, string>(probationaryCapacity: 2, protectedCapacity: 2, frequencyOf: Freq);
    ...
    {
        var cache = new OpenTail.Stingray.Pipeline.SlruCache<int, string>(probationaryCapacity: 2, protectedCapacity: 1);
        cache.Put(1, "a", out _, out _);
    ...
    {
        var cache = new OpenTail.Stingray.Pipeline.SlruCache<int, string>(probationaryCapacity: 1, protectedCapacity: 1);
        cache.Put(1, "a", out _, out _);
        cache.Pin(1);
        Assert.True(cache.IsPinned(1));
        cache.Unpin(1);
    ...
    public void ExpertCache_Pin_KeepsHotExpertResidentUnderChurn()
    {
        ...
        var cache = new OpenTail.Stingray.Pipeline.ExpertCache<string>(capacity: 4, onEvict: evicted.Add);
        cache.Put(0, 7, "hot");
        cache.Pin(0, 7);
    ...
    public void ExpertCache_FrequencyAware_EvictsLeastAccessedExpert()
    {
        ...
        string? evicted = null;
        var cache = new OpenTail.Stingray.Pipeline.ExpertCache<string>(
    ...
    {
        var p = new OpenTail.Stingray.Pipeline.ExpertRoutePredictor(numLayers: 4, maxActiveExperts: 8);
    ...
    {
        var p = new OpenTail.Stingray.Pipeline.ExpertRoutePredictor(numLayers: 2, maxActiveExperts: 4);
    ...
    {
        var p = new OpenTail.Stingray.Pipeline.ExpertRoutePredictor(numLayers: 1, maxActiveExperts: 2);
    ...
    public void MoeCacheSizing_AmpleVram_CapsAtTotalExperts()
    {
        ...
        var plan = OpenTail.Stingray.Pipeline.MoeCacheSizing.Plan(
    ...
    {
        var plan = OpenTail.Stingray.Pipeline.MoeCacheSizing.Plan(0, 64, 8, 1L << 30, 1L << 20, 0);
        Assert.Equal(0, plan.Slots);
        Assert.Equal(OpenTail.Stingray.Pipeline.MoeCacheSizingStatus.Empty, plan.Status);
    }
    ...
    public void MoeCacheSizing_AmpleVram_StatusIsOk()
    {
        var plan = OpenTail.Stingray.Pipeline.MoeCacheSizing.Plan(
            gpuLayers: 8, numExperts: 64, numActiveExperts: 8,
            freeVramBytes: 100L << 30, perExpertBytes: 2L << 20, reserveBytes: 512L << 20);
        Assert.Equal(OpenTail.Stingray.Pipeline.MoeCacheSizingStatus.Ok, plan.Status);
    }
    ...
    public void MoeCacheSizing_TightVram_StatusIsBelowRecommended()
    {
        ...
        var plan = OpenTail.Stingray.Pipeline.MoeCacheSizing.Plan(
            gpuLayers: 8, numExperts: 64, numActiveExperts: 8,
            freeVramBytes: free, perExpertBytes: perExpert, reserveBytes: 512L << 20);
        Assert.Equal(OpenTail.Stingray.Pipeline.MoeCacheSizingStatus.BelowRecommended, plan.Status);
    }
    ...
    public void MoeCacheSizing_BudgetExhausted_ClampsToOneAndFlagsStatus()
    {
        ...
        Assert.Equal(OpenTail.Stingray.Pipeline.MoeCacheSizingStatus.BudgetExhausted, plan.Status);
    }
    ...
    public void MoeCacheSizing_UnknownExpertSize_FlagsStatus()
    {
        ...
        Assert.Equal(OpenTail.Stingray.Pipeline.MoeCacheSizingStatus.UnknownExpertSize, plan.Status);
    }
}
