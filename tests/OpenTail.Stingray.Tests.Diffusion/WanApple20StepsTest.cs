...
public sealed class WanApple20StepsTest
{
    ...
    public void GenerateApple_20Steps_Wan21()
    {
        ...
        var (rawCond, rawUncond) = umt5.EncodePairGpu(tokens, uncondTokens, vulkan);
        swText.Stop();
        string msgText = $"[TestProfile] Text encoding (cond + uncond) took {swText.ElapsedMilliseconds} ms";
        _output.WriteLine(msgText);
        ...
        swLoad.Stop();
        string msgLoad = $"[TestProfile] Pipeline load took {swLoad.ElapsedMilliseconds} ms";
        ...
        swGen.Stop();
        swTotal.Stop();
        string msgGen = $"[TestProfile] pipeline.Generate total took {swGen.ElapsedMilliseconds} ms";
        string msgTot = $"[TestProfile] Full test total took {swTotal.ElapsedMilliseconds} ms";
    ...
    [Fact]
    public void GenerateApple_20Steps_Wan21_Cpu()
    {
        string tokPath = FindModelPath(Path.Combine("models", "wan2.1", "umt5-tokenizer.json"));
        string encPath = FindModelPath(Path.Combine("models", "wan2.1", "models_t5_umt5-xxl-enc-bf16.safetensors"));
        string ditPath = FindModelPath(Path.Combine("models", "wan2.1", "wan2.1-t2v-1.3b-dit.safetensors"));
        ...
        var (rawCond, rawUncond) = umt5.EncodePairGpu(tokens, uncondTokens, vulkan);
        swText.Stop();
        string msgText = $"[TestProfile CPU] Text encoding (cond + uncond) took {swText.ElapsedMilliseconds} ms";
        _output.WriteLine(msgText);
        Console.WriteLine(msgText);
        ...
        // 2. Load Pipeline on CPU (backend: null)
        var swLoad = System.Diagnostics.Stopwatch.StartNew();
        using var pipeline = WanPipeline.Load(ditPath, vaePath, backend: null);
        swLoad.Stop();
        string msgLoad = $"[TestProfile CPU] Pipeline load took {swLoad.ElapsedMilliseconds} ms";
        _output.WriteLine(msgLoad);
        Console.WriteLine(msgLoad);

        string outputPath = Path.Combine(@"c:\Git-Public\OpenTail.Stingray", "wan_apple_cpu_20steps.png");

        // 3. Generate 20 steps on CPU
        var swGen = System.Diagnostics.Stopwatch.StartNew();
        pipeline.Generate(
            prompt: prompt,
            negativePrompt: "",
        ...
        swGen.Stop();
        swTotal.Stop();
        string msgGen = $"[TestProfile CPU] pipeline.Generate CPU took {swGen.ElapsedMilliseconds} ms";
        string msgTot = $"[TestProfile CPU] Full CPU test total took {swTotal.ElapsedMilliseconds} ms";
        _output.WriteLine(msgGen);
        _output.WriteLine(msgTot);
        Console.WriteLine(msgGen);
        Console.WriteLine(msgTot);

        Assert.True(File.Exists(outputPath), "Output CPU image was not generated.");
    }
}
