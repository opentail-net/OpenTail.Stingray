...
/// <summary>
/// TEMPORARY, throwaway perf timing bench for Stable Audio 3 Small Music for
/// PerformanceLeague.md backfill. Small Music is the golden-verified DiT/VAE reference this
/// project's other Stable Audio 3 variants (SFX, Medium) reuse -- see StableAudio3SmallSfxTests'
/// doc comment. No C++ reference exists for this pipeline. Not part of the permanent suite,
/// delete after use.
/// </summary>
public sealed class StableAudio3SmallMusicPerfBaselineDebugTest
{
    ...
    public void Bench_Generate_3sAudio_8Steps()
    {
        ...
        string msg = $"[StableAudio3-SmallMusic] prompt=\"{prompt}\" steps={steps} audio={audioSec:F2}s samples={sampleCount}\n" +
                     $"[StableAudio3-SmallMusic] runs(s)=[{string.Join(", ", elapsedSec.Select(x => x.ToString("F3")))}] mean={meanSec:F3}s RTF={rtf:F3} (lower=faster; 1.0=realtime; {steps}-step CFG, not the full recommended step count)";
