...
public sealed class VulkanSpecBatchVerifyTests : HeavyTestBase
{
        ...
        int ctx = 512, DType kvDtype = DType.Float32)
    {
        ...
        finally { Environment.SetEnvironmentVariable("STINGRAY_SNAPKV_BUDGET", prev); }
    }
    ...
    /// the K-loop) and logs the real ratio. Skips unless STINGRAY_RUN_VULKAN_SPEC_BENCH=1 (it's a
    /// timing probe, not a correctness gate).
    /// </summary>
    [Fact]
    public void Qwen3_8B_Q4K_BatchVerify_MicroBench()
    {
        if (Environment.GetEnvironmentVariable("STINGRAY_RUN_VULKAN_SPEC_BENCH") != "1")
            return;
        ...
        DType kvDtype = Environment.GetEnvironmentVariable("STINGRAY_SPEC_BENCH_KV")?.ToLowerInvariant() switch
        {
            "bf16" => DType.BFloat16,
    ...
    /// <summary>
    /// Micro-bench (GPU-only, gated on STINGRAY_RUN_VULKAN_SPEC_BENCH=1): the PER-MATVEC arbiter for
