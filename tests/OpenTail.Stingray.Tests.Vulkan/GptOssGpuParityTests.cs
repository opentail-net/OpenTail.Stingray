...
/// <summary>
/// gpt-oss-20b on Vulkan (<see cref="GptOssGpuForwardPass"/>) against the CPU
/// <see cref="GptOssForwardPass"/> (itself matched to llama-server, docs/101): a real prompt, then
...
/// </summary>
public sealed class GptOssGpuParityTests : HeavyTestBase
{
    ...
    public void PrefillAndDecodeLogits_AgreeWithCpu()
    {
        ...
        using (var fwd = new GptOssGpuForwardPass(model, gpu!, hp, maxContextLength: 512))
        {
        ...
        Console.WriteLine($"[gptoss-parity] worst cosine {worst:F6}, argmax flips {flips}/{cpu.Count}");
