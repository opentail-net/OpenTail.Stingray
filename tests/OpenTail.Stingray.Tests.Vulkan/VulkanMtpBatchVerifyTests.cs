...
/// </summary>
public sealed class VulkanMtpBatchVerifyTests : HeavyTestBase
{
    ...
    /// <summary>
    /// Constructs the pass with a 4-token snapshot ring. STINGRAY_MTP_BATCH_MAX is instance-resolved
    ...
    private static VulkanHybridGdnForwardPass? CreatePass(GgufModel model, VulkanBackend gpu,
                                                          ModelHyperparams hp)
    {
        var prev = Environment.GetEnvironmentVariable("STINGRAY_MTP_BATCH_MAX");
        Environment.SetEnvironmentVariable("STINGRAY_MTP_BATCH_MAX", "4");
        try
        {
        ...
        {
            Environment.SetEnvironmentVariable("STINGRAY_MTP_BATCH_MAX", prev);
        }
    }
