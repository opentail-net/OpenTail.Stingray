# MoE & Expert Offloading: State of the Art vs. OpenTail.Stingray

> Research note, 2026-05. Surveys the current literature on Mixture-of-Experts
> (MoE) inference and expert offloading, audits what OpenTail.Stingray does today
...
>   slot managers support opt-in warm-pinning via `STINGRAY_MOE_WARMPIN`
>   (commit `b6b2763`).
...
OpenTail.Stingray already has a **solid offloading skeleton** that maps onto several
...
## 2. What OpenTail.Stingray does today

Verified against the source on this branch.

### 2.1 MoE model support
- Architecture/hparam detection in `src/OpenTail.Stingray.Core/ModelGraph.cs` and
...
- **CUDA GDN hybrid** (`CudaHybridGdnForwardPass.cs`, for qwen35moe): the most
  ...
  (`STINGRAY_CPU_MOE=1`), and dumps `ExpertAccessProfiler` stats on dispose.
...
### 2.3 Offloading infrastructure (`OpenTail.Stingray.Pipeline` + Engine)
...
| SOTA capability | OpenTail.Stingray today | Gap |
|---|---|---|
...
| Cache-miss CPU compute (Fiddler) | CPU-fallback compute on Vulkan + (via `STINGRAY_CPU_MOE`) GDN paths | *Per-dispatch* CPU/GPU decision tracked as #54. |
