...
- `src/OpenTail.Stingray.Cuda/CudaTextKernels.cs` — add `llm_matvec_q8_0` (clone of `llm_matvec_q4_k_m` pattern), `llm_embed_lookup_q8_0`
- `src/OpenTail.Stingray.Cuda/CudaBackend.cs:1157-1390` — extend MatMul dispatch with Q8_0 branch
- `src/OpenTail.Stingray.Cuda/CudaBackend.cs:2598-2650` — resolve via GetKernelFunc with extern-C names
- `src/OpenTail.Stingray.Cuda/CudaBackend.cs:2575-2590` — add to ForceEagerJit
- `src/OpenTail.Stingray.Engine/CudaForwardPass.cs:1145-1192` — UploadExpertWeight Q8_0 raw-upload branch (not dequant-to-F32)
- `src/OpenTail.Stingray.Engine/CudaForwardPass.cs:1282-1287` — EstimateGpuTensorBytes Q8_0 sizing
...
- `src/OpenTail.Stingray.Core/ModelGraph.cs:33-34` — add scalar properties: `EmbeddingScale`, `FinalLogitSoftcap`, `RopeThetaSwa`
- `src/OpenTail.Stingray.Core/ModelGraph.cs:86-92` — add: `SlidingWindowSize`, `PerLayerEmbeddingWidth`, `HasPostAttnNorm`, `HasPostFfwNorm`, `HasPerLayerTokenEmbd`, `FfnActivation`, `IReadOnlyList<bool>? IsSwaLayer`, `IReadOnlyList<int>? KvSourceLayer`, `IReadOnlyList<int>? LayerHeadDim`, `IReadOnlyList<int>? LayerKvHeadDim`, `IReadOnlyList<int>? LayerRopeDim`
- `src/OpenTail.Stingray.Core/ModelGraph.cs:184-191` — `bool isGemma4` block
- `src/OpenTail.Stingray.Core/ModelGraph.cs:229-269` — parse all `gemma4.attention.*`, `gemma4.rope.*_swa`, `gemma4.final_logit_softcapping`, `gemma4.embedding_length_per_layer_input`, build per-layer `IsSwaLayer[]` from `gemma4.attention.sliding_window_pattern` bool[], derive `KvSourceLayer[]` from `gemma4.attention.shared_kv_layers` (llama.cpp rule: `il<NLayerKvFromStart → -1`, else `NLayerKvFromStart-2` if SWA else `NLayerKvFromStart-1`), populate per-layer head_dim arrays
- `src/OpenTail.Stingray.Core/ModelGraph.cs:271-305` — initializer block
- `src/OpenTail.Stingray.Core/ModelGraph.cs:330+` — add `enum FfnActivation { Silu, GeluApprox }`
- `src/OpenTail.Stingray.Core/ModelGraph.cs:308-312` — add `GetBoolArray` helper
- `src/OpenTail.Stingray.Core/GgufModel.cs:123-137` — synthetic `_opentailllm.has_ple`, `_opentailllm.has_post_attn_norm`, etc.
...
- `src/OpenTail.Stingray.Cpu/SimdKernels.cs` — `GeluTanhMul(gate, up, n)` (AVX2 + scalar fallback for parity tests), `ScaleInPlace(x, n, scale)`, `SoftcapInPlace(x, n, cap)`
...
- `src/OpenTail.Stingray.Engine/CudaForwardPass.cs:268-313` — per-layer KV cache allocation with per-layer window + alias-mode (track aliased layers to skip double-free at Dispose)
- `src/OpenTail.Stingray.Engine/CudaForwardPass.cs:152-216` — load PLE refs as CPU-resident (existing `_cpuEmbedding` precedent for large tables)
- `src/OpenTail.Stingray.Engine/TierPlanner.cs:32-47` — PLE on CPU unconditionally
- `src/OpenTail.Stingray.Engine/CudaForwardPass.cs:1215-1244` — EstimateMaxContext: subtract PLE bytes; size per-layer KV by `min(maxSeqLen, window)`
