# OpenTail.Stingray — Design Document
...
straight into the LLM embedding space. OpenTail.Stingray implements **image → text** for this model
on CPU and CUDA (issue #250); `OpenTail.Stingray.Vision` holds the vision-side code.
...
```
opentail-llm-cli -m gemma-4-12b-it-qat-q4_0.gguf \
...
OpenTail.Stingray exposes an HTTP API server compatible with the Anthropic Messages API, enabling drop-in use with existing client libraries, SDKs, and tools that target the Anthropic API. An OpenAI-compatible endpoint is also provided for broader ecosystem compatibility.
...
```json
{
  "model": "opentail-llmnference-qwen3-30b-a3b",
  "max_tokens": 1024,
  "messages": [
...
```
event: message_start
data: {"type":"message_start","message":{"id":"msg_...","type":"message","role":"assistant","content":[],"model":"opentail-llmnference-qwen3-30b-a3b","usage":{"input_tokens":42}}}
...
```json
{
  "data": [
    {
      "id": "opentail-llmnference-qwen3-30b-a3b",
      "object": "model",
      "owned_by": "local",
      "capabilities": {
        "max_context": 65536,
...
- [x] CLI chat REPL (`opentail-llm-cli`) with llama.cpp-compatible flags
...
- [x] Attention bias support: optional Q/K/V/O bias tensors (detected via `_opentailllm.has_attn_bias` synthetic metadata)
...
- [x] Per-head QK-norm: RMSNorm on Q/K per head (CPU + HeadNorm GPU shader), detected via `_opentailllm.has_qk_norm`
...
- CPU (`-g 0`) with `STINGRAY_TRACE_NORMS=1`: per-layer residual L2 norms grow smoothly
...
`STINGRAY_ALLOW_BROKEN_MOE_HYBRID=1` to bypass for further investigation.
...
**Key files:**
- `src/OpenTail.Stingray.Core/IForwardPass.cs` — `Forward`, `TruncateTo`, `VocabSize`, `MaxSeqLen` interface.
- `src/OpenTail.Stingray.Engine/ForwardPass.cs` — `BatchVerify(int[] tokens, int startPos)` (sequential fallback for MoE; throws for TurboQuant).
- `src/OpenTail.Stingray.Engine/KvCache.cs`, `TurboQuantKvCache.cs` — `TruncateTo(int length)`.
- `src/OpenTail.Stingray.Engine/SpeculativeDecoder.cs` — full greedy speculative decode loop.
- `src/OpenTail.Stingray.Cli/RunCommand.cs` — `--draft-model` and `--spec-lookahead` flags.
- `benchmarks/OpenTail.Stingray.Bench/InferenceBenchmark.cs` — `SpeculativeDecodingBenchmark` class.
- `src/OpenTail.Stingray.Cpu/Dequantize.cs` — added Q4_0, Q5_0, Q8_0, F16, BF16 dequantization.

**CLI usage:**
```
opentail-llm -m SmolLM2-1.7B-Instruct-Q4_K_M.gguf \
...
- Override: `STINGRAY_MIN_BATCH_BLAS=N` environment variable
...
- [x] Configuration via `STINGRAY_MODEL` / `STINGRAY_N_GPU_LAYERS` env vars and `appsettings.json`
...
```bash
# Start server (CPU)
STINGRAY_MODEL=models/SmolLM2-1.7B-Instruct-Q4_K_M.gguf dotnet run --project src/OpenTail.Stingray.Server
...
**PagedKvCache** (`src/OpenTail.Stingray.Engine/PagedKvCache.cs`):
...
**`ContinuousBatchingEngine`** (`src/OpenTail.Stingray.Engine/ContinuousBatchingEngine.cs`):
...
- [x] Enabled via `STINGRAY_MAX_BATCH` environment variable (server `Program.cs`); `> 1` activates continuous batching
...
- **Chunked / interleaved prefill (Gap 1)** — admitted prompts prefill
  `prefillChunkTokens` (default 256, `STINGRAY_PREFILL_CHUNK` / `PrefillChunkTokens`
...
- **KV token-budget admission backpressure (Gap 3)** — each request reserves
  ...
  `kvBudgetBytes` ctor arg / `KvBudgetMb` option / `STINGRAY_KV_BUDGET_MB`
...
A/B harness: `dotnet run --project benchmarks/OpenTail.Stingray.Bench -c Release -- --cb`
...
- [x] `opentail-llm_tokens_per_second` gauge — lifetime-average tokens/second since server start
- [x] `opentail-llm_queue_depth` gauge — `engine.QueueDepth` (live pending requests)
- [x] `opentail-llm_active_requests` gauge — `engine.ActiveRequests` (live active sequences)
...
**Implementation** (`src/OpenTail.Stingray.Diffusion/RRDBNet.cs`):
...
**IImageOpsBackend interface** (`src/OpenTail.Stingray.Core/IImageOpsBackend.cs`):
...
**CUDA NVRTC Kernels** (`src/OpenTail.Stingray.Cuda/NvrtcInterop.cs`, `CudaKernels.cs`):
...
```bash
dotnet run --project src/OpenTail.Stingray.Cli -- image \
  -m models/z_image_turbo-Q5_K_M.gguf \
  --vae models/z-image-turbo/vae \
  --qwen-encoder models/Z-Image-AbliteratedV1.Q5_K_M.gguf \
  --qwen-tokenizer models/z-image-turbo/tokenizer/tokenizer.json \
...
These are community-reported llama.cpp benchmarks on hardware comparable to our target profile. They establish the performance floor OpenTail.Stingray must reach and the ceiling it aims to approach.
...
Key observation: on the RTX 3060 12GB, 8B-class dense models at Q4_K_M produce approximately 38–52 tokens/second for generation and ~1,500 t/s for prompt processing when fully resident in VRAM. This is the primary benchmark OpenTail.Stingray Phase 2 must approach.
...
Key observation: partial GPU offloading for dense models causes severe performance degradation (40 t/s → 8 t/s when just 11 layers spill to RAM). This is the bottleneck OpenTail.Stingray's pipelined double-buffered layer streaming (Phase 4) aims to improve.
...
### 13.3 OpenTail.Stingray Performance Targets

Based on the reference benchmarks above, these are concrete targets per phase:

| Phase | Model | Configuration | llama.cpp Baseline | OpenTail.Stingray Target | Actual | Notes |
|-------|-------|---------------|-------------------|----------------------|--------|-------|
...
| 7c | Multi-user server | PagedAttention + continuous batching | vLLM ~485 tot t/s @10 users | ≥ 300 tot t/s | ✅ `ContinuousBatchingEngine` + `BatchForwardMulti`; STINGRAY_MAX_BATCH controls batch size |
...
All OpenTail.Stingray benchmarks will use:
...
```
OpenTail.Stingray v0.X.0 — Qwen3 8B Q4_K_M, RTX 3060 12GB, 64GB DDR4
