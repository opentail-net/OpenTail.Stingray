# KVarN KV-Cache Quantization: Feasibility for OpenTail.Stingray

> Research note, 2026-06-09. Evaluates whether to implement **KVarN**
> (Müller et al., Huawei CSL, arXiv:2606.03458, 2 Jun 2026) in OpenTail.Stingray,
...
subsystem.** KVarN's core idea sits one step beyond what `OpenTail.Stingray.TurboQuant`
...
## 3. How it maps onto OpenTail.Stingray

Reusable today (file references):

- **Hadamard rotation** — `src/OpenTail.Stingray.TurboQuant/WalshHadamard.cs`
  (`Transform`, normalized self-inverse, AVX2). Directly usable for step 1.
- **Tile-staged hybrid cache** — `src/OpenTail.Stingray.Engine/TurboQuantKvCache.cs`
...
- **Compressor wrapper shape** — `src/OpenTail.Stingray.TurboQuant/KvCacheCompressor.cs`
...
- The win is real and *on-target*. 2-bit KV at FP16 accuracy → 3–5× context is
  exactly what OpenTail.Stingray's long-context / agentic / reasoning users want, and
...
- **MLA support is largely irrelevant to us today** — it matters for
  DeepSeek/GLM-style latent attention, which OpenTail.Stingray doesn't target. Don't
