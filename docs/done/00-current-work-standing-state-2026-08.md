# Archived "Standing state, not plans" log from docs/00-current-work.md

This is the completed-work/historical-log section that used to live at the bottom of
`docs/00-current-work.md`, moved here per that file's own Archive rule ("Move a document or
section to `done` when its outcome is implemented and verified, or when a measured negative
result closes that line of investigation"). The live file had grown well past its own stated
"keep active documents short" policy — `00-current-work_DONE.md` (a prior, incomplete attempt at
this same split) only covered roughly the first half of this content before falling behind again.
This file supersedes and replaces `00-current-work_DONE.md` with the FULL set, brought back into
compliance with the archive rule (2026-08-31). Every entry below represents closed, completed, or
deliberately-shelved work — nothing here is an open task; see the live `docs/00-current-work.md`
for what's still active.
...
This is the **second** sighting of the same thing; `upstream-port-status.md` records the first
...
in [03-cpu-prefill-plan.md](03-cpu-prefill-plan.md): the int8 batched prefill path's
...
[upstream-port-status.md](upstream-port-status.md).
...
- SafeTensors Phases 4-6 — [08-safetensors-support-plan.md](08-safetensors-support-plan.md).
  Remaining: GPU offload, quantized packages, SentencePiece.
- Session-native runtime CLI surface — [01-session-native-inference-runtime-plan.md](01-session-native-inference-runtime-plan.md).
