# Implementation Plan: `qwen35moe` Architecture for OpenTail.Stingray
...
## GGUF metadata (from `opentail-llm-cli list-metadata`)
...
**Tasks:**
- Add a `list-tensors` CLI command mirroring `ListMetadataCommand` (`src/OpenTail.Stingray.Cli/ListMetadataCommand.cs:12`) that iterates `model.Tensors` and prints `Name | DType | Shape | ByteSize`. Wire it into `Program.cs` next to `list-metadata`.
...
**Files to touch:**
- `src/OpenTail.Stingray.Cli/Program.cs` (register new command)

**Files to create:**
- `src/OpenTail.Stingray.Cli/ListTensorsCommand.cs`
...
5. **Detect SSM presence** at load time the same way `HasAttnBias` is detected (`GgufModel.cs:127`): probe for `blk.1.ssm_in.weight` (knowing layer 1 is an SSM block under `full_attention_interval=4`). Inject synthetic `_opentailllm.is_hybrid_ssm = true` to keep `FromGgufMetadata` pure.
...
**Files to touch:**
- `src/OpenTail.Stingray.Core/ModelGraph.cs` — add `LayerTypes`, `SsmConfig`, `RopeDim`, `IsHybridSsm`; extend `FromGgufMetadata` to parse `qwen35moe.*` keys; extend the NEOX list.
- `src/OpenTail.Stingray.Core/GgufModel.cs:124-129` — extend synthetic-metadata injection to detect SSM tensors.
- `src/OpenTail.Stingray.Core/IComputeBackend.cs:93` — add overload `RoPE(Tensor x, int position, int headDim, int ropeDim, float ropeTheta, bool neox)`. Old signature stays as a forwarding wrapper.
...
1. **New kernel module** `SsmKernels` in `OpenTail.Stingray.Cpu` (separate file `src/OpenTail.Stingray.Cpu/SsmKernels.cs`). Keeps the SSM math co-located and out of `SimdKernels.cs` (already 1978 lines).
...
**Tests (in `tests/OpenTail.Stingray.Tests.Core` or a new `Tests.Ssm`):**
...
**Files to create:**
- `src/OpenTail.Stingray.Cpu/SsmKernels.cs`
- `tests/OpenTail.Stingray.Tests.Core/SsmKernelsTests.cs` (or a new project `Tests.Ssm`)

**Files to touch:**
- `src/OpenTail.Stingray.Cpu/Dequantize.cs` if any new SSM-specific tensor layouts need dequant (unlikely; the standard rowmajor Q4_K_M dequant covers projections).
...
**Files to create:**
- `src/OpenTail.Stingray.Engine/SsmStateCache.cs`
...
**Files to touch:**
- `src/OpenTail.Stingray.Engine/ForwardPass.cs` — major: add SSM tensor arrays, refactor `ForwardCore` to dispatch on layer type, add `SsmBlock` method, extend prefill MoE guard, extend `TruncateTo`/`ResetCache`.
- `src/OpenTail.Stingray.Cpu/SimdKernels.cs` — add `ApplyRoPECachedNeoxPartial` (and the LLaMA variant for symmetry).
- `src/OpenTail.Stingray.Core/IComputeBackend.cs` — already covered in Phase 1.
...
- Add a `--dump-logits` flag to `opentail-llm-cli` (or wire it into existing tracing infra at `ForwardPass.cs:77`, env `STINGRAY_TRACE_NORMS`). Dump first-N logits at each position.
...
4. **MoE in CUDA hybrid.** Currently `CudaHybridForwardPass` *refuses MoE+GPU at construction time* (`CudaHybridForwardPass.cs:103-109`). For Qwen3.6, MoE *is* on both sides, and the user's 12 GB card cannot fit all 256 experts × 30 layers × ~512×2048 Q4_K weights (~5 GB just for one weight matrix × 3 × 30 = ~14 GB). **Must enable MoE on the GPU side of the hybrid** — either:
   ...
   **Recommend SLRU.** It's the only way the 22 GB model fits a 12 GB card. The Vulkan equivalent already exists at `src/OpenTail.Stingray.Engine/ExpertSlotManager.cs` and integrates with `HybridForwardPass`. Port the pattern to CUDA: needs `cudaMemcpyAsync` background uploads on a dedicated stream, plus a `MoEPrefetcher` analogue.
...
**Files to touch:**
- `src/OpenTail.Stingray.Engine/CudaHybridForwardPass.cs` — add SSM dispatch on CPU side; remove the "MoE not supported" guard if porting SLRU; route SSM layers through CPU regardless of `_placement`.
- `src/OpenTail.Stingray.Engine/ExpertSlotManager.cs` — either generalize over `IComputeBackend` (introduces lots of churn) or duplicate as `CudaExpertSlotManager`. **Recommend duplicate** to avoid destabilizing the working Vulkan SLRU during this work.
- `src/OpenTail.Stingray.Engine/TierPlanner.cs` — needs to account for SSM weight footprint differently (SSM `ssm_in` + `ssm_out` are dense linear projections sized `embDim → ssm_inner` = 2048×4096 = ~8 MiB Q4_K each; 30 layers × 4 SSM tensors × 8 MiB ≈ 1 GiB).

**Files to create:**
- `src/OpenTail.Stingray.Engine/CudaExpertSlotManager.cs` (if porting SLRU)
...
**Files to touch:**
- `src/OpenTail.Stingray.Cuda/CudaTextKernels.cs` — append new kernels.
- `src/OpenTail.Stingray.Cuda/CudaBackend.cs` — bind kernels, add methods `SsmConvDecode`, `SsmScanDecode`, `SsmConvPrefill`, `SsmScanPrefill`.
- `src/OpenTail.Stingray.Engine/CudaHybridForwardPass.cs` — switch GPU-tier SSM blocks to use these kernels; create `_gpuSsmConvState[]` and `_gpuSsmScanState[]` mirrors of `SsmStateCache`.
...
**Files (if pursued):**
- `src/OpenTail.Stingray.Vulkan/Shaders.cs` — append `SsmConv1dDecode`, `SsmScanDecode` GLSL.
- `src/OpenTail.Stingray.Vulkan/VulkanBackend.cs` — bind shaders.
- `src/OpenTail.Stingray.Engine/HybridForwardPass.cs` — SSM dispatch.
...
- `src/OpenTail.Stingray.Core/JinjaChatTemplate.cs`
...
- Documentation: update `docs/OpenTail.Stingray-Design.md` with a "Hybrid SSM Architecture" section. Note the limitations (no speculative decoding rewind, no continuous batching, no Vulkan).
...
6. **TruncateTo restrictions cascade.** Disabling speculative decoding rewind disables one of OpenTail.Stingray's distinctive features for this model. Confirm acceptable upfront with the user. Implementing checkpoint/restore for SSM state is doable (snapshot the 62 MiB per-sequence state on each pre-draft point, restore on rejection) but takes ~3 days.
...
- `src/OpenTail.Stingray.Core/ModelGraph.cs`
- `src/OpenTail.Stingray.Engine/ForwardPass.cs`
- `src/OpenTail.Stingray.Engine/CudaHybridForwardPass.cs`
- `src/OpenTail.Stingray.Cpu/SsmKernels.cs` (new)
- `src/OpenTail.Stingray.Engine/SsmStateCache.cs` (new)
...
**Run on 2026-05-25 against `C:\p\opentail-llm\models\Qwen3.6-27B-MTP-Q4_K_M.gguf` (17.11 GB, 866 tensors, 54 metadata keys).**
...
| `_opentailllm.is_hybrid_ssm` | true | **synthetic probe trips correctly** — no arch hardcode needed for hybrid detection |
...
- Blocker #3 (`qwen35` arch unknown to `ModelGraph`) — **CONFIRMED**: needs to be added to the NEOX-rope arch list. Hybrid detection via synthetic `_opentailllm.is_hybrid_ssm` probe **already works** (verified by metadata dump showing `_opentailllm.is_hybrid_ssm = true`).
...
- **11.9 `STINGRAY_DISABLE_MTP=1` env switch** — `InferenceEngine.GenerateChunksAsync` routes through `MtpDecoder` when `fwd.HasMtpHead && STINGRAY_DISABLE_MTP != "1" && sp.Temperature <= 0f && !thinkingEnabled`. Acceptance rate tracing under `STINGRAY_TRACE_MTP=1`.
...
| CUDA hybrid | qwen36-27b-mtp Q4_K_M | **21 FFN layers on GPU (exact-size)** | **6.3** | **+58 % over baseline.** STINGRAY_DENSE_FFN_GPU_MARGIN_MB=32, ctx ≤ 2048 |
...
**`GpuBufferPool.RoundUp` rounded every device allocation to the next power of two** (`CudaBackend.cs:2120-2127`). Per the diagnostic checkpoints (run with `STINGRAY_TRACE_VRAM=1`):
...
2. **`ModelGraph` doesn't know `qwen3_5`** (`ModelGraph.cs:161-181`): not in the NEOX-rope architecture list; hybrid detection only triggers via the synthetic `_opentailllm.is_hybrid_ssm` tensor probe (`GgufModel.cs:132-136`). Add `qwen3_5` to the NEOX list. The tensor probe at layers 0-3 will trip on `blk.0.ssm_conv1d.weight` (layers 0-2 are GDN under `full_attention_interval=4`), so hybrid auto-detection should work without an arch hardcode.
...
| 11.9 | `STINGRAY_DISABLE_MTP=1` env switch | wiring at `InferenceEngine`/`RunCommand` | Mirrors `STINGRAY_BYPASS_GDN`/`STINGRAY_CPU_GDN` |
