...
|---|---|---|
| Skills / tool validation | `AttachSkill`/`DetachSkill`/`AttachedSkills`/`ToolProvider`/`ToolContext`/`ValidateToolCall` | Fully functional, same two-source (skill tools + `ToolProvider`) check `InferenceSession` did. |
| `ToolCallParser` | Settable delegate property | Stored only — `HotSession` does not invoke it (chunk-based detection already happens at the Server layer independently; see the TODO doc's item #5). |
...
| LoRA | `HotSession.ActiveLora` (`LoraAdapter?`) | **Inert, by design** — matches what it did on `InferenceSession` (nothing). Real wiring needs new engine work; see the TODO doc's item #6. |

## Wired into the live Server path

### 1. Skills / tool validation
...
- No propagation through `Fork()`: attaching a skill to a parent session does **not** carry it to
  ...
  real design question, not just a wiring detail, and nothing needed it yet. Tracked as a TODO.
- `ToolProvider`/`ToolCallParser` remain unwired, as this doc already recommended (see the TODO
  doc's item #5) — the validate endpoint only ever calls `ValidateToolCall` against whatever the
  caller supplies.
...
- **Sessions API (`/v1/sessions/*`) — real gap, now closed, deliberately narrower.**
  ...
  afterward; the confirmed decision was that late-attach affects only future turns rather than
  being rejected). Test: `HotSessionCapabilityPortTests.HotSession_AttachSkillInstructions_PrependToNextTurnOnly`.

Still not done, deliberately: `Fork()` propagation (unchanged from the original scoping note above)
and `ToolProvider`/`ToolCallParser` wiring — both are TODOs.
...
This plan did not block `docs/030`'s deletion — the capabilities themselves were ported and
regression-tested regardless of whether anything in `Server` called them yet, and doc 030's
deletion pass has since been executed.
