...
> cache (`STINGRAY_PREFIX_SLOTS=2`) is rejected at attach — a scratch-slot request would
> clobber tap rows the owned slot still needs. `InferenceEngineLoader` wires
> `STINGRAY_DSPARK_MODEL` / `STINGRAY_DSPARK_PLACE` (options `DSparkModelPath`/`DSparkPlace`)
...
> (§7's server half — the threshold trim resolves from `STINGRAY_DSPARK_*` per request).
>
...
> `STINGRAY_DSPARK_TIMING=1` breakdown overturned the launch-bound theory: the old round
...
> existing `STINGRAY_BATCH_DECODE_MMQ=1` A/B (which force-routes the pinned
...
- `SpeculativeDecoder` (`src/OpenTail.Stingray.Engine/SpeculativeDecoder.cs`) already accepts two
...
- `HardwareProfile.Detect(...)` (`src/OpenTail.Stingray.Engine/HardwareProfile.cs`) already
...
- `TierPlanner.Plan(...)` (`src/OpenTail.Stingray.Engine/TierPlanner.cs`) is the existing
...
New type, `DSparkPlacementPlanner` in `OpenTail.Stingray.Engine`, mirroring `TierPlanner`'s shape.
...
Mirroring the existing `--spec-draft-n-max`/`--spec-draft-p-min`/`STINGRAY_MTP_*` conventions
...
| `--dspark-place auto\|gpu\|cpu\|off` | `STINGRAY_DSPARK_PLACE` | Overrides `DSparkPlacementPlanner.Plan`'s decision outright — skips the VRAM/RAM math entirely, same as `pinGpuLayers` skips `TierPlanner`'s greedy packer. | `-g N` pinning `TierPlanner`'s `pinGpuLayers` |
| `--dspark-verify-len <n>` | `STINGRAY_DSPARK_VERIFY_LEN` | Caps the confidence scheduler's per-step verify length (§7). `0`/unset = scheduler decides. | `--spec-draft-n-max` |
| `--dspark-min-confidence <p>` | `STINGRAY_DSPARK_MIN_CONFIDENCE` | Floor on the confidence head's acceptance-probability estimate below which the scheduler trims a position off the verify batch. | `--spec-draft-p-min` |
...
- **Safetensors loader.** The main inference path (Core/Engine/backends) is GGUF-only, but
  `OpenTail.Stingray.Diffusion` already ships a `SafetensorsLoader` (single-file and multi-shard,
  ...
  (extending `OpenTailStingrayJsonContext` or a sibling context) rather than reflection-based
...
CLI path is validated, analogous to how `STINGRAY_KV_BUDGET_MB` already gates admission by a
