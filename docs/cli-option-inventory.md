...
`[Description]` pairs under `src/OpenTail.Stingray.Cli`. Last regenerated **2026-08-15**, recording
...
a blank cell — three rows (`RunCommand`'s `--device`, `--thinking`, `--tq-mode`) had real, detailed
...
with all three descriptions now populated — a pure information gain, no row/count change.
...
(see below) — this was the outstanding half of Phase 0 deliverable 1 of
...
Class meanings (deliberately prose, not a table — a `` | `class-name` | `` row here would be
...
- **stable** — supported user configuration, the ordinary/documented way to use this flag or
  ...
  invocation," not "belongs in a generation profile" — those commands aren't part of the profile
  surface at all.
- **expert** — supported but sharp-edged; documented, not surfaced by default; a real operational
  lever for a sophisticated operator.
- **diagnostic** — troubleshooting only. Never written to a profile.
- **experimental** — may change or vanish without notice. Must never become a default. Used here
...
`--tensor-split`, `--ubatch-size`): classified `stable`, not `experimental` — the CONTRACT here
