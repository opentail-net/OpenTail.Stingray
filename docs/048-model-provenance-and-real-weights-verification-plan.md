...
| **TTS** | CosyVoice 300M / 2.0 / 3 | Safetensors / ONNX / GGUF | ✅ `cosyvoice2_0.5b.safetensors`, `cosyvoice_speech_tokenizer.onnx`, `cosyvoice3/CosyVoice3-2512_F16.gguf` | ✅ Passed | ⚠️ **PARTIAL (real intelligible non-buzzing speech generated; zero-shot voice cloning does not yet reliably reproduce the reference speaker's identity -- see `docs/audio-review-progress.md`'s "CosyVoice3: status set to PARTIALLY SUPPORTED" entry)** | **Partially Supported** |
...
| **TTS** | MMS-TTS (Meta, VITS architecture) | Safetensors | ✅ `mms-tts-eng/{config.json,vocab.json,model.safetensors}` | ✅ Passed | ✅ **PROVEN (every stage cosine >0.99 vs real HuggingFace `transformers.VitsModel` reference -- text encoder, duration predictor, flow, HiFi-GAN decoder, independently verified with golden-captured noise; real CLI synthesis at RTF 0.68x; see docs/audio-review-progress.md's "MMS-TTS: DONE" entry. English (`mms-tts-eng`) only tested -- other ~1100 language checkpoints untested. License `cc-by-nc-4.0`, non-commercial.)** | **Level 2 (Fully Proven)** |
