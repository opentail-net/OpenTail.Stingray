...
///   - <c>STINGRAY_CUDA13=1</c> prefers the CUDA 13 pair when both DLLs load (A/B knob —
...
internal static class CudaLibraryResolver
{
    ...
    private static int DecideRuntimeMajor()
    {
        ...
        bool prefer13 = Environment.GetEnvironmentVariable("STINGRAY_CUDA13") == "1";
        if (prefer13)
        {
        ...
        if (major == 13)
        {
            Console.Error.WriteLine("[OpenTail.Stingray] CUDA runtime: using cudart64_13/cublas64_13"
                + (prefer13 ? " (STINGRAY_CUDA13=1)" : " (CUDA 12 runtime not found)"));
        }
