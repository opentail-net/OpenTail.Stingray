...
/// <see cref="CudaTextKernels"/> / <see cref="CudaWsKernels"/> (whose <c>opentail-llm_*</c>
...
/// plain struct kernel parameters (<c>opentail-llm_seq_pos</c> / <c>opentail-llm_seq_ptrs</c>, capacity
...
typedef struct { const void* p[16]; } opentail-llm_seq_ptrs;
typedef struct { int v[16]; } opentail-llm_seq_pos;
...
extern ""C"" __global__ void llm_rope_neox_ragged(
    ...
    int num_heads, int head_dim, opentail-llm_seq_pos positions, float theta, int n_tok)
{
...
extern ""C"" __global__ void llm_kv_append_ragged(
    ...
    opentail-llm_seq_ptrs k_caches, opentail-llm_seq_ptrs v_caches,
    opentail-llm_seq_pos positions,
    ...
    int kv_dim, int max_seq_len, int n_tok)
{
    ...
    long offset = (long)(positions.v[token] % max_seq_len) * (long)kv_dim + (long)i;
    long in_off = (long)token * (long)kv_dim + (long)i;
    k_cache[offset] = (unsigned short)opentail-llm_fp32_to_bf16(k_in_all[in_off]);
    v_cache[offset] = (unsigned short)opentail-llm_fp32_to_bf16(v_in_all[in_off]);
}

// q8_0-store variant: identical warp-collaborative block quantization to
// llm_kv_append_q8_0 (opentail-llm_q8_append_one — amax shuffle reduce, fp16 scale,
    ...
    int kv_dim, int max_seq_len, int n_tok)
{
    ...
    long block = elem >> 5;
    long in_off = (long)token * (long)kv_dim + (long)i;
    opentail-llm_q8_append_one(valid ? k_in_all[in_off] : 0.f, valid, k_cache, block, lane);
    opentail-llm_q8_append_one(valid ? v_in_all[in_off] : 0.f, valid, v_cache, block, lane);
}
...
__device__ void llm_attention_ragged_impl(
    ...
    opentail-llm_seq_pos positions, int max_seq_len, float attn_scale, int n_tok)
{
    ...
    for (int t = (int)tid; t < seq_len; t += 256) {
        ...
        for (int d = 0; d < head_dim; d++)
            dot += q_all[q_off + d] * opentail-llm_kvload(k_cache, k_off + d);
