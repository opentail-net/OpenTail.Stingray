...
/// NVRTC compilation after <see cref="CudaTextKernels"/> (whose <c>opentail-llm_*</c>
...
/// decode MMQ (<c>STINGRAY_BATCH_DECODE_MMQ=1</c>), not to load-shape tweaks.</para>
/// </summary>
    ...
    int rows, int cols, int n_tok)
{
    ...
    for (int t = 0; t < NT; t++)
        if (t < n_tok) {
            float result = opentail-llm_warp_reduce_sum(acc[t]);
    ...
    int rows, int cols, int n_tok)
{
    ...
    for (int block = 0; block < num_blocks; block++) {
        float d = opentail-llm_fp16_to_fp32(scales[srow + block]);
    ...
    int rows, int cols, int n_tok)
{
    ...
    for (int t = 0; t < NT; t++)
        if (t < n_tok)
            acc[t] = opentail-llm_warp_reduce_sum(acc[t]);
...
// loads (8 × opentail-llm_int8_at + 2 d bytes). Same bytes → same int8 / fp16 values
    ...
    int rows, int cols, int n_tok)
{
    ...
    for (int block = 0; block < num_blocks; block++) {
        ...
        // same word opentail-llm_byte_at(b0+209) already reads in the #194 kernel.
        ...
        // Same byte, same sign-widening as opentail-llm_int8_at(weights, b0+192+2k+isc).
...
";
    ...
    /// <summary>
    /// #201 decode MMQ (<c>STINGRAY_BATCH_DECODE_MMQ=1</c>): the bit-exact WS kernels
    ...
    int rows, int cols, int n_tok)
{
    ...
    for (int ksb = 0; ksb < nb_super; ksb++) {
        ...
        for (int sb = 0; sb < 8; sb++) {
            ...
            float dwA = opentail-llm_fp16_to_fp32(ddA & 0xffffu) * (float)sWsB[(mrow0 + grp) * 16 + sb];
            float dmA = opentail-llm_fp16_to_fp32(ddA >> 16)     * (float)sWsB[(mrow0 + grp) * 16 + 8 + sb];
            float dwB = opentail-llm_fp16_to_fp32(ddB & 0xffffu) * (float)sWsB[(mrow0 + grp + 8) * 16 + sb];
            float dmB = opentail-llm_fp16_to_fp32(ddB >> 16)     * (float)sWsB[(mrow0 + grp + 8) * 16 + 8 + sb];
            ...
            float dC0 = opentail-llm_fp16_to_fp32(dy0 & 0xffffu), sC0 = opentail-llm_fp16_to_fp32(dy0 >> 16);
            float dC1 = opentail-llm_fp16_to_fp32(dy1 & 0xffffu), sC1 = opentail-llm_fp16_to_fp32(dy1 >> 16);
    ...
    int rows, int cols, int n_tok)
{
    ...
    for (int ksb = 0; ksb < nb_super; ksb++) {
        ...
        for (int sb = 0; sb < 8; sb++) {
            ...
            float dA = opentail-llm_fp16_to_fp32(sWd[mrow0 + grp]     & 0xffffu);
            float dB = opentail-llm_fp16_to_fp32(sWd[mrow0 + grp + 8] & 0xffffu);
            ...
            unsigned int dy0 = sYd[(ncol0 + tig * 2) * 8 + sb];
            unsigned int dy1 = sYd[(ncol0 + tig * 2 + 1) * 8 + sb];
            float dC0 = opentail-llm_fp16_to_fp32(dy0 & 0xffffu);
            float dC1 = opentail-llm_fp16_to_fp32(dy1 & 0xffffu);
