...
/// Mirrors the GLSL compute shaders in <c>OpenTail.Stingray.Vulkan.Shaders</c>:
...
__device__ __forceinline__ float opentail-llm_neg_inf() { return __int_as_float(0xff800000); }
...
__device__ __forceinline__ float opentail-llm_fp16_to_fp32(unsigned int h)
{
    float result;
...
__device__ __forceinline__ unsigned int opentail-llm_f32x2_to_f16x2(float a, float b)
{
    unsigned int r;
...
__device__ __forceinline__ unsigned int opentail-llm_hfma2(unsigned int a, unsigned int b, unsigned int acc)
{
    unsigned int r;
...
__device__ __forceinline__ float opentail-llm_f16x2_sum(unsigned int p)
{
    float lo, hi;
...
__device__ __forceinline__ float opentail-llm_warp_reduce_sum(float v)
{
...
__device__ __forceinline__ unsigned int opentail-llm_fp32_to_fp16(float f)
{
    unsigned short h;
...
__device__ __forceinline__ float opentail-llm_bf16_to_fp32(unsigned int bits)
{
    unsigned int f = (bits & 0xFFFFu) << 16;
    return __int_as_float(f);
}

__device__ __forceinline__ unsigned int opentail-llm_fp32_to_bf16(float f)
{
...
__device__ __forceinline__ float opentail-llm_kvload(const float* __restrict__ p, long i) { return p[i]; }
__device__ __forceinline__ float opentail-llm_kvload(const unsigned short* __restrict__ p, long i) { return opentail-llm_bf16_to_fp32((unsigned int)p[i]); }
...
// opentail-llm_kvload. The load is purely per-element (block `i>>5`, lane `i&31`); the
...
__device__ __forceinline__ float opentail-llm_kvload(const block_q8_0* __restrict__ p, long i)
{
    long b = i >> 5;
    int  j = (int)(i & 31);
    return opentail-llm_fp16_to_fp32((unsigned int)p[b].d) * (float)p[b].qs[j];
}
...
// the inlined opentail-llm_kvload produced (no change for those dtypes). The q8_0 overload caches the
...
__device__ __forceinline__ float opentail-llm_kv_dot(const float* __restrict__ q, const float* __restrict__ k, long off, int n)
{
    float dot = 0.f;
    for (int d = 0; d < n; d++) dot += q[d] * k[off + d];
    return dot;
}
__device__ __forceinline__ float opentail-llm_kv_dot(const float* __restrict__ q, const unsigned short* __restrict__ k, long off, int n)
{
    float dot = 0.f;
    for (int d = 0; d < n; d++) dot += q[d] * opentail-llm_bf16_to_fp32((unsigned int)k[off + d]);
    return dot;
}
__device__ __forceinline__ float opentail-llm_kv_dot(const float* __restrict__ q, const block_q8_0* __restrict__ k, long off, int n)
{
    float dot = 0.f;
    ...
    for (int d = 0; d < n; )
    {
        float s = opentail-llm_fp16_to_fp32((unsigned int)k[b].d);   // convert the fp16 scale ONCE per block
...
__device__ __forceinline__ unsigned int opentail-llm_byte_at(const unsigned int* __restrict__ buf, long B)
{
    unsigned int w = buf[(B) >> 2];
    return (w >> (((unsigned int)(B) & 3u) * 8u)) & 0xFFu;
}

// Same as opentail-llm_byte_at but interprets the byte as int8 (signed).
__device__ __forceinline__ int opentail-llm_int8_at(const unsigned int* __restrict__ buf, long B)
{
    int v = (int)opentail-llm_byte_at(buf, B);
...
__device__ __forceinline__ unsigned int opentail-llm_uint_at(const unsigned int* __restrict__ buf, long B)
{
    long aB = B & ~3L;
...
#define STINGRAY_ARGMAX_NEG_INF (-3.402823466e38f)
    ...
    const float* __restrict__ logits, int n, float* __restrict__ partialVal, int* __restrict__ partialIdx)
{
    ...
    float best = STINGRAY_ARGMAX_NEG_INF;
    int bestIdx = 0;
    ...
    int rank)
{
    ...
    float v = opentail-llm_fp16_to_fp32((unsigned int)w1_f16[(long long)prev * rank + i]);
    w1row[i] = v;
...
extern ""C"" __global__ void llm_softmax(float* __restrict__ x, int n)
{
    ...
    float local_max = opentail-llm_neg_inf();
    for (int i = (int)tid; i < n; i += 256)
    ...
    int kv_dim, int position, int max_seq_len)
{
    ...
    long offset = (long)(position % max_seq_len) * (long)kv_dim + (long)i;
    k_cache[offset] = (unsigned short)opentail-llm_fp32_to_bf16(k_in[i]);
    v_cache[offset] = (unsigned short)opentail-llm_fp32_to_bf16(v_in[i]);
}
...
// f32/bf16 appends. Read-side recovery is opentail-llm_kvload(const block_q8_0*, ...).
__device__ __forceinline__ void opentail-llm_q8_append_one(
    float val, bool valid, block_q8_0* __restrict__ cache, long block_idx, int lane)
{
    ...
    if (lane == 0) dst->d = (unsigned short)opentail-llm_fp32_to_fp16(d);
}
    ...
    int kv_dim, int position, int max_seq_len)
{
    ...
    long block = elem >> 5;
    opentail-llm_q8_append_one(valid ? k_in[i] : 0.f, valid, k_cache, block, lane);
    opentail-llm_q8_append_one(valid ? v_in[i] : 0.f, valid, v_cache, block, lane);
}
    ...
    int token_id, int emb_dim)
{
    ...
    for (int block = 0; block < num_blocks; block++) {
        ...
        float d    = opentail-llm_fp16_to_fp32(w0 & 0xffffu);
        float dmin = opentail-llm_fp16_to_fp32(w0 >> 16);
    ...
    int token_id, int emb_dim)
{
    ...
    for (int block = 0; block < num_blocks; block++) {
        ...
        float d    = opentail-llm_fp16_to_fp32(dword0 & 0xffffu);
        float dmin = opentail-llm_fp16_to_fp32(dword0 >> 16);
    ...
    int token_id, int emb_dim)
{
    ...
    for (int block = 0; block < num_blocks; block++) {
        ...
        float d = opentail-llm_fp16_to_fp32((unsigned int)blk[208] | ((unsigned int)blk[209] << 8));
    ...
    int token_id, int emb_dim)
{
    ...
    for (int outer = 0; outer < outer_iters; outer++) {
        ...
        float d = opentail-llm_fp16_to_fp32(d_bits);
    ...
    int rows, int cols)
{
    ...
    float result = opentail-llm_warp_reduce_sum(acc);
    ...
    int n)
{
    ...
    if (lane == 0) {
        ...
        unsigned int d_bits = opentail-llm_fp32_to_fp16(d);
        unsigned int s_bits = opentail-llm_fp32_to_fp16(d * (float)qsum);
    ...
    int rows, int cols)
{
    ...
    for (int block = warp_id; block < num_blocks; block += MATVEC_Q4K_NWARPS) {
        ...
        float super_d    = opentail-llm_fp16_to_fp32(w0 & 0xffffu);
        float super_dmin = opentail-llm_fp16_to_fp32(w0 >> 16);
        ...
        float d8_lo = opentail-llm_fp16_to_fp32(d_bits_lo);
        float d8_hi = opentail-llm_fp16_to_fp32(d_bits_hi);
    ...
    acc = opentail-llm_warp_reduce_sum(acc);
    ...
    int rows, int cols)
{
    ...
    for (int block = warp_id; block < num_blocks; block += MATVEC_Q4K_SOA_NWARPS) {
        ...
        float super_d    = opentail-llm_fp16_to_fp32(dd & 0xffffu);
        float super_dmin = opentail-llm_fp16_to_fp32(dd >> 16);
    ...
    int rows, int cols)
{
    ...
    for (int block = 0; block < num_blocks; block++) {
        ...
        float d = opentail-llm_fp16_to_fp32(dlo | (dhi << 8));

        // Eight signed-int8 scales for this lane (isc = lane >> 4 selects 0 or 1).
        long isc = (long)(lane >> 4);

        float sc0 = d * (float)opentail-llm_int8_at(weights, b0 + 192 + isc);
        float sc1 = d * (float)opentail-llm_int8_at(weights, b0 + 194 + isc);
        float sc2 = d * (float)opentail-llm_int8_at(weights, b0 + 196 + isc);
        float sc3 = d * (float)opentail-llm_int8_at(weights, b0 + 198 + isc);
        float sc4 = d * (float)opentail-llm_int8_at(weights, b0 + 200 + isc);
        float sc5 = d * (float)opentail-llm_int8_at(weights, b0 + 202 + isc);
        float sc6 = d * (float)opentail-llm_int8_at(weights, b0 + 204 + isc);
        float sc7 = d * (float)opentail-llm_int8_at(weights, b0 + 206 + isc);

        // Six quant bytes per lane.
        unsigned int ql0 = opentail-llm_byte_at(weights, b0 +   0 + lane);
        unsigned int ql1 = opentail-llm_byte_at(weights, b0 +  32 + lane);
        unsigned int ql2 = opentail-llm_byte_at(weights, b0 +  64 + lane);
        unsigned int ql3 = opentail-llm_byte_at(weights, b0 +  96 + lane);
        unsigned int qh0 = opentail-llm_byte_at(weights, b0 + 128 + lane);
        unsigned int qh1 = opentail-llm_byte_at(weights, b0 + 160 + lane);
    ...
    int rows, int cols)
{
    ...
    for (int block = 0; block < num_blocks; block++) {
        ...
        float d = opentail-llm_fp16_to_fp32(dbits);
    ...
    int rows, int cols)
{
    ...
    for (int block = 0; block < num_blocks; block++) {
        ...
        unsigned int qh_byte = opentail-llm_byte_at(weights, b0 + 16 + lane);
        ...
        for (int chunk = 0; chunk < 4; chunk++) {
            ...
            if (j_lo < 4) {
                ...
                sc_lo_byte = opentail-llm_byte_at(weights, b0 + 4 + j_lo);
                sc_hi_byte = opentail-llm_byte_at(weights, b0 + 4 + j_lo + 4);
                sc1 = sc_lo_byte & 63u;
                m1  = sc_hi_byte & 63u;
                // Pair j_hi = j_lo + 1 is also < 4.
                unsigned int sc_lo2 = opentail-llm_byte_at(weights, b0 + 4 + j_hi);
                unsigned int sc_hi2 = opentail-llm_byte_at(weights, b0 + 4 + j_hi + 4);
                sc2 = sc_lo2 & 63u;
                m2  = sc_hi2 & 63u;
            } else {
                // Pairs 4..7: scale = (q[j+4] & 0xF) | ((q[j-4] >> 6) << 4)
                //             min   = (q[j+4] >> 4)  | ((q[j]   >> 6) << 4)
                unsigned int a_lo = opentail-llm_byte_at(weights, b0 + 4 + j_lo + 4); // q[j+4] for j=j_lo
                unsigned int b_lo = opentail-llm_byte_at(weights, b0 + 4 + j_lo - 4); // q[j-4]
                unsigned int c_lo = opentail-llm_byte_at(weights, b0 + 4 + j_lo);     // q[j]
                sc1 = (a_lo & 0xFu) | (((b_lo >> 6) & 3u) << 4);
                m1  = ((a_lo >> 4) & 0xFu) | (((c_lo >> 6) & 3u) << 4);

                unsigned int a_hi = opentail-llm_byte_at(weights, b0 + 4 + j_hi + 4);
                unsigned int b_hi = opentail-llm_byte_at(weights, b0 + 4 + j_hi - 4);
                unsigned int c_hi = opentail-llm_byte_at(weights, b0 + 4 + j_hi);
            ...
            unsigned int ql_byte = opentail-llm_byte_at(weights, b0 + 48 + chunk * 32 + lane);
    ...
    int rows, int cols)
{
    ...
    for (int block = 0; block < num_blocks; block++) {
        ...
        unsigned int dlo = opentail-llm_byte_at(weights, b0 + 0);
        unsigned int dhi = opentail-llm_byte_at(weights, b0 + 1);
        float d = opentail-llm_fp16_to_fp32(dlo | (dhi << 8));

        // One int8 quant per lane; element offset within the block = lane.
        int q = opentail-llm_int8_at(weights, b0 + 2 + (long)lane);
    ...
    int rows, int cols)
{
    ...
    for (int block = 0; block < num_blocks; block++) {
        ...
        unsigned int qbyte = opentail-llm_byte_at(weights, b0 + 2 + (long)(lane & 15));
    ...
    int rows, int cols)
{
    ...
    float acc = 0.f;
    for (int block = 0; block < num_blocks; block++) {
        float d = opentail-llm_fp16_to_fp32(ws[srow + block]);
        int q = opentail-llm_int8_at(weights, qrow + (long)block * 32 + lane);
        float x = input[block * 32 + lane];
        acc += d * (float)q * x;
    }
    float result = opentail-llm_warp_reduce_sum(acc);
    ...
    int rows, int cols)
{
    ...
    for (int block0 = warp_id * 4; block0 < num_blocks; block0 += MATVEC_Q80_NWARPS * 4) {
        ...
        if (block < num_blocks) {
            long b0 = row_base_bytes + (long)block * 34L;
            unsigned int dlo = opentail-llm_byte_at(weights, b0);
            unsigned int dhi = opentail-llm_byte_at(weights, b0 + 1);
            float dw = opentail-llm_fp16_to_fp32(dlo | (dhi << 8));
            ...
            float da = opentail-llm_fp16_to_fp32(d_bits);
    ...
    int rows, int cols)
{
    ...
    for (int block0 = warp_id * 4; block0 < num_blocks; block0 += MATVEC_Q80_NWARPS * 4) {
        ...
        float part = 0.f;
        if (block < num_blocks) {
            float dw = opentail-llm_fp16_to_fp32(ws[srow + block]);
...
__device__ __forceinline__ int opentail-llm_get_int_b2(const unsigned char* __restrict__ p, int i32)
{
    ...
    int rows, int cols)
{
    ...
    for (int kbx = tid / (qi / vdr); kbx < nb; kbx += blocks_per_iter)
    {
        ...
        const float dw = opentail-llm_fp16_to_fp32(d_bits);     // weight block scale
        ...
        const float da = opentail-llm_fp16_to_fp32(da_bits);    // activation block scale
        const int* aq = reinterpret_cast<const int*>(y_q81 + ab + 4);

        int sumi = 0;
        #pragma unroll
        for (int i = 0; i < vdr; i++)
            sumi = __dp4a(opentail-llm_get_int_b2(qs, kqs + i), aq[kqs + i], sumi);
    ...
    for (int block0 = warp_id * 8; block0 < num_blocks; block0 += MATVEC_Q40_NWARPS * 8) {
        ...
        if (block < num_blocks) {
            ...
            if (sub == 0) {
                ...
                float s = opentail-llm_fp16_to_fp32(s_bits);
                part -= 8.f * dw * s;
            }
        }
    ...
    int rows, int cols, int n_tok)
{
    ...
    #define MMQ_LOAD_TILE(KB) do { \
        ...
        rW0 = (gw0 < rows) ? opentail-llm_uint_at(weights, ((long)gw0 * nb + (KB)) * 34L + 2 + (long)(tid & 7) * 4) : 0u; \
        int gw1 = row_block + ((tid + 256) >> 3); \
        rW1 = (gw1 < rows) ? opentail-llm_uint_at(weights, ((long)gw1 * nb + (KB)) * 34L + 2 + (long)((tid + 256) & 7) * 4) : 0u; \
        if (tid < MMQ_BM) { long wb = ((long)(row_block + tid) * nb + (KB)) * 34L; \
            rWd = (row_block + tid < rows) ? opentail-llm_fp16_to_fp32(opentail-llm_byte_at(weights, wb) | (opentail-llm_byte_at(weights, wb + 1) << 8)) : 0.f; } \
        ...
        if (tid < MMQ_BN) { int gt = tok_block + tid; \
            rYd = (gt < n_tok) ? opentail-llm_fp16_to_fp32((*reinterpret_cast<const unsigned int*>(y_q81 + ((long)gt * nb + (KB)) * 36L)) & 0xffffu) : 0.f; } \
    } while (0)
...
// + __funnelshift (opentail-llm_uint_at); here every weight word is a plain aligned load.
    ...
    int rows, int cols, int n_tok)
{
    ...
    #define MMQ_LOAD_TILE_SOA(KB) do { \
        ...
        if (tid < MMQ_BM) { int gw = row_block + tid; \
            rWd = (gw < rows) ? opentail-llm_fp16_to_fp32(ws[(long)gw * nb + (KB)]) : 0.f; } \
        int gy0 = tok_block + (tid >> 3); \
        rY0 = (gy0 < n_tok) ? *reinterpret_cast<const unsigned int*>(y_q81 + ((long)gy0 * nb + (KB)) * 36L + 4 + (long)(tid & 7) * 4) : 0u; \
        int gy1 = tok_block + ((tid + 256) >> 3); \
        rY1 = (gy1 < n_tok) ? *reinterpret_cast<const unsigned int*>(y_q81 + ((long)gy1 * nb + (KB)) * 36L + 4 + (long)((tid + 256) & 7) * 4) : 0u; \
    ...
    int rows, int cols, int n_tok)
{
    ...
    for (int kb = 0; kb < nb; kb++) {
        ...
        if (tid < MMQ_BM) { int gw = row_block + tid;
            sWd[tid] = (gw < rows) ? opentail-llm_fp16_to_fp32(ws[(long)gw * nb + kb]) : 0.f; }
        int gy0 = tok_block + (tid >> 3);
        ...
        if (tid < MMQ_BN) { int gt = tok_block + tid;
            sYd[tid] = (gt < n_tok) ? opentail-llm_fp16_to_fp32(y_ds[(long)gt * nb + kb] & 0xffffu) : 0.f; }
        __syncthreads();
...
__device__ __forceinline__ void opentail-llm_cp_async16(int* smem, const void* gmem)
{
...
__device__ __forceinline__ void opentail-llm_cp_commit()
{
#if __CUDA_ARCH__ >= 800
    asm volatile(""cp.async.commit_group;"");
#endif
}
__device__ __forceinline__ void opentail-llm_cp_wait_keep1()
{
#if __CUDA_ARCH__ >= 800
    asm volatile(""cp.async.wait_group 1;"");
#endif
}
__device__ __forceinline__ void opentail-llm_cp_wait_all()
{
#if __CUDA_ARCH__ >= 800
    ...
    int rows, int cols, int n_tok)
{
    ...
    #define CPA_ISSUE(KB, ST) do { \
        if (tid < MMQ_BM * 2) { int row = tid >> 1, half = tid & 1; int gw = row_block + row; \
            ...
            if (gw < rows) opentail-llm_cp_async16(dstp, &qw[((long)gw * nb + (KB)) * 8L + half * 4]); \
            else { dstp[0] = dstp[1] = dstp[2] = dstp[3] = 0; } } \
        { int token = tid >> 1, half = tid & 1; int gy = tok_block + token; \
            int* dstp = &sY[ST][token * 8 + half * 4]; \
            if (gy < n_tok) opentail-llm_cp_async16(dstp, &y_qs[((long)gy * nb + (KB)) * 8L + half * 4]); \
            else { dstp[0] = dstp[1] = dstp[2] = dstp[3] = 0; } } \
    } while (0)

    CPA_ISSUE(0, 0);
    opentail-llm_cp_commit();
    ...
    for (int kb = 0; kb < nb; kb++) {
        ...
        if (kb + 1 < nb) {
            ...
            opentail-llm_cp_commit();
            opentail-llm_cp_wait_keep1();   // keep the just-issued next tile in flight
        } else {
            opentail-llm_cp_wait_all();
        }
    ...
    int rows, int cols, int n_tok)
{
    ...
    #define CPA_ISSUE_Q40(KB, ST) do { \
        if (tid < MMQ_BM) { int gw = row_block + tid; \
            ...
            if (gw < rows) opentail-llm_cp_async16(dstp, &qw[((long)gw * nb + (KB)) * 4L]); \
            else { dstp[0] = dstp[1] = dstp[2] = dstp[3] = 0; } } \
        { int token = tid >> 1, half = tid & 1; int gy = tok_block + token; \
            int* dstp = &sY[ST][token * 8 + half * 4]; \
            if (gy < n_tok) opentail-llm_cp_async16(dstp, &y_qs[((long)gy * nb + (KB)) * 8L + half * 4]); \
    ...
    for (int kb = 0; kb < nb; kb++) {
        ...
        if (tid < MMQ_BN) { int gt = tok_block + tid;
            if (gt < n_tok) { unsigned int dw = y_ds[(long)gt * nb + kb];
                sYd[tid] = opentail-llm_fp16_to_fp32(dw & 0xffffu); sYs[tid] = opentail-llm_fp16_to_fp32(dw >> 16); }
...
__device__ __forceinline__ void opentail-llm_q4k_scale_min(
    ...
    #define MMQ_LOAD_TILE_Q4K(KB) do { \
        ...
        if (tid < MMQ_BM) { int gw = row_block + tid; \
          if (gw < rows) { long hb = (long)(gw * nb_super + kbs) * 36L; \
            ...
            unsigned int sc, mn; opentail-llm_q4k_scale_min(sm0, sm1, sm2, sb, &sc, &mn); \
            rWdd = opentail-llm_fp16_to_fp32(w0 & 0xffffu) * (float)sc; \
            rWdm = opentail-llm_fp16_to_fp32(w0 >> 16)     * (float)mn; \
        ...
        if (tid < MMQ_BN) { int gt = tok_block + tid; \
          if (gt < n_tok) { unsigned int dw = *reinterpret_cast<const unsigned int*>(y_q81 + ((long)gt * sub_total + (KB)) * 36L); \
            rYd = opentail-llm_fp16_to_fp32(dw & 0xffffu); rYs = opentail-llm_fp16_to_fp32(dw >> 16); } \
    ...
    #define MMQ_LOAD_TILE_Q4K_SOA(KB) do { \
        ...
        if (tid < MMQ_BM) { int gw = row_block + tid; \
          if (gw < rows) { long sbi = (long)gw * nb_super + kbs; \
            ...
            rWdd = opentail-llm_fp16_to_fp32(dd & 0xffffu) * (float)sc; \
            rWdm = opentail-llm_fp16_to_fp32(dd >> 16)     * (float)mn; \
          } else { rWdd = 0.f; rWdm = 0.f; } } \
        int gy0 = tok_block + (tid >> 3); \
        rY0 = (gy0 < n_tok) ? *reinterpret_cast<const unsigned int*>(y_q81 + ((long)gy0 * sub_total + (KB)) * 36L + 4 + (long)(tid & 7) * 4) : 0u; \
        int gy1 = tok_block + ((tid + 256) >> 3); \
...
// opentail-llm_uint_at (funnelshift). Argmax-stable, not bit-exact (both operands int8).
//
    ...
    int rows, int cols, int n_tok)
{
    ...
    #define MMQ_LOAD_TILE_Q40(KB) do { \
        ...
        int gw0 = row_block + (tid >> 3); \
        { unsigned int w = (gw0 < rows) ? opentail-llm_uint_at(weights, ((long)gw0 * nb + (KB)) * 18L + 2 + (long)qword * 4) : 0u; \
          rWq0 = (ww < 4) ? (w & 0x0F0F0F0Fu) : ((w >> 4) & 0x0F0F0F0Fu); } \
        int gw1 = row_block + ((tid + 256) >> 3); \
        { unsigned int w = (gw1 < rows) ? opentail-llm_uint_at(weights, ((long)gw1 * nb + (KB)) * 18L + 2 + (long)qword * 4) : 0u; \
          rWq1 = (ww < 4) ? (w & 0x0F0F0F0Fu) : ((w >> 4) & 0x0F0F0F0Fu); } \
        if (tid < MMQ_BM) { int gw = row_block + tid; \
          if (gw < rows) { long bb = ((long)gw * nb + (KB)) * 18L; \
            rWd = opentail-llm_fp16_to_fp32(opentail-llm_byte_at(weights, bb) | (opentail-llm_byte_at(weights, bb + 1) << 8)); } \
          else rWd = 0.f; } \
        int gy0 = tok_block + (tid >> 3); \
        rY0 = (gy0 < n_tok) ? *reinterpret_cast<const unsigned int*>(y_q81 + ((long)gy0 * nb + (KB)) * 36L + 4 + (long)(tid & 7) * 4) : 0u; \
        int gy1 = tok_block + ((tid + 256) >> 3); \
    ...
    int rows, int cols)
{
    ...
    for (int c = (int)threadIdx.x; c < cols; c += (int)blockDim.x) {
        ...
        float d = opentail-llm_fp16_to_fp32(dlo | (dhi << 8));
        int q = opentail-llm_int8_at(weights, b0 + 2 + (long)lane);
        out[out_row + c] = (unsigned short)opentail-llm_fp32_to_fp16(d * (float)q);
    }
}
    ...
    int rows, int cols)
{
    ...
    for (int c = (int)threadIdx.x; c < cols; c += (int)blockDim.x) {
        ...
        unsigned int dhi = opentail-llm_byte_at(weights, b0 + 1);
        float d = opentail-llm_fp16_to_fp32(dlo | (dhi << 8));
        unsigned int qbyte = opentail-llm_byte_at(weights, b0 + 2 + (long)(within & 15));
...
// (opentail-llm_uint_at) in the MMQ / dp4a readers, which the roofline puts at ~52 TFLOPS
    ...
    int rows, int cols)
{
    ...
    for (int block = 0; block < num_blocks; block++) {
        ...
        float d = opentail-llm_fp16_to_fp32(ws[blk_idx]);
        unsigned int qbyte = opentail-llm_byte_at(weights, blk_idx * 16L + (long)(lane & 15));
        int nib = (lane < 16) ? (int)(qbyte & 0xFu) : (int)(qbyte >> 4);
        int q = nib - 8;
        float x = input[block * 32 + lane];
        acc += d * (float)q * x;
    ...
    int rows, int cols)
{
    ...
    for (int block0 = warp_id * 8; block0 < num_blocks; block0 += MATVEC_Q40_NWARPS * 8) {
        ...
        if (block < num_blocks) {
            ...
            float dw = opentail-llm_fp16_to_fp32(ws[blk_idx]);
...
// SoA Q4_0 → fp16 dequant (cuBLAS prefill GEMM fallback, STINGRAY_PREFILL_MMQ=0) —
    ...
    int rows, int cols)
{
    ...
    for (int c = (int)threadIdx.x; c < cols; c += (int)blockDim.x) {
        ...
        long blk_idx = srow + block;
        float d = opentail-llm_fp16_to_fp32(ws[blk_idx]);
        unsigned int qbyte = opentail-llm_byte_at(weights, blk_idx * 16L + (long)(within & 15));
        int nib = (within < 16) ? (int)(qbyte & 0xFu) : (int)(qbyte >> 4);
        int q = nib - 8;
        out[out_row + c] = (unsigned short)opentail-llm_fp32_to_fp16(d * (float)q);
    }
...
// 8-warp mma accumulation), aligned weight uint loads instead of opentail-llm_uint_at.
#define MMQ_BM 64
#define MMQ_BN 128
    ...
    int rows, int cols)
{
    ...
    for (int block = 0; block < num_blocks; block++) {
        ...
        out[out_row + (long)block * 256 + (int)tid] = (unsigned short)opentail-llm_fp32_to_fp16(val);
        __syncthreads();
    }
}
    ...
    int rows, int cols)
{
    ...
    for (int block = 0; block < num_blocks; block++) {
        ...
        unsigned int dlo = opentail-llm_byte_at(weights, b0 + 108);
        unsigned int dhi = opentail-llm_byte_at(weights, b0 + 109);
        float dAll = opentail-llm_fp16_to_fp32(dlo | (dhi << 8));

        unsigned int a0 = opentail-llm_byte_at(weights,b0+96)|(opentail-llm_byte_at(weights,b0+97)<<8)|(opentail-llm_byte_at(weights,b0+98)<<16)|(opentail-llm_byte_at(weights,b0+99)<<24);
        unsigned int a1 = opentail-llm_byte_at(weights,b0+100)|(opentail-llm_byte_at(weights,b0+101)<<8)|(opentail-llm_byte_at(weights,b0+102)<<16)|(opentail-llm_byte_at(weights,b0+103)<<24);
        unsigned int tmp= opentail-llm_byte_at(weights,b0+104)|(opentail-llm_byte_at(weights,b0+105)<<8)|(opentail-llm_byte_at(weights,b0+106)<<16)|(opentail-llm_byte_at(weights,b0+107)<<24);
        unsigned int aux[4];
        ...
        unsigned int qsb = opentail-llm_byte_at(weights, b0 + 32 + half * 32 + lane);
        unsigned int hmb = opentail-llm_byte_at(weights, b0 + lane);
...
// Used only on the STINGRAY_PREFILL_MMQ=0 dequant→fp16→GEMM fallback so a SoA
    ...
    int rows, int cols)
{
    ...
    for (int block = 0; block < num_blocks; block++) {
        ...
        unsigned int dd = __ldg(&dReg[sb]);
        float d    = opentail-llm_fp16_to_fp32(dd & 0xffffu);
        float dmin = opentail-llm_fp16_to_fp32(dd >> 16);
    ...
    int rows, int cols)
{
    ...
    for (int block = 0; block < num_blocks; block++) {
        long b0 = row_base_bytes + (long)block * 210L;
        unsigned int dlo = opentail-llm_byte_at(weights, b0 + 208);
        unsigned int dhi = opentail-llm_byte_at(weights, b0 + 209);
        float d = opentail-llm_fp16_to_fp32(dlo | (dhi << 8));
        ...
        for (int e = (int)threadIdx.x; e < 256; e += (int)blockDim.x) {
            ...
            float sc  = d * (float)opentail-llm_int8_at(weights, b0 + 192 + group * 2 + isc);

            unsigned int ql, qh; int q;
            switch (group) {
                case 0:  ql = opentail-llm_byte_at(weights, b0 +   0 + lane); qh = opentail-llm_byte_at(weights, b0 + 128 + lane);
                         q = (int)((ql & 0xFu)        | (((qh >> 0) & 3u) << 4)); break;
                case 1:  ql = opentail-llm_byte_at(weights, b0 +  32 + lane); qh = opentail-llm_byte_at(weights, b0 + 128 + lane);
                         q = (int)((ql & 0xFu)        | (((qh >> 2) & 3u) << 4)); break;
                case 2:  ql = opentail-llm_byte_at(weights, b0 +   0 + lane); qh = opentail-llm_byte_at(weights, b0 + 128 + lane);
                         q = (int)(((ql >> 4) & 0xFu) | (((qh >> 4) & 3u) << 4)); break;
                case 3:  ql = opentail-llm_byte_at(weights, b0 +  32 + lane); qh = opentail-llm_byte_at(weights, b0 + 128 + lane);
                         q = (int)(((ql >> 4) & 0xFu) | (((qh >> 6) & 3u) << 4)); break;
                case 4:  ql = opentail-llm_byte_at(weights, b0 +  64 + lane); qh = opentail-llm_byte_at(weights, b0 + 160 + lane);
                         q = (int)((ql & 0xFu)        | (((qh >> 0) & 3u) << 4)); break;
                case 5:  ql = opentail-llm_byte_at(weights, b0 +  96 + lane); qh = opentail-llm_byte_at(weights, b0 + 160 + lane);
                         q = (int)((ql & 0xFu)        | (((qh >> 2) & 3u) << 4)); break;
                case 6:  ql = opentail-llm_byte_at(weights, b0 +  64 + lane); qh = opentail-llm_byte_at(weights, b0 + 160 + lane);
                         q = (int)(((ql >> 4) & 0xFu) | (((qh >> 4) & 3u) << 4)); break;
                default: ql = opentail-llm_byte_at(weights, b0 +  96 + lane); qh = opentail-llm_byte_at(weights, b0 + 160 + lane);
                         q = (int)(((ql >> 4) & 0xFu) | (((qh >> 6) & 3u) << 4)); break;
            }
            float val = sc * (float)(q - 32);
            out[out_row + (long)block * 256 + e] = (unsigned short)opentail-llm_fp32_to_fp16(val);
        }
    }
}
...
// SAME 6-bit packing as Q4_K (opentail-llm_q4k_scale_min over sub-block e>>5); the value is
    ...
    int rows, int cols)
{
    ...
    for (int block = 0; block < num_blocks; block++) {
        ...
        for (int e = (int)threadIdx.x; e < 256; e += (int)blockDim.x) {
            ...
            unsigned int sc, mn;
            opentail-llm_q4k_scale_min(sm0, sm1, sm2, (e >> 5), &sc, &mn);   // sub-block = e/32
...
{
    int i = (int)(blockIdx.x * blockDim.x + threadIdx.x);
    if (i < n) out[i] = (unsigned short)opentail-llm_fp32_to_fp16(in[i]);
}
    ...
    int rows, int cols)
{
    ...
    float ra = opentail-llm_warp_reduce_sum(acc_a);
    float rb = opentail-llm_warp_reduce_sum(acc_b);
    if (lane == 0) {
        output_a[row] = ra;
        output_b[row] = rb;
    }
    ...
    int rows, int cols)
{
    ...
    for (int block = warp_id; block < num_blocks; block += MATVEC_Q4K_NWARPS) {
        ...
        float d8_lo_a = opentail-llm_fp16_to_fp32(d_bits_lo_a);
        float d8_hi_a = opentail-llm_fp16_to_fp32(d_bits_hi_a);
        float d8_lo_b = opentail-llm_fp16_to_fp32(d_bits_lo_b);
        float d8_hi_b = opentail-llm_fp16_to_fp32(d_bits_hi_b);
    ...
    acc_a = opentail-llm_warp_reduce_sum(acc_a);
    acc_b = opentail-llm_warp_reduce_sum(acc_b);
    ...
    for (int block = 0; block < num_blocks; block++) {
        ...
        for (int chunk = 0; chunk < 4; chunk++) {
            ...
            if (j_lo < 4) {
                ...
                unsigned int a_lo = opentail-llm_byte_at(weights, b0 + 4 + j_lo + 4);
                unsigned int b_lo = opentail-llm_byte_at(weights, b0 + 4 + j_lo - 4);
                unsigned int c_lo = opentail-llm_byte_at(weights, b0 + 4 + j_lo);
                sc1 = (a_lo & 0xFu) | (((b_lo >> 6) & 3u) << 4);
                m1  = ((a_lo >> 4) & 0xFu) | (((c_lo >> 6) & 3u) << 4);
...
// STINGRAY_PREFILL_MMQ=0 fallback prefill path so a SoA weight never throws there.
...
// opentail-llm_byte_at / opentail-llm_int8_at byte gathers (cf. llm_matvec_q6k), never a
    ...
    int rows, int cols, int n_tok)
{
    ...
    for (int block = 0; block < num_blocks; block++) {
        ...
        const unsigned int kmask1 = 0x03030303u;
        const unsigned int kmask2 = 0x0f0f0f0fu;
        unsigned int a0 = opentail-llm_byte_at(weights, b0 + 96)
                        | (opentail-llm_byte_at(weights, b0 + 97) << 8)
                        | (opentail-llm_byte_at(weights, b0 + 98) << 16)
                        | (opentail-llm_byte_at(weights, b0 + 99) << 24);
        unsigned int a1 = opentail-llm_byte_at(weights, b0 + 100)
                        | (opentail-llm_byte_at(weights, b0 + 101) << 8)
                        | (opentail-llm_byte_at(weights, b0 + 102) << 16)
                        | (opentail-llm_byte_at(weights, b0 + 103) << 24);
        unsigned int tmp = opentail-llm_byte_at(weights, b0 + 104)
                        | (opentail-llm_byte_at(weights, b0 + 105) << 8)
                        | (opentail-llm_byte_at(weights, b0 + 106) << 16)
                        | (opentail-llm_byte_at(weights, b0 + 107) << 24);
        unsigned int aux[4];
        aux[2] = ((a0 >> 4) & kmask2) | (((tmp >> 4) & kmask1) << 4);
        aux[3] = ((a1 >> 4) & kmask2) | (((tmp >> 6) & kmask1) << 4);
        aux[0] = (a0 & kmask2) | (((tmp >> 0) & kmask1) << 4);
    ...
    int max_seq_len, int num_kv_heads, int block_bytes)
{
    ...
    for (int iter = 0; iter < 2; iter++) {
        ...
        if (tid == 0) {
            ...
            unsigned int norm_bits = opentail-llm_fp32_to_fp16(norm) & 0xFFFFu;
    ...
    int tq_seq_len, int fp32_seq_len, int max_seq_len, int block_bytes)
{
    ...
    for (int t_base = 0; t_base < tq_seq_len; t_base += 8) {
        ...
        if (t < tq_seq_len) {
            ...
            float norm = opentail-llm_fp16_to_fp32(k_block_uints[warp][0] & 0xFFFFu);
    ...
    if (use_shared) {
        for (int t = total_seq + tid; t < MAX_SHARED_SCORES; t += 256)
            shared_scores[t] = opentail-llm_neg_inf();
    }
    __syncthreads();
    ...
    for (int d = tid; d < head_dim; d += 256) {
        ...
        for (int t = 0; t < tq_seq_len; t++) {
            ...
            long base_uint = (long)t * row_stride_uints + head_uint_offset;
            float norm = opentail-llm_fp16_to_fp32(v_cache_tq[base_uint] & 0xFFFFu);
    ...
    int k_tile_bytes, int v_tile_bytes)
{
    ...
    if (tid < KPF_MQ) { s_m[tid] = opentail-llm_neg_inf(); s_l[tid] = 0.0f; }
    ...
    for (int wb = 0; wb < w_total; wb += T) {
        ...
        // tail chunks must not leave a warp half-active around a full-mask shuffle).
        {
            ...
            bool ok = mnew > opentail-llm_neg_inf();
    ...
    float* __restrict__ c_out)        // [16*8]  row-major C = A·B
{
    ...
    unsigned int a0 = opentail-llm_f32x2_to_f16x2(a_in[(gid    ) * 16 + (2 * tig    )], a_in[(gid    ) * 16 + (2 * tig + 1)]);
    unsigned int a1 = opentail-llm_f32x2_to_f16x2(a_in[(gid + 8) * 16 + (2 * tig    )], a_in[(gid + 8) * 16 + (2 * tig + 1)]);
    unsigned int a2 = opentail-llm_f32x2_to_f16x2(a_in[(gid    ) * 16 + (2 * tig + 8)], a_in[(gid    ) * 16 + (2 * tig + 9)]);
    unsigned int a3 = opentail-llm_f32x2_to_f16x2(a_in[(gid + 8) * 16 + (2 * tig + 8)], a_in[(gid + 8) * 16 + (2 * tig + 9)]);

    // B fragment (16x8 fp16, col-major): 2 regs, each a K-row-pair of column `gid`.
    unsigned int b0 = opentail-llm_f32x2_to_f16x2(b_in[(2 * tig    ) * 8 + gid], b_in[(2 * tig + 1) * 8 + gid]);
    unsigned int b1 = opentail-llm_f32x2_to_f16x2(b_in[(2 * tig + 8) * 8 + gid], b_in[(2 * tig + 9) * 8 + gid]);
...
__device__ __forceinline__ float fatc_mask(float s, int qpos, int abs_k, int window_size)
{
    ...
    return ok ? s : opentail-llm_neg_inf();
}
#define FATC_KT 16
    ...
    int start_pos, int window_size, int max_seq_len, int n_tok, float attn_scale)
{
    ...
    float m0 = opentail-llm_neg_inf(), l0 = 0.f;   // running max/sum, query row gid
    float m1 = opentail-llm_neg_inf(), l1 = 0.f;   // query row gid+8
    ...
    for (int kt0 = key_start; kt0 < key_end; kt0 += FATC_KT) {
        ...
        for (int idx = lane; idx < FATC_KT * head_dim; idx += 32) {
            ...
            sKV[idx] = (unsigned short)opentail-llm_fp32_to_fp16(kv);
        }
        __syncthreads();
        ...
        for (int dc = 0; dc < nchunk; dc++) {
            ...
            unsigned int a0 = opentail-llm_f32x2_to_f16x2(qa0l, qa0h);
            unsigned int a1 = opentail-llm_f32x2_to_f16x2(qa1l, qa1h);
            unsigned int a2 = opentail-llm_f32x2_to_f16x2(qa2l, qa2h);
            unsigned int a3 = opentail-llm_f32x2_to_f16x2(qa3l, qa3h);
        ...
        bool ok0 = mnew0 > opentail-llm_neg_inf();
        bool ok1 = mnew1 > opentail-llm_neg_inf();
        ...
        unsigned int pa0 = opentail-llm_f32x2_to_f16x2(p0[0], p0[1]);
        unsigned int pa1 = opentail-llm_f32x2_to_f16x2(p0[2], p0[3]);
        unsigned int pa2 = opentail-llm_f32x2_to_f16x2(p1[0], p1[1]);
        unsigned int pa3 = opentail-llm_f32x2_to_f16x2(p1[2], p1[3]);
        ...
        for (int idx = lane; idx < FATC_KT * head_dim; idx += 32) {
            ...
            sKV[idx] = (unsigned short)opentail-llm_fp32_to_fp16(vv);
        }
        __syncthreads();
...
// cache-load sites, which go through opentail-llm_kvload — so the float instantiation is
// byte-identical to the pre-#179 kernel. Three extern ""C"" thunks below give NVRTC
// stable entry points (fp32 / bf16 / q8_0).
template<typename KV>
__device__ __forceinline__ void opentail-llm_flash_attn_prefill_tc2_impl(
    const float* __restrict__ q_all,
    ...
    int start_pos, int window_size, int max_seq_len, int n_tok, float attn_scale)
{
    ...
    float m0 = opentail-llm_neg_inf(), l0 = 0.f;
    float m1 = opentail-llm_neg_inf(), l1 = 0.f;
    ...
    for (int kt0 = key_start; kt0 < key_end; kt0 += FATC2_KT) {
        ...
        for (int idx = tid; idx < FATC2_KT * head_dim; idx += FATC2_W * 32) {
            ...
            // wraps a window-sized SWA ring); abs_k stays logical for the causal bound.
            float kv = (abs_k < key_end)
                ? opentail-llm_kvload(k_cache, (long)(abs_k % max_seq_len) * kv_dim + (long)kv_head * head_dim + d) : 0.f;
            sKV[idx] = (unsigned short)opentail-llm_fp32_to_fp16(kv);
        }
        __syncthreads();
        ...
        for (int idx = tid; idx < FATC2_KT * head_dim; idx += FATC2_W * 32) {
            ...
            int abs_k = kt0 + kk;
            float vv = (abs_k < key_end)
                ? opentail-llm_kvload(v_cache, (long)(abs_k % max_seq_len) * kv_dim + (long)kv_head * head_dim + d) : 0.f;
            sKV[idx] = (unsigned short)opentail-llm_fp32_to_fp16(vv);
        }
        __syncthreads();
...
extern ""C"" __global__ void llm_flash_attn_prefill_tc2(
    ...
    int start_pos, int window_size, int max_seq_len, int n_tok, float attn_scale)
{
    opentail-llm_flash_attn_prefill_tc2_impl<float>(q_all, k_cache, v_cache, out_all,
...
extern ""C"" __global__ void llm_flash_attn_prefill_tc2_bf16(
    ...
    int start_pos, int window_size, int max_seq_len, int n_tok, float attn_scale)
{
    opentail-llm_flash_attn_prefill_tc2_impl<unsigned short>(q_all, k_cache, v_cache, out_all,
        num_heads, num_kv_heads, head_dim, start_pos, window_size, max_seq_len, n_tok, attn_scale);
}
...
extern ""C"" __global__ void llm_flash_attn_prefill_tc2_q8_0(
    ...
    int start_pos, int window_size, int max_seq_len, int n_tok, float attn_scale)
{
    opentail-llm_flash_attn_prefill_tc2_impl<block_q8_0>(q_all, k_cache, v_cache, out_all,
        num_heads, num_kv_heads, head_dim, start_pos, window_size, max_seq_len, n_tok, attn_scale);
}
    ...
    int kt_tile, float attn_scale)
{
    ...
    if (active) {
        ...
        for (int p = 0; p < ndj2; p++) {
            int pi = lane + 32 * p;
            if (pi < hd2) qh2[p] = opentail-llm_f32x2_to_f16x2(q_all[q_base + 2 * pi], q_all[q_base + 2 * pi + 1]);
        }
    }
    float m_run = opentail-llm_neg_inf();
    float l_run = 0.f;
    ...
    for (int kt0 = blk_key_start; kt0 < blk_key_end; kt0 += kt_tile) {
        ...
        for (int idx = tid; idx < kt_tile * hd2; idx += (int)blockDim.x) {
            ...
            if (kk < tile_keys) {
                ...
                kh = opentail-llm_f32x2_to_f16x2(k_cache[off], k_cache[off + 1]);
            }
            sKh[idx] = kh;
        }
        ...
        if (active) {
            for (int kk = 0; kk < tile_keys; kk++) {
                ...
                for (int p = 0; p < ndj2; p++) {
                    int pi = lane + 32 * p;
                    if (pi < hd2) acc = opentail-llm_hfma2(qh2[p], sKh[kk * hd2 + pi], acc);
                }
                float part = opentail-llm_f16x2_sum(acc);
...
// q8_0). Body unchanged bar the two cache loads, which go through opentail-llm_kvload;
    ...
    int window_start, int window_end, int max_seq_len, float attn_scale)
{
    ...
    for (int t = (int)tid; t < eff_seq; t += 256) {
        int abs_t = t + window_start;
        long k_off = (long)(abs_t % max_seq_len) * (long)kv_dim + (long)kv_head * (long)head_dim;
        float dot = opentail-llm_kv_dot(q + q_off, k_cache, k_off, head_dim);
        float score = dot * scale;
        if (use_shared) shared_scores[t] = score;
        else            head_scratch[t]  = score;
    }
    ...
    for (int d = (int)tid; d < head_dim; d += 256) {
        ...
        for (int t = 0; t < eff_seq; t++) {
            ...
            float weight = use_shared ? shared_scores[t] : head_scratch[t];
            long v_off = (long)(abs_t % max_seq_len) * (long)kv_dim + (long)kv_head * (long)head_dim;
            acc += weight * opentail-llm_kvload(v_cache, v_off + d);
        }
        out[out_off + d] = acc;
    }
}
...
// (head, query); K/V decoded to fp32 on load (opentail-llm_kvload), all arithmetic fp32.
    ...
    int start_pos, int window_size, int max_seq_len, int n_tok, float attn_scale)
{
    ...
    for (int t = (int)tid; t < eff_seq; t += 256) {
        ...
        long k_off = (long)(abs_t % max_seq_len) * (long)kv_dim + (long)kv_head * (long)head_dim;
        for (int dd = 0; dd < head_dim; dd++)
            dot += q[q_off + dd] * opentail-llm_kvload(k_cache, k_off + dd);
        shared_scores[t] = dot * scale;
    }
    __syncthreads();
    ...
    for (int dd = (int)tid; dd < head_dim; dd += 256) {
        ...
        for (int t = 0; t < eff_seq; t++) {
            int abs_t = t + window_start;
            long v_off = (long)(abs_t % max_seq_len) * (long)kv_dim + (long)kv_head * (long)head_dim;
            acc += shared_scores[t] * opentail-llm_kvload(v_cache, v_off + dd);
        }
        out[out_off + dd] = acc;
    }
}
...
// (stored as raw unsigned short, decoded via opentail-llm_bf16_to_fp32). Score
...
// K/V are decoded via opentail-llm_kv_dot / opentail-llm_kvload → fp32/bf16/q8_0 thunks.
    ...
    int seq_len, int n_splits, float attn_scale)
{
    ...
    if (t0 >= seq_len) {
        if (tid == 0) { partial_meta[meta_off] = opentail-llm_neg_inf(); partial_meta[meta_off + 1] = 0.f; }
        return;
    }
    ...
    for (int t = (int)tid; t < n; t += 256) {
        ...
        sk_scores[t] = opentail-llm_kv_dot(q + q_off, k_cache, k_off, head_dim) * scale;
    }
    __syncthreads();
    ...
    for (int d = (int)tid; d < head_dim; d += 256) {
        ...
        for (int t = 0; t < n; t++) {
            ...
            acc += sk_scores[t] * opentail-llm_kvload(v_cache, v_off + d);
        }
    ...
    int num_heads, int head_dim, int n_splits)
{
    ...
    float lmax = opentail-llm_neg_inf();
...
// opentail-llm_kv_dot calls each re-reading the row). q8_0 keeps the per-32-block scale cache
// (block-walk identical to opentail-llm_kv_dot). dots[] must hold ≥ GF (GF ≤ 8, host-enforced).
__device__ __forceinline__ void opentail-llm_kv_dot_multi(const float* __restrict__ q_base, int head_dim, int gf,
                                                     ...
                                                     const unsigned short* __restrict__ k, long off, float* dots)
{
    for (int g = 0; g < gf; g++) dots[g] = 0.f;
    for (int d = 0; d < head_dim; d++) {
        float kd = opentail-llm_bf16_to_fp32((unsigned int)k[off + d]);
        for (int g = 0; g < gf; g++) dots[g] += q_base[(long)g * (long)head_dim + d] * kd;
    }
}
__device__ __forceinline__ void opentail-llm_kv_dot_multi(const float* __restrict__ q_base, int head_dim, int gf,
                                                     const block_q8_0* __restrict__ k, long off, float* dots)
{
    ...
    for (int d = 0; d < head_dim; ) {
        float s = opentail-llm_fp16_to_fp32((unsigned int)k[b].d);
    ...
    int seq_len, int n_splits, float attn_scale)
{
    ...
    if (t0 >= seq_len) {
        if ((int)tid < G) {
            ...
            partial_meta[mo] = opentail-llm_neg_inf(); partial_meta[mo + 1] = 0.f;
        }
        return;
    }
    int t1 = t0 + SPLITKV_CHUNK; if (t1 > seq_len) t1 = seq_len;
    ...
    for (int t = (int)tid; t < n; t += 256) {
        float dots[8];
        opentail-llm_kv_dot_multi(q_base, head_dim, G, k_cache, kv_base + (long)t * kv_dim_l, dots);
    ...
    for (int d = (int)tid; d < head_dim; d += 256) {
        ...
        for (int t = 0; t < n; t++) {
            float vd = opentail-llm_kvload(v_cache, kv_base + (long)t * kv_dim_l + d);
    ...
    int prompt_len, int q_abs_pos, int max_seq_len)
{
    ...
    for (int t = (int)tid; t < prompt_len; t += 256) {
        float s = use_shared ? shared_scores[t] : head_scratch[t];
        float e = (s == opentail-llm_neg_inf()) ? 0.f : __expf(s - max_val);
        if (use_shared) shared_scores[t] = e;
        else            head_scratch[t]  = e;
        local_sum += e;
    }
...
// opentail-llm_bf16_to_fp32; arithmetic stays in fp32. Used when STINGRAY bf16-KV is
    ...
    int prompt_len, int q_abs_pos, int max_seq_len)
{
    ...
    for (int t = (int)tid; t < prompt_len; t += 256) {
        ...
        if (t > q_abs_pos) {
            ...
            long k_off = (long)t * (long)kv_dim + (long)kv_head * (long)head_dim;
            for (int d = 0; d < head_dim; d++)
                dot += q[q_off + d] * opentail-llm_bf16_to_fp32((unsigned int)k_cache[k_off + d]);
            score = dot * scale;
        }
        if (use_shared) shared_scores[t] = score;
        else            head_scratch[t]  = score;
    ...
    int hv, int d)
{
    ...
    float kv_col = opentail-llm_warp_reduce_sum(kv_shard);
    ...
    float o_col = opentail-llm_warp_reduce_sum(attn_shard) * rsqrtf((float)d);
    ...
    int kv_dim, int start_pos, int max_seq_len, int n_tok)
{
    ...
    long off = (long)((start_pos + i) % max_seq_len) * (long)kv_dim + (long)e;
    k_cache[off] = (unsigned short)opentail-llm_fp32_to_bf16(k_all[(long)i * kv_dim + e]);
    v_cache[off] = (unsigned short)opentail-llm_fp32_to_bf16(v_all[(long)i * kv_dim + e]);
}
    ...
    int kv_dim, int start_pos, int max_seq_len, int n_tok)
{
    ...
    opentail-llm_q8_append_one(valid ? k_all[(long)i * kv_dim + e] : 0.f, valid, k_cache, block, lane);
    opentail-llm_q8_append_one(valid ? v_all[(long)i * kv_dim + e] : 0.f, valid, v_cache, block, lane);
}
...
// decoding each cache element to fp32 on load via opentail-llm_kvload.
template<typename KV>
...
// path, decoding each cache element to fp32 on load via opentail-llm_kvload.
template<typename KV>
    ...
    int start_pos, int max_seq_len, int n_tok, int score_stride, float attn_scale)
{
    ...
    for (int dd = (int)tid; dd < head_dim; dd += 256) {
        ...
        for (int t = 0; t < seq_len; t++) {
            long v_off = (long)t * (long)kv_dim + (long)kv_head * (long)head_dim;
            acc += sc[t] * opentail-llm_kvload(v_cache, v_off + dd);
        }
        out[out_off + dd] = acc;
    }
}
