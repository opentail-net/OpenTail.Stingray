...
public sealed unsafe class CudaBackend : IComputeBackend, IImageOpsBackend, IVisionOpsBackend, IDisposable
{
    ...
    // compaction. Used by CudaHybridGdnForwardPass.Prefill when STINGRAY_SNAPKV_BUDGET
    ...
    // #205 kill-switch: STINGRAY_DECODE_MMQ_BM32=0 forces the BM=64 decode-MMQ tile for all
    ...
    private readonly bool _decodeMmqBm32Enabled =
        Environment.GetEnvironmentVariable("STINGRAY_DECODE_MMQ_BM32") != "0";
    // #204 kill-switch: STINGRAY_Q6K_DECODE_MMQ=0 disables the Q6_K decode-MMQ tile (the
    ...
    private readonly bool _q6kDecodeMmqEnabled =
        Environment.GetEnvironmentVariable("STINGRAY_Q6K_DECODE_MMQ") != "0";
    ...
    /// Default on (<c>STINGRAY_Q80_DP4A</c>); the dp4a path quantizes the activation
    ...
    public bool Q80Dp4aEnabled { get; set; } =
        Environment.GetEnvironmentVariable("STINGRAY_Q80_DP4A") != "0";
    ...
    /// <c>STINGRAY_TRUNK_MATVEC_FAST</c>. Only affects the <see cref="MatMul"/> N=1 path.
    /// </summary>
    ...
    /// <c>STINGRAY_GPU_ARGMAX=0</c> forces the legacy full-download path. Settable so parity oracles
    /// can pin both sides.
    /// </summary>
    public bool GpuArgmaxEnabled { get; set; } =
        Environment.GetEnvironmentVariable("STINGRAY_GPU_ARGMAX") != "0";
    ...
    /// (<c>llm_matvec_q4_0</c>). Default on (<c>STINGRAY_Q40_DP4A</c>); the dp4a path
    ...
    public bool Q40Dp4aEnabled { get; set; } =
        Environment.GetEnvironmentVariable("STINGRAY_Q40_DP4A") != "0";
    ...
    /// (<c>STINGRAY_ACT_SOA</c>=1 enables) until the e2e A/B win is proven; settable so
    ...
    public bool ActSoaEnabled { get; set; } =
        Environment.GetEnvironmentVariable("STINGRAY_ACT_SOA") == "1";
    ...
    /// <b>Default OFF</b> (<c>STINGRAY_ACT_SOA_CPA=1</c> to enable). It is a real
    ...
    public bool ActSoaCpaEnabled { get; set; } =
        Environment.GetEnvironmentVariable("STINGRAY_ACT_SOA_CPA") == "1";
    ...
    /// stalls on. <c>STINGRAY_BATCH_DECODE_WS_V2=0</c> restores the #194 byte-gather
    /// kernel. Settable so tests can A/B both generations on one backend.
    /// </summary>
    public bool WsV2Enabled { get; set; } =
        Environment.GetEnvironmentVariable("STINGRAY_BATCH_DECODE_WS_V2") != "0";
    ...
    {
        // STINGRAY_CUDA_PRECISION = fp32 | fp16 | bf16 | fp8 — debug override for the
        ...
        var env = Environment.GetEnvironmentVariable("STINGRAY_CUDA_PRECISION");
        if (env is not null)
        {
                              ...
                              int nTok, DType weightDType)
    {
        ...
        if (weightDType == DType.Q4_K)
        {
            // #156: the GEMM-N fallback prefill (STINGRAY_PREFILL_MMQ=0) over a
    ...
    /// <summary>
    /// Issue #201: int8 tensor-core batched-decode matmul (<c>STINGRAY_BATCH_DECODE_MMQ=1</c>).
                                       ...
                                       int nTok, DType weightDType)
    {
        ...
        // SoA-registered + the tile compiled + the kill-switch is on (STINGRAY_Q6K_DECODE_MMQ).
                                  ...
                                  int nTok, DType weightDType)
    {
        ...
        //    write a row-major [rows×cols] fp16 buffer for the GemmEx below.
        {
            ...
            int pRows = rows, pCols = cols;
            // #156: a repacked SoA Q4_K weight on this fallback (STINGRAY_PREFILL_MMQ=0)
    ...
    /// via <c>STINGRAY_ATTN_WAVE_BUDGET_MB</c> (default 256 MiB).
    /// </summary>
    private static long WaveScratchBudgetFloats()
    {
        long mb = 256;
        var ov = Environment.GetEnvironmentVariable("STINGRAY_ATTN_WAVE_BUDGET_MB");
    ...
    private void CompileAndLoadKernels()
    {
        ...
        byte[] nameBytes = NvrtcInterop.ToUtf8("opentail-llm_kernels.cu");
    ...
    private string GetCubinCachePath()
    {
        ...
        return Path.Combine(Path.GetTempPath(), $"opentail-llm_cubin_sm{_smVersion}_{hex}.bin");
    }
    ...
    private bool TryLoadCubinFromCache(string cacheFile)
    {
        ...
        try
        {
            ...
            // Defend against an older opentail-llm build that wrote PTX into the cubin path:
    ...
    /// <summary>Not implemented on CUDA yet -- same Vulkan-only status as MultiHeadAttention above.</summary>
    public Tensor MultiHeadAttentionTiled(Tensor q, Tensor k, Tensor v, int qSeq, int kvSeq, int numHeads, int headDim)
        => throw new NotSupportedException("MultiHeadAttentionTiled is not implemented on the CUDA backend yet -- Vulkan-only for now.");
    ...
    public void Dispose()
    {
        ...
        foreach (var (handle, entry) in _devPtrs)
        {
            if (_viewHandles.ContainsKey(handle)) continue;
            if (_pinnedAllocs.ContainsKey(handle))
                CuBlasInterop.FreeHost(entry.devPtr);
            else
