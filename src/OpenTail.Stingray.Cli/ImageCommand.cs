...
///     opentail-llm image -m models/z_image_turbo-Q5_K_M.gguf --vae vae/
...
///     opentail-llm image -m flux1-schnell-q4_k.gguf --vae ae.safetensors
...
public sealed class ImageCommand : Command<ImageCommand.Settings>
{
    public sealed class Settings : CommandSettings
    {
        ...
        [Description("Path to sd-cli executable used when --use-sdcpp is set (overrides STINGRAY_SDCPP env var)")]
    ...
    private static bool IsSd3(string path)
    {
        ...
        return n.Contains("sd3") || n.Contains("sd35") || n.Contains("sd-3") || n.Contains("sd_3") || n.Contains("mmdit");
    }

    private static int RunSd3(Settings s, string modelPath, int deviceIndex, bool deviceNone)
    {
        string output = s.OutputPath ?? "output.png";
        int steps = s.Steps > 0 ? s.Steps : 20;
        ...
        AnsiConsole.MarkupLine("[bold]Stable Diffusion 3 / 3.5 (MMDiT)[/]");
        AnsiConsole.MarkupLine($"[dim]Model:[/]    {Markup.Escape(modelPath)}");
    ...
    private static string? FindSdCli(string? explicit_)
    {
        ...
        var env = Environment.GetEnvironmentVariable("STINGRAY_SDCPP");
