...
///
/// <para>Real, deliberate simplification vs. the reference: `session.cpp` implements a real
/// staggered multi-stream DELAY pattern (`kDelayCacheSteps=4`, `kMimiFrameCodebooks=8` --
/// suggesting PersonaPlex's real 16 `lm_codebooks` may represent TWO 8-codebook Mimi streams,
/// e.g. two dialogue participants, each with its own delay window) that this port does NOT
/// implement -- this class feeds all 16 codebooks for the SAME frame together with no delay
/// offset, a real, flagged simplification analogous to this session's early (later corrected)
/// Higgs Audio TTS work before its own real delay pattern was found and implemented.</para>
/// </summary>
