...
/// <summary>
/// Real XTTS-v2 mel-spectrogram feature extractor for reference audio conditioning, matching
/// `dvae_wav_to_mel` (`TTS/tts/layers/xtts/dvae.py`) tensor-for-tensor -- a real
/// `torchaudio.transforms.MelSpectrogram(n_fft=1024, hop_length=256, win_length=1024, power=2,
/// normalized=False, sample_rate=22050, f_min=0, f_max=8000, n_mels=80, norm="slaney")` followed
/// by `log(clamp(mel, min=1e-5))` and a real per-mel-bin normalization against `mel_stats.pth`'s
/// values.
///
...
public sealed class XttsMelExtractor
{
    ...
    public XttsMelExtractor()
    {
        _hannWindow = new float[WinLength];
        for (int i = 0; i < WinLength; i++)
            _hannWindow[i] = 0.5f * (1.0f - MathF.Cos(2.0f * MathF.PI * i / WinLength)); // periodic Hann, matches torch.hann_window's default (periodic=True)
