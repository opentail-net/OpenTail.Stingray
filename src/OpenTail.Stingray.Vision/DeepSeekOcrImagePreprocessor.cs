...
public sealed record DeepSeekOcrPreprocessedImage(float[] Chw, int TargetWidth, int TargetHeight, int PatchesX, int PatchesY);

/// <summary>
/// Image preprocessor for DeepSeek-OCR and DeepSeek-OCR2 document vision models.
/// </summary>
public static class DeepSeekOcrImagePreprocessor
{
    private static readonly float[] Mean = { 0.485f, 0.456f, 0.406f };
    private static readonly float[] Std = { 0.229f, 0.224f, 0.225f };

    public static DeepSeekOcrPreprocessedImage Preprocess(ReadOnlySpan<byte> rgb, int width, int height, int imageSize = 1024, int patchSize = 16)
    {
        if (width <= 0 || height <= 0)
        ...
        return new DeepSeekOcrPreprocessedImage(chw, targetW, targetH, patchesX, patchesY);
    }
}
