...
public sealed record PixtralPreprocessedImage(float[] Chw, int TargetWidth, int TargetHeight, int PatchesX, int PatchesY);

/// <summary>
/// Preprocessor for Mistral / Pixtral 12B dynamic aspect ratio image encoding.
/// </summary>
public static class PixtralImagePreprocessor
{
    private static readonly float[] Mean = { 0.48145466f, 0.4578275f, 0.40821073f };
    private static readonly float[] Std = { 0.26862954f, 0.26130258f, 0.27577711f };

    public static PixtralPreprocessedImage Preprocess(ReadOnlySpan<byte> rgb, int width, int height, int patchSize = 16, int maxDim = 1024)
    {
        if (width <= 0 || height <= 0)
            throw new ArgumentException("Width and height must be positive.");

        // Calculate aspect-preserving dimensions aligned to patchSize (16)
        ...
        return new PixtralPreprocessedImage(chw, targetW, targetH, patchesX, patchesY);
    }
}
