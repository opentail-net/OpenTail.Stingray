...
public sealed record InternVlPreprocessedImage(float[] Chw, int TargetWidth, int TargetHeight, int PatchesX, int PatchesY);

/// <summary>
/// Image preprocessor for OpenGVLab InternVL 2.5 / 3 / 4.
/// </summary>
public static class InternVlImagePreprocessor
{
    private static readonly float[] Mean = { 0.485f, 0.456f, 0.406f };
    private static readonly float[] Std = { 0.229f, 0.224f, 0.225f };

    public static InternVlPreprocessedImage Preprocess(ReadOnlySpan<byte> rgb, int width, int height, int imageSize = 448, int patchSize = 14)
    {
        if (width <= 0 || height <= 0)
        ...
        return new InternVlPreprocessedImage(chw, targetW, targetH, patchesX, patchesY);
    }
}
