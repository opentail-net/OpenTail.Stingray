...
/// <summary>
/// Preallocated zero-allocation GPU memory workspace for UMT5-XXL text encoder inference.
/// Holds all activation, projection, attention, intermediate FFN buffers, and per-layer
/// relative position bias tensors in device-local VRAM across all 24 encoder layers.
/// </summary>
public sealed class UMT5GpuWorkspace : IDisposable
{
        ...
        int ffDim = 10240)
    {
        ...
        RelPosBias = new CoreTensor[relPosBiases.Length];
        for (int i = 0; i < relPosBiases.Length; i++)
        {
            RelPosBias[i] = backend.Upload(relPosBiases[i], TensorShape.D3(heads, seqLen, seqLen), exact: true);
        }
    }
    ...
    public void Dispose()
    {
        ...
        _backend.Free(Val);
        _backend.Free(FfOut);
        foreach (var rpb in RelPosBias) _backend.Free(rpb);
    }
}
