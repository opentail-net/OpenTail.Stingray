...
/// <summary>
/// GPU-resident VRAM representation of Stable Audio 3 Medium DiT weights.
/// Holds all 24 transformer layers (5-way QKV, Out, 2-way CrossQ, CrossOut, SwiGLU FFN),
/// input/output projections, memory tokens resident in VRAM as FP16.
/// Sized and structured for zero-allocation reuse across every Euler ODE step.
/// </summary>
public sealed class StableAudioMediumGpuWeights : IDisposable
{
    ...
    public sealed class BlockWeights : IDisposable
    {
        ...
        public BlockWeights(IComputeBackend backend, Func<string, float[]> getWeight, string prefix, int dim, int headDim, int ffInner)
        {
            ...
            // 5-way differential self-attention QKV [5 * dim, dim]
            SelfAttnQkvW = UploadWeight(backend, getWeight($"{prefix}.self_attn.to_qkv.weight"), TensorShape.D2(5 * dim, dim));
            ...
            // 2-way differential cross-attention Q [2 * dim, dim]
            CrossAttnQW = UploadWeight(backend, getWeight($"{prefix}.cross_attn.to_q.weight"), TensorShape.D2(2 * dim, dim));
    ...
    public StableAudioMediumGpuWeights(
