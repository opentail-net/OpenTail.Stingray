...
public sealed class Flux2GpuWeights : IDisposable
{
    ...
    private readonly IWeightLoader? _loader;
    ...
    public sealed class DoubleBlockGpuWeights : IDisposable
    {
        ...
        public DoubleBlockGpuWeights(IComputeBackend backend, IWeightLoader? loader, Func<string, float[]> getWeight, int idx, int d, int mlpHidden)
    {
        _backend = backend;
        string p = $"double_blocks.{idx}.";

        ImgAttnQkv = UploadWeight(backend, loader, $"{p}img_attn.qkv.weight", getWeight, TensorShape.D2(d * 3, d));
        TxtAttnQkv = UploadWeight(backend, loader, $"{p}txt_attn.qkv.weight", getWeight, TensorShape.D2(d * 3, d));
        ...
        ImgAttnProjWeight = UploadWeight(backend, loader, $"{p}img_attn.proj.weight", getWeight, TensorShape.D2(d, d));
        TxtAttnProjWeight = UploadWeight(backend, loader, $"{p}txt_attn.proj.weight", getWeight, TensorShape.D2(d, d));

        ImgMlp0Weight = UploadWeight(backend, loader, $"{p}img_mlp.0.weight", getWeight, TensorShape.D2(2 * mlpHidden, d));
        ImgMlp2Weight = UploadWeight(backend, loader, $"{p}img_mlp.2.weight", getWeight, TensorShape.D2(d, mlpHidden));
        TxtMlp0Weight = UploadWeight(backend, loader, $"{p}txt_mlp.0.weight", getWeight, TensorShape.D2(2 * mlpHidden, d));
        TxtMlp2Weight = UploadWeight(backend, loader, $"{p}txt_mlp.2.weight", getWeight, TensorShape.D2(d, mlpHidden));
    }
...
public sealed class SingleBlockGpuWeights : IDisposable
{
    ...
    public SingleBlockGpuWeights(IComputeBackend backend, IWeightLoader? loader, Func<string, float[]> getWeight, int idx, int d, int mlpHidden)
    {
        _backend = backend;
        string p = $"single_blocks.{idx}.";

        Linear1Weight = UploadWeight(backend, loader, $"{p}linear1.weight", getWeight, TensorShape.D2(3 * d + 2 * mlpHidden, d));
        ...
        Linear2Weight = UploadWeight(backend, loader, $"{p}linear2.weight", getWeight, TensorShape.D2(d, d + mlpHidden));
    }
...
/// </summary>
public Flux2GpuWeights(IComputeBackend backend, Func<string, float[]> getWeight, Flux2Params p, IWeightLoader? loader = null, bool includeSingleBlocks = false)
{
    _backend = backend;
    _loader = loader;
    ...
    ImgInWeight = UploadWeight(backend, loader, "img_in.weight", getWeight, TensorShape.D2(d, p.InChannels));
    TxtInWeight = UploadWeight(backend, loader, "txt_in.weight", getWeight, TensorShape.D2(d, p.ContextInDim));

    TimeInLayer0Weight = UploadWeight(backend, loader, "time_in.in_layer.weight", getWeight, TensorShape.D2(d, 256));
    TimeInLayer2Weight = UploadWeight(backend, loader, "time_in.out_layer.weight", getWeight, TensorShape.D2(d, d));

    if (p.GuidanceEmbed)
    {
        GuidanceInLayer0Weight = UploadWeight(backend, loader, "guidance_in.in_layer.weight", getWeight, TensorShape.D2(d, 256));
        GuidanceInLayer2Weight = UploadWeight(backend, loader, "guidance_in.out_layer.weight", getWeight, TensorShape.D2(d, d));
    }

    // Shared modulation -- ONE set, uploaded once (real finding, docs/087).
    DoubleModImgWeight = UploadWeight(backend, loader, "double_stream_modulation_img.lin.weight", getWeight, TensorShape.D2(6 * d, d));
    DoubleModTxtWeight = UploadWeight(backend, loader, "double_stream_modulation_txt.lin.weight", getWeight, TensorShape.D2(6 * d, d));
    SingleModWeight = UploadWeight(backend, loader, "single_stream_modulation.lin.weight", getWeight, TensorShape.D2(3 * d, d));

    DoubleBlocks = new DoubleBlockGpuWeights[p.DepthDoubleBlocks];
    for (int i = 0; i < p.DepthDoubleBlocks; i++)
        DoubleBlocks[i] = new DoubleBlockGpuWeights(backend, loader, getWeight, i, d, mlpHidden);
    ...
    for (int i = 0; i < SingleBlocks.Length; i++)
        SingleBlocks[i] = new SingleBlockGpuWeights(backend, loader, getWeight, i, d, mlpHidden);

    FinalModWeight = UploadWeight(backend, loader, "final_layer.adaLN_modulation.1.weight", getWeight, TensorShape.D2(2 * d, d));
    FinalLinearWeight = UploadWeight(backend, loader, "final_layer.linear.weight", getWeight, TensorShape.D2(p.OutChannels, d));
...
public SingleBlockGpuWeights LoadSingleBlock(int layerIdx)
{
    ...
    return new SingleBlockGpuWeights(_backend, _loader, _getWeight, layerIdx, d, mlpHidden);
}

private static unsafe CoreTensor UploadWeight(
    IComputeBackend backend,
    IWeightLoader? loader,
    string name,
    Func<string, float[]> fallbackGetWeight,
    TensorShape shape)
{
    if (loader != null
        ...
        && dtype == DType.Q4_K)
    {
        ...
        try
        {
            var fp16Gpu = backend.Allocate(shape, DType.Float16);
            int totalElements = (int)shape.ElementCount;
            int numBlocks = totalElements / 256;
            backend.DequantQ4KM(rawGpu, fp16Gpu, numBlocks);
            return fp16Gpu;
        }
        finally
        {
    ...
    var f32Data = fallbackGetWeight(name);
    return UploadWeight(backend, f32Data, shape);
}
