...
public sealed unsafe class VulkanBackend : IComputeBackend, IImageOpsBackend, IVisionOpsBackend, IDisposable
{
    ...
    /// <summary>fp32 x raw-bf16 GEMM via Shaders.SgemmBf16W (32-bit loads only, no extension needed).</summary>
    public bool SupportsBf16WeightSgemm => true;

    /// <summary>fp32 x raw fp8-E4M3 GEMM via Shaders.SgemmFp8W (32-bit loads only).</summary>
    public bool SupportsFp8WeightSgemm => true;
    ...
    private ComputePipeline? _sgemmBf16WPipeline;
    private ComputePipeline? _sgemmFp8WPipeline;
    ...
    public void MatMul(Tensor output, Tensor matrix, Tensor vector, DType weightDType)
    {
        ...
        switch (weightDType)
        {
            ...
            case DType.Float16:
                ...
                break;
            case DType.IQ4_XS:
                _matVecIQ4XSPipeline ??= new ComputePipeline(this, Shaders.MatVecIQ4XS, 3, pushConstantSize: sizeof(MatVecParams));
                DispatchOrRecord(_matVecIQ4XSPipeline, bufs, (totalRows + 7) / 8, &p);
                break;
            case DType.IQ3_S:
    ...
    public unsafe void Sgemm(Tensor C, Tensor A, Tensor B, int M, int K, int N, int inputRowOffsetElements)
    {
        ...
        // fp32 activations x raw fp8 E4M3 weights (see Shaders.SgemmFp8W).
        if (A.DType == DType.Float32 && B.DType == DType.Float8E4M3)
        {
            _sgemmFp8WPipeline ??= new ComputePipeline(this, Shaders.SgemmFp8W, 3,
                pushConstantSize: sizeof(SgemmParams));
            DispatchOrRecord(_sgemmFp8WPipeline, [GetBuffer(A), GetBuffer(B), GetBuffer(C)],
                ((uint)M + 127u) / 128u, &p, ((uint)N + 255u) / 256u);
            return;
        }

        // fp32 activations x raw-bf16 weights (see Shaders.SgemmBf16W).
        if (A.DType == DType.Float32 && B.DType == DType.BFloat16)
        {
            _sgemmBf16WPipeline ??= new ComputePipeline(this, Shaders.SgemmBf16W, 3,
                pushConstantSize: sizeof(SgemmParams));
            DispatchOrRecord(_sgemmBf16WPipeline, [GetBuffer(A), GetBuffer(B), GetBuffer(C)],
                ((uint)M + 127u) / 128u, &p, ((uint)N + 255u) / 256u);
            return;
        }
    ...
    {
        _addRowBroadcastPipeline ??= new ComputePipeline(this, Shaders.AddRowBroadcast, 2, pushConstantSize: sizeof(AddRowBroadcastParams));
        var p = new AddRowBroadcastParams { n = (uint)n, d = (uint)d };
        uint total = (uint)(n * d);
        uint groupsX = (total + 255u) / 256u;
    ...
    public unsafe void SgemmSiluGate(Tensor C, Tensor A, Tensor B, int M, int K, int N, int inputRowOffsetElements = 0)
    {
        ...
        if (A.DType == DType.Float32 && B.DType == DType.Float16 && HasShaderFloat16Int8 && Has16BitStorage)
        {
            ...
            _sgemmSiluGateF16Pipeline ??= new ComputePipeline(this, Shaders.SgemmSiluGateF16, 3,
                pushConstantSize: sizeof(SgemmParams));
            DispatchOrRecord(_sgemmSiluGateF16Pipeline, [GetBuffer(A), GetBuffer(B), GetBuffer(C)], gx128, &p, gy256);
            return;
        }
    ...
    public void Dispose()
    {
        ...
        _sgemmBf16WPipeline?.Dispose();
        _sgemmFp8WPipeline?.Dispose();
