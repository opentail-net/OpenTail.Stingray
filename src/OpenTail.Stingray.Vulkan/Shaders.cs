...
internal static class Shaders
{
    ...
    /// <summary>
    /// Batched RoPE with per-pair frequency factors and a magnitude scale, NORM (interleaved) or
    /// NEOX (half) pairing — the generic form of <see cref="RoPEBatched"/>/<see cref="RoPENeoxBatched"/>
    /// for models that ship <c>rope_freqs.weight</c> (Llama-3.1-style scaling, Apertus) or LongRoPE
    /// <c>rope_factors_short/long</c>. Pair i of row r (position base_pos + r) rotates by
    ...
    internal const string RoPEFactorsBatched = """
        ...
        void main() {
            ...
            uint head_base = row * num_heads * head_dim + h * head_dim;
            uint a_idx = neox != 0u ? head_base + i : head_base + 2u * i;
            uint b_idx = neox != 0u ? head_base + i + half_dim : head_base + 2u * i + 1u;
            float x0 = x_data[a_idx];
            float x1 = x_data[b_idx];
            x_data[a_idx] = x0 * cos_a - x1 * sin_a;
            x_data[b_idx] = x0 * sin_a + x1 * cos_a;
    ...
    /// A is [M, 2*K] float activations (first K is gate, second K is value).
    /// B is [N, K] float16 weights.
    /// C is [M, N] float output.
    /// Tile: 128x256 output per workgroup (512 threads, 8x8/thread), 32 K per step -- same kernel
    /// as <see cref="SgemmF16"/>, see there for the tiling rationale.
    /// Computes silu(gate)*val on the fly during tileA load into shared memory,
    /// avoiding materialization of the gated intermediate buffer.
    /// </summary>
    internal const string SgemmSiluGateF16 = """
        #version 450
        #extension GL_EXT_shader_explicit_arithmetic_types_float16 : require
        #extension GL_EXT_shader_16bit_storage : require
        #extension GL_EXT_control_flow_attributes : enable
        ...
        void main() {
            ...
            for (uint t = 0u; t < numTiles; t++) {
                ...
                [[unroll]] for (uint p = 0u; p < 2u; p++) {
                    ...
                    uint gm = row_base + r;
                    uint gk = k_base + c4;
                    // A is [M, 2*K]: gate = A[:, :K], val = A[:, K:]; tile gets silu(gate) * val.
                    vec4 v;
                    if (gm < pc.M && gk + 3u < pc.K && vecOk) {
                        vec4 gate = a_vec4[(gm * strideA + gk + pc.aOffset) >> 2];
                        vec4 val  = a_vec4[(gm * strideA + pc.K + gk + pc.aOffset) >> 2];
                        v = gate / (vec4(1.0) + exp(-gate)) * val;
                    } else {
                        [[unroll]] for (uint i = 0u; i < 4u; i++) {
                            float o = 0.0;
                            if (gm < pc.M && gk + i < pc.K) {
                                float g = a_data[gm * strideA + gk + i + pc.aOffset];
                                float vv = a_data[gm * strideA + pc.K + gk + i + pc.aOffset];
                                o = g / (1.0 + exp(-g)) * vv;
                            }
                            v[i] = o;
                        }
    ...
    ///
    /// 32x32 tiled GEMM with 64 threads (8x8) computing 4x4 outputs per thread.
    /// Cooperative loading of tileA[32][17] (im2col on the fly) and tileB[32][17] (weights).
    ///
    /// Push constants: { inCh, outCh, inHeight, inWidth, ksize, padding, stride, outHeight, outWidth }.
    /// Bindings: 0=input, 1=weight, 2=bias, 3=output.
    ...
    internal const string Conv2dImplicitGemm = """
        ...
        void main() {
            ...
            for (uint t = 0u; t < numTiles; t++) {
                ...
                // Load tileB [32, 16] - weights
                [[unroll]] for (uint p = 0u; p < 8u; p++) {
                    uint c = c_base + p;
                    uint gk = k_base + c;
                    tileB[r][c] = (valid_col && gk < K) ? weight_data[gn * K + gk] : 0.0;
                }
    ...
    internal const string GeGlu = """
        ...
        void main() {
            ...
            float arg  = 0.79788456 * (gate + 0.044715 * gate * gate * gate);
            float th   = (arg > 10.0) ? 1.0 : ((arg < -10.0) ? -1.0 : tanh(arg));
            float geluGate = 0.5 * gate * (1.0 + th);
            y[idx] = val * geluGate;
        }
        """;
    ...
    internal const string MultiHeadAttentionTiled40 = """
        ...
        void main() {
            ...
            // Stage Q tile into LDS: BR*HEAD_DIM = 640 floats, 128 threads -> 5 each.
            ...
            float accum0 = 0.0, accum1 = 0.0, accum2 = 0.0, accum3 = 0.0, accum4 = 0.0;
            bool q_valid = (q_row < p.qSeq);

            for (uint tile_base = 0u; tile_base < p.kvSeq; tile_base += BC) {
                // Stage K/V tile: BC*HEAD_DIM = 1280 floats each, 128 threads -> 10 each.
                ...
                if (q_valid) {
                    ...
                    uint d1 = d0 + 1u, d2 = d0 + 2u, d3 = d0 + 3u, d4 = d0 + 4u;
                    ...
                    for (uint k = 0u; k < BC; ++k) {
                        ...
                        float partial = q_tile[qBaseLocal + d0] * k_tile[k * HEAD_DIM + d0]
                                       ...
                                       + q_tile[qBaseLocal + d4] * k_tile[k * HEAD_DIM + d4];
                ...
                if (q_valid) {
                    ...
                    accum0 *= alpha; accum1 *= alpha; accum2 *= alpha; accum3 *= alpha; accum4 *= alpha;
    ...
    /// <summary>
    /// Tiled ("flash-attention"-style) variant of MultiHeadAttention for HEAD_DIM=80 (SD 1.5 Level 1).
    /// </summary>
    internal const string MultiHeadAttentionTiled80 = """
        #version 450
        #extension GL_KHR_shader_subgroup_basic   : require
        #extension GL_KHR_shader_subgroup_shuffle : require
        ...
        void main() {
            ...
            // Stage Q tile into LDS: BR*HEAD_DIM = 1280 floats, 256 threads -> 5 each.
            for (uint i = tid; i < BR * HEAD_DIM; i += WG_SIZE) {
                uint r = i / HEAD_DIM;
                uint d = i % HEAD_DIM;
                uint globalQRow = gl_WorkGroupID.x * BR + r;
            ...
            for (uint tile_base = 0u; tile_base < p.kvSeq; tile_base += BC) {
                // Stage K/V tile: BC*HEAD_DIM = 1280 floats each, 256 threads -> 5 each.
                for (uint i = tid; i < BC * HEAD_DIM; i += WG_SIZE) {
                    uint krow = i / HEAD_DIM;
                    uint d = i % HEAD_DIM;
                    uint globalK = tile_base + krow;
    ...
    /// <summary>
    /// Tiled ("flash-attention"-style) variant of MultiHeadAttention for HEAD_DIM=160 (SD 1.5 Level 2 &amp; Mid).
    /// </summary>
    internal const string MultiHeadAttentionTiled160 = """
        #version 450
        #extension GL_KHR_shader_subgroup_basic   : require
        #extension GL_KHR_shader_subgroup_shuffle : require
        ...
        void main() {
            ...
            for (uint tile_base = 0u; tile_base < p.kvSeq; tile_base += BC) {
                // Stage K/V tile: BC*HEAD_DIM = 2560 floats each, 256 threads -> 10 each.
                for (uint i = tid; i < BC * HEAD_DIM; i += WG_SIZE) {
                    uint krow = i / HEAD_DIM;
                    uint d = i % HEAD_DIM;
                    uint globalK = tile_base + krow;
