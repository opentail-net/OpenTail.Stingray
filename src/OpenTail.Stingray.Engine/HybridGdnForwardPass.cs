...
public sealed unsafe class HybridGdnForwardPass : IForwardPass
{
    ...
    // the STINGRAY_Q3K_Q8K / STINGRAY_Q8_0_Q8K routed-expert dot kernels (allocated
    ...
    private static readonly bool _bypassGdn =
        Environment.GetEnvironmentVariable("STINGRAY_BYPASS_GDN") == "1";
    private static readonly bool _bypassAttn =
        Environment.GetEnvironmentVariable("STINGRAY_BYPASS_ATTN") == "1";
    ...
    // STINGRAY_Q3K_Q8K / STINGRAY_Q8_0_Q8K = "1" or "0" override. Mirrors the
        ...
        int maxContextLength = 0)
    {
        ...
        if (hp.IsMoE && (_q3kQ8KEnabled || _q8_0Q8KEnabled))
        {
            ...
            Console.Error.WriteLine(
                $"[HybridGdnForwardPass] Routed-MoE Q8_K-input kernels enabled: {string.Join(", ", enabled)}. Override with STINGRAY_Q3K_Q8K=0 / STINGRAY_Q8_0_Q8K=0.");
        }
    ...
    /// <c>STINGRAY_DISABLE_BATCH_VERIFY=1</c> env var forces the legacy sequential
    ...
    public float[][] BatchVerify(int[] tokens, int startPos)
    {
        ...
        if (k > MaxBatchVerifyTokens)
            throw new ArgumentOutOfRangeException(nameof(tokens), k,
                ...
                "raise STINGRAY_MTP_BATCH_MAX or shorten the draft chain.");
        if (k == 1)
        {
    ...
    // ============================================================
    //  Trace helpers (STINGRAY_TRACE_LAYERS=1)
    // ============================================================
    ...
    /// <summary>
    /// Diagnostic only (STINGRAY_PROBE_LOGITS=1). Projects the residual at
    ...
    private void ProbeResidual(int position, int layer)
    {
        ...
        var pos0Env = Environment.GetEnvironmentVariable("STINGRAY_PROBE_POS");
    ...
    private void Attention(int layer, int position)
    {
        ...
        Parallel.For(0, _numHeads, h =>
        {
            ...
            for (int t = 0; t < seqLen; t++)
            {
                float* vVec = cache.ValueAtHead(layer, t, kvHead);
    ...
    private void MtpAttnBlock(int position)
    {
        ...
        Parallel.For(0, _numHeads, h =>
        {
            ...
            for (int t = 0; t < seqLen; t++)
            {
                float* vVec = mtpCache.ValueAtHead(layer: 0, position: t, kvHead: kvHead);
                float w = headScores[t];
        ...
        float* hiddenOutExt = null)
    {
        ...
        // STINGRAY_Q3K_Q8K=1 / STINGRAY_Q8_0_Q8K=1: prepack the Phase-A input as
    ...
    /// to the <c>STINGRAY_PREFAULT=0</c> kill switch).
    /// </summary>
