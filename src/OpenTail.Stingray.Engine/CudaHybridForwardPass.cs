...
public sealed unsafe class CudaHybridForwardPass : IForwardPass
{
    ...
    // resolved from STINGRAY_KV_DTYPE. Applies to the GPU-trunk layers only — CPU-offloaded layers
    ...
    // before the GPU upload loop. STINGRAY_CPU_MOE=0|1 overrides. Always false for non-MoE.
    ...
    // STINGRAY_HYBRID_PREFILL_COMPUTE=0 reverts to the byte-exact per-token matvec.
    internal static bool HybridPrefillComputeEnabled =
        Environment.GetEnvironmentVariable("STINGRAY_HYBRID_PREFILL_COMPUTE") != "0";
    ...
    // STINGRAY_Q3K_Q8K=0 / STINGRAY_Q8_0_Q8K=0 / STINGRAY_Q4K_Q8K=0 for byte parity with
    ...
    // where the int8 tier auto-enables. Kill-switch: STINGRAY_HYBRID_BATCHED_MOE=0.
    internal static bool BatchedCpuMoePrefillEnabled =
        CudaHybridGdnForwardPass.ResolveGate("STINGRAY_HYBRID_BATCHED_MOE", true);
    ...
    internal static bool ForceCpuResidentEmbedding =
        Environment.GetEnvironmentVariable("STINGRAY_FORCE_CPU_EMBED") == "1";
    ...
    // AVX-512 hardware where the f32 AVX-512 dot wins (STINGRAY_Q3K_Q8K / STINGRAY_Q8_0_Q8K /
    // STINGRAY_Q4K_Q8K override). NOT byte-exact vs the f32 dots — the bitwise oracles pin
    ...
    // of the per-row loop whenever a routed down weight is Q6_K, independent of the STINGRAY_*
        ...
        int expertSlotCapacity = -1)
    {
        ...
        // KV-cache dtype (#230): honor STINGRAY_KV_DTYPE on this layer-split hybrid path — it was
        ...
        if (_kvDType == DType.Q8_0 && !CudaForwardPass.Q8KvGeometrySupported(hp, _nGpuLayers))
            throw new NotSupportedException(
                "STINGRAY_KV_DTYPE=q8_0 requires every GPU-resident layer's kvDim (kvHeads × headDim) to be a " +
        ...
        if (_isMoE)
        {
            ...
            else
            {
                ...
                Console.Error.WriteLine(
                    $"[CudaHybridForwardPass] MoE auto-select: expert-cache capacity ≈ {predictedSlots}/{totalExperts} ({ratio:P0}) → {(_cpuMoe ? "CPU" : "GPU SLRU")} MoE.  Override with STINGRAY_CPU_MOE=0|1.");
            }
        }
        else
        {
        ...
        if (_cpuMoe)
        {
            _q3kQ8KEnabled = CudaHybridGdnForwardPass.ResolveGate("STINGRAY_Q3K_Q8K",
                CudaHybridGdnForwardPass.HasRoutedExpertsOfDType(model, hp, DType.Q3_K));
            _q8_0Q8KEnabled = CudaHybridGdnForwardPass.ResolveGate("STINGRAY_Q8_0_Q8K",
                CudaHybridGdnForwardPass.HasRoutedExpertsOfDType(model, hp, DType.Q8_0));
            _q4kQ8KEnabled = CudaHybridGdnForwardPass.ResolveGate("STINGRAY_Q4K_Q8K",
            ...
            if (_q3kQ8KEnabled || _q8_0Q8KEnabled || _q4kQ8KEnabled)
            {
                ...
                Console.Error.WriteLine(
                    ...
                    "Override with STINGRAY_Q3K_Q8K=0 / STINGRAY_Q8_0_Q8K=0 / STINGRAY_Q4K_Q8K=0.");
            }
        ...
        // cold-start cost. MmapPrefault filters empty configs and honours STINGRAY_PREFAULT.
    ...
    // Shared with the GDN pass (STINGRAY_MOE_THREADS-resolved ParallelOptions), same as
    ...
    // for every dtype; pin STINGRAY_Q8_0_Q8K=0 for byte parity). The per-row byte
    ...
    private void BatchedRoutedExpertsCpu(int layer, int N, float* normAll, float* routedAll)
    {
        ...
        // Engages whenever the down dtype is Q6_K, independent of the STINGRAY_*_Q8K gates.
    ...
    /// argmax-stable rather than bitwise; the parity oracles pin STINGRAY_SPLIT_DECODE=0.</para>
    ///
    ...
    // STINGRAY_CUDA_GRAPH=1 or the UseCudaGraph setter. Falls back to direct launches on
    ...
    private bool _useCudaGraph =
        Environment.GetEnvironmentVariable("STINGRAY_CUDA_GRAPH") == "1";
    private bool _graphCaptured;

    /// <summary>
    /// Enable/disable CUDA-graph capture+replay for the Gemma 4 GPU layer loop. Defaults
    /// from the <c>STINGRAY_CUDA_GRAPH</c> env var; set before the first decode (tests/bench).
    /// </summary>
    ...
    public void Dispose()
    {
        ...
        // (InferenceEngineLoader adds it to owned[] when constructing the CUDA hybrid/partial-
        // offload path) — without this guard every _gpu.Free() call below runs twice, a
        // double-free that corrupts the native/VRAM heap).
        if (_disposed) return;
        _disposed = true;
