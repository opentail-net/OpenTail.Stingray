...
public interface IInferenceEngine
{
    ...
    /// <summary>Whether this engine supports batched MoE expert routing across concurrent sequences.</summary>
    bool IsMoeSupported => false;

    /// <summary>Whether this engine supports batched TurboQuant KVarN compressed KV caches.</summary>
    bool IsTurboQuantBatchedSupported => false;
...
/// <summary>
/// Optional, read-only operational counters exposed by an engine that performs continuous
/// batching. Server hosts can use this without making every inference-engine implementation
/// carry batching-specific state.
/// </summary>
public interface IContinuousBatchingObservability
{
    /// <summary>Whether the wrapped engine is using continuous batching.</summary>
    bool IsContinuousBatching { get; }
    int PrefillChunkTokens { get; }
    long KvTokenBudget { get; }
    long CommittedKvTokens { get; }
    long PrefixCacheBudgetBytes { get; }
    long PrefixCacheUsedBytes { get; }
    int PrefixCacheEntries { get; }
    long PrefixCacheHits { get; }
    long PrefixCacheMisses { get; }
    long PrefixCacheEvictions { get; }
    long BatchedArgmaxSteps { get; }
    long BatchedFullLogitsSteps { get; }
    long BatchedArgmaxSequences { get; }
    long BatchedFullLogitsSequences { get; }
}
