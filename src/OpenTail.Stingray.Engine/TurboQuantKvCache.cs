...
public sealed unsafe class TurboQuantKvCache : IDisposable
{
    ...
    /// independently, so this can exceed <see cref="TqLength"/> (layer 0's view) mid-prefill.
    /// Callers gating a rewind must use this, not <see cref="TqLength"/>.
    /// </summary>
    ...
    public int FastScanTileSize => OpenTail.Stingray.TurboQuant.FastScan.TileSize;
        ...
        TqQuantizer quantizer = TqQuantizer.LloydMax)
    {
        ...
        else
        {
            ...
            _tileBytes = OpenTail.Stingray.TurboQuant.FastScan.TileBytes(headDim);
            _vTileBytes = OpenTail.Stingray.TurboQuant.FastScan.VTileBytes(headDim);
            _stagingBytesPerHead = OpenTail.Stingray.TurboQuant.FastScan.TileSize * _tqBlockSize;
            _tileTokens = OpenTail.Stingray.TurboQuant.FastScan.TileSize;
        ...
        float* scoresOut)
    {
        ...
        int numFullTiles = totalTq / OpenTail.Stingray.TurboQuant.FastScan.TileSize;
        int stagingCount = totalTq % OpenTail.Stingray.TurboQuant.FastScan.TileSize;

        var centroids = OpenTail.Stingray.TurboQuant.TurboQuantCodebooks.GetCentroids(_bits, hd);
        ...
        float lutScale = _bits == 4
            ? OpenTail.Stingray.TurboQuant.FastScan.BuildLut4Bit(
                new ReadOnlySpan<float>(rotatedQuery, hd), centroids, lut, hd)
            : OpenTail.Stingray.TurboQuant.FastScan.BuildLut3Bit(
                new ReadOnlySpan<float>(rotatedQuery, hd), centroids, lut, hd);
        ...
        fixed (sbyte* lutPtr = lut)
        {
            for (int tileIdx = 0; tileIdx < numFullTiles; tileIdx++)
            {
                ...
                OpenTail.Stingray.TurboQuant.FastScan.KScoreTile4BitAvx2(
                    tile, lutPtr, lutScale, attnScale,
                    scoresOut + (long)tileIdx * OpenTail.Stingray.TurboQuant.FastScan.TileSize,
                    hd);
            }
        }
        ...
        if (stagingCount > 0)
        {
            ...
            int tailStart = numFullTiles * OpenTail.Stingray.TurboQuant.FastScan.TileSize;
        ...
        float* outAcc)
    {
        ...
        Span<float> effectiveW = stackalloc float[OpenTail.Stingray.TurboQuant.FastScan.TileSize];
        Span<sbyte> vLut = stackalloc sbyte[OpenTail.Stingray.TurboQuant.FastScan.TileSize * 16];
        ...
        fixed (sbyte* vLutPtr = vLut)
        {
            for (int tileIdx = 0; tileIdx < numFullTiles; tileIdx++)
            {
                ...
                int baseT = tileIdx * OpenTail.Stingray.TurboQuant.FastScan.TileSize;
                for (int t = 0; t < OpenTail.Stingray.TurboQuant.FastScan.TileSize; t++)
                {
                ...
                float vScale = _bits == 4
                    ? OpenTail.Stingray.TurboQuant.FastScan.BuildVLut4Bit(effectiveW, centroids, vLut)
                    : OpenTail.Stingray.TurboQuant.FastScan.BuildVLut3Bit(effectiveW, centroids, vLut);

                OpenTail.Stingray.TurboQuant.FastScan.VAggregateTile4BitAvx2(
        ...
        OpenTail.Stingray.TurboQuant.WalshHadamard.Transform(rotatedAcc, rotatedAcc, hd);
    ...
    private void CompressOldestFp32(int layer)
    {
        ...
        int stagingSlot = _layerTqLengths[layer] % OpenTail.Stingray.TurboQuant.FastScan.TileSize;
        ...
        if (_layerTqLengths[layer] % OpenTail.Stingray.TurboQuant.FastScan.TileSize == 0)
        {
            int tileIdx = _layerTqLengths[layer] / OpenTail.Stingray.TurboQuant.FastScan.TileSize - 1;
            for (int head = 0; head < _numKvHeads; head++)
            {
                ...
                if (_bits == 4)
                {
                    OpenTail.Stingray.TurboQuant.FastScan.PackTile4Bit(keyStageSpan, keyTileSpan, _headDim);
                    OpenTail.Stingray.TurboQuant.FastScan.PackVTile4Bit(valStageSpan, valTileSpan, _headDim);
                }
                else
                {
                    OpenTail.Stingray.TurboQuant.FastScan.PackTile3Bit(keyStageSpan, keyTileSpan, _headDim);
                    OpenTail.Stingray.TurboQuant.FastScan.PackVTile3Bit(valStageSpan, valTileSpan, _headDim);
                }
            }
        }
    ...
    public void Compact(int[] keep, int N)
    {
        ...
        if (_quantizer == TqQuantizer.KVarN)
            throw new NotSupportedException(
                ...
                "Disable SnapKV (unset STINGRAY_SNAPKV_BUDGET) or use the Lloyd-Max quantizer.");
        ...
        int tileSize = OpenTail.Stingray.TurboQuant.FastScan.TileSize;
        int maxTqPositions = _maxSeqLen - _fp32WindowSize;
        if (maxTqPositions < 0) maxTqPositions = 0;
    ...
    private void UnpackTileSlotKey(int layer, int tileIdx, int kvHead, int slot, Span<byte> blockOut)
    {
        ...
        byte* codes = tile + OpenTail.Stingray.TurboQuant.FastScan.NormBytesPerTile;
        int byteInRow = slot & 15;
        bool highNibble = slot >= 16;

        for (int d = 0; d < _headDim; d++)
        {
            byte pair = codes[d * OpenTail.Stingray.TurboQuant.FastScan.CodeBytesPerDim + byteInRow];
