...
public sealed unsafe class CudaForwardPass : IForwardPass, IBatchedForwardPass, IMultiSlotKvCache
{
    ...
    // bf16. Set via STINGRAY_KV_DTYPE=bf16. Mirrors CudaHybridGdnForwardPass (#27).
    ...
    /// <c>STINGRAY_BATCHED_PREFILL</c> (default on); settable so tests can A/B the
    ...
    /// <c>STINGRAY_KVARN_BATCHED_PREFILL</c> (default on; <c>=0</c> reverts to the per-token
    ...
    // when STINGRAY_KV_DTYPE=bf16. Arithmetic is identical to the fp32 kernels; only
    ...
    // STINGRAY_KV_DTYPE — issue #179. F32 (default on the dense path; bf16 is opt-in
    ...
    private static DType ParseKvDType(string? envValue) => envValue?.Trim().ToLowerInvariant() switch
    {
        ...
        var other     => throw new ArgumentException(
            $"STINGRAY_KV_DTYPE must be 'fp32', 'bf16', or 'q8_0' (got '{other}').", nameof(envValue)),
    };

    /// <summary>
    /// Resolves the configured KV-cache dtype from the STINGRAY_KV_DTYPE environment variable
    ...
    /// <c>STINGRAY_KV_DTYPE</c> is unset — i.e. it distinguishes "the user asked for fp32" from
    /// "the user asked for nothing".
    ///
    /// <para>That distinction is what lets a backend pick its own default and then FALL BACK
    /// rather than throw when the default cannot be honoured (TurboQuant owns the KV quantization,
    /// an explicit SnapKV budget, an odd head dim). An explicit request must still fail loudly —
    /// silently ignoring <c>--kv-type bf16</c> would be worse than an error. Same parser, so an
    /// invalid value still throws here.</para>
    /// </summary>
    ...
    /// (<c>STINGRAY_PREFILL_GEMM</c>); settable so tests can A/B the GEMM path
    ...
    /// (<c>STINGRAY_PREFILL_MMQ</c>=0 reverts to the GEMM path). Argmax-stable, not
    ...
    /// Default on (<c>STINGRAY_PREFILL_FLASH</c>=0 reverts to the scalar kernels).
    /// </summary>
    ...
    /// single-warp #146 fallback +5%. Default on (<c>STINGRAY_PREFILL_FLASH_TC</c>=0 reverts
    ...
    // STINGRAY_BATCH_DECODE_WS=0 falls back to the #190 GEMM-N matvec (_gpu.MatMulBatched, grid
    // Y = nTok: launch-overhead + L2 reuse only — the A/B baseline WS replaced).
    // STINGRAY_BATCH_DECODE_GEMM=1 routes the compute-bound GEMM/MMQ path (GpuMatMulBatchedCore)
    ...
    private readonly bool _batchDecodeComputeBound =
        Environment.GetEnvironmentVariable("STINGRAY_BATCH_DECODE_GEMM") == "1";
    private readonly bool _batchDecodeWeightStationary =
        Environment.GetEnvironmentVariable("STINGRAY_BATCH_DECODE_WS") != "0";
    // STINGRAY_BATCH_DECODE_MMQ (#201/#206): int8 tensor-core decode matmuls for the big
    ...
    private readonly bool _batchDecodeMmqKill =
        Environment.GetEnvironmentVariable("STINGRAY_BATCH_DECODE_MMQ") == "0";
    private readonly bool _batchDecodeMmqForceAll =
        Environment.GetEnvironmentVariable("STINGRAY_BATCH_DECODE_MMQ") == "1";
    ...
    // STINGRAY_BATCH_DECODE_RAGGED=0 restores the #190 per-sequence loop. Not readonly only so the
    ...
    private bool _batchDecodeRagged =
        Environment.GetEnvironmentVariable("STINGRAY_BATCH_DECODE_RAGGED") != "0";
    ...
    // STINGRAY_SPLIT_DECODE=0 reverts to the single-block-per-head decode attention (the A/B
    // baseline; lets parity tests compare the split path against it). Default on.
    private readonly bool _splitDecodeEnabled =
        Environment.GetEnvironmentVariable("STINGRAY_SPLIT_DECODE") != "0";
    ...
    // STINGRAY_SPLIT_DECODE_GROUPED: "0" forces per-head, "1" forces grouped whenever eligible
    ...
    private readonly string? _splitGroupedMode =
        Environment.GetEnvironmentVariable("STINGRAY_SPLIT_DECODE_GROUPED");
        ...
        TqQuantizer tqQuantizer = TqQuantizer.LloydMax)
    {
        ...
        // (STINGRAY_MMQ_SOA=0 reverts).
        _mmqSoa = mmqSoa ?? (Environment.GetEnvironmentVariable("STINGRAY_MMQ_SOA") != "0");
        ...
        // bit-identical (Qwen3-8B 70.0 → 74.7 t/s). Default on (STINGRAY_Q4K_SOA=0
        // reverts). Dense-only: the MoE Q4_K readers are not SoA-converted, so the
        // repack is gated on !_isMoE at upload.
        _q4kSoa = Environment.GetEnvironmentVariable("STINGRAY_Q4K_SOA") != "0";
        ...
        // Default on; STINGRAY_Q6K_SOA=0 keeps the AoS weight everywhere and disables the decode
        // MMQ. Dense-only: the MoE Q6_K readers are not SoA-converted, so the repack is gated on
        // !_isMoE at upload.
        _q6kSoa = Environment.GetEnvironmentVariable("STINGRAY_Q6K_SOA") != "0";
        ...
        // Batched-trunk prefill is on by default; STINGRAY_BATCHED_PREFILL=0 forces the
        // bit-exact per-token loop (useful for A/B and parity debugging).
        BatchedPrefillEnabled =
            Environment.GetEnvironmentVariable("STINGRAY_BATCHED_PREFILL") != "0";
        // Chunked batched KVarN prefill (issue #180 Task 6) is on by default;
        // STINGRAY_KVARN_BATCHED_PREFILL=0 reverts KVarN prompts to the per-token loop.
        KvarnBatchedPrefillEnabled =
            Environment.GetEnvironmentVariable("STINGRAY_KVARN_BATCHED_PREFILL") != "0";
        PrefillGemmEnabled =
            Environment.GetEnvironmentVariable("STINGRAY_PREFILL_GEMM") != "0";
        ...
        // prefill) and drops the fp16 weight HBM temp. STINGRAY_PREFILL_MMQ=0 reverts to
        // the cuBLAS GEMM path. Gated under PrefillGemmEnabled (the compute-bound path).
        PrefillMmqEnabled =
            Environment.GetEnvironmentVariable("STINGRAY_PREFILL_MMQ") != "0";
        ...
        // ~1389→~2180 t/s. STINGRAY_PREFILL_FLASH=0 reverts to the scalar kernels.
        PrefillFlashAttnEnabled =
            Environment.GetEnvironmentVariable("STINGRAY_PREFILL_FLASH") != "0";
        // Issues #146/#147: tensor-core flash prefill — default on (the #147 multi-warp
        // kernel is +27-40% over half2 on Gemma 4 at d=512). STINGRAY_PREFILL_FLASH_TC=0
        // reverts to the half2 kernel.
        PrefillFlashTcEnabled =
            Environment.GetEnvironmentVariable("STINGRAY_PREFILL_FLASH_TC") != "0"
            && (_headDim & 15) == 0;
        // Issue #147 A/B: force the single-warp TC kernel even where #147 would apply.
        _forceFlashTc1 =
            Environment.GetEnvironmentVariable("STINGRAY_PREFILL_FLASH_TC1") == "1";
        ...
        // call). Note STINGRAY_SNAPKV_BUDGET=0 means "disable SnapKV" (IsBudgetExplicit=true,
        ...
        if (kvNarrowed && _tqEnabled)
            throw new NotSupportedException(
                $"STINGRAY_KV_DTYPE={_kvDType} + TurboQuant is not supported (TQ owns the KV " +
        ...
        if (kvNarrowed && _snapKvCfg.IsBudgetExplicit && _snapKvCfg.Budget > 0)
            throw new NotSupportedException(
                $"STINGRAY_KV_DTYPE={_kvDType} + SnapKV is not yet implemented (narrowed-KV " +
                "physical compaction kernel not wired on the dense path; issue #179). " +
                "Set STINGRAY_SNAPKV_BUDGET=0 to disable SnapKV.");
        ...
        else if (MultiSlotPrefixRequested())
        {
            ...
            // then see budget==0 and engage. An explicit STINGRAY_SNAPKV_BUDGET>0 still wins above
            ...
            Console.Error.WriteLine(
                ...
                "(STINGRAY_PREFIX_SLOTS=2, issue #212). Set an explicit STINGRAY_SNAPKV_BUDGET>0 to " +
        ...
        else if (preferBatchingOverAutoSnapKv)
        {
            ...
            // per-sequence eviction with an explicit STINGRAY_SNAPKV_BUDGET>0 (handled above; it
            ...
            Console.Error.WriteLine(
                ...
                "enabled (MaxBatchSize>1, issue #196). Set STINGRAY_SNAPKV_BUDGET>0 for per-sequence " +
        ...
        bool vramTrace = Environment.GetEnvironmentVariable("STINGRAY_TRACE_VRAM") == "1";
        ...
        {
            // Per-layer KV-cache sizing for gemma4 (LayerHeadDim != null): each layer's K/V
            // buffer takes its own head_dim, SWA layers cap context at SlidingWindowSize, and
            // KV-share layers (KvSourceLayer[L] >= 0) alias the source layer's handle. Dense
            // models get full-context K/V at the model-wide head_dim. Shared with CreateCache
            // via FillKvCacheArrays so the per-sequence batched caches match this layout
            // exactly. Fills the owned arrays in place (identity preserved → _ownedKCache, set
            // above, stays valid).
            foreach (int aliasedLayer in FillKvCacheArrays(_gpuKCache, _gpuVCache))
                _kvAliasedLayers.Add(aliasedLayer);
        }
        ...
        if (hp.HasPerLayerTokenEmbd)
        {
            ...
            _pleGpuDequant = (_pleDType == DType.Q8_0 || _pleDType == DType.Q6_K)
                          ...
                          && Environment.GetEnvironmentVariable("STINGRAY_PLE_GPU_DEQUANT") != "0";
        ...
        if (Environment.GetEnvironmentVariable("STINGRAY_CUDA_MATVEC_BENCH") == "1")
            BenchMatVec();
    }
    ...
    // Profiling state (only used when STINGRAY_CUDA_PROFILE is set).
    private static readonly bool s_profile =
        Environment.GetEnvironmentVariable("STINGRAY_CUDA_PROFILE") == "1";
    private static readonly bool s_prefillProfile =
        Environment.GetEnvironmentVariable("STINGRAY_PREFILL_PROFILE") == "1";
    ...
    // earlier short-context regression that kept #136 default-off is gone). STINGRAY_CUDA_GRAPH=0
    // reverts to direct launches.
    private bool _useCudaGraph =
        Environment.GetEnvironmentVariable("STINGRAY_CUDA_GRAPH") != "0";
    ...
    /// the <c>STINGRAY_CUDA_GRAPH</c> env var; set before the first decode (tests/bench).
    /// </summary>
    ...
    /// <inheritdoc/>
    // #219: only the non-profiled decode paths share FinishLogits; the STINGRAY_CUDA_PROFILE /
    // STINGRAY_DECODE_REGIONS variants keep the full download so their phase timings stay meaningful.
    ...
    // Region profiler (STINGRAY_DECODE_REGIONS=1): splits the graphs-ON decode token into
    ...
    // overhead but reveal where the per-token time goes (the STINGRAY_CUDA_PROFILE path runs
    // graphs-off, which inflates the launch-bound phases). Prints every 64 tokens.
    private static readonly bool s_regionProfile =
        Environment.GetEnvironmentVariable("STINGRAY_DECODE_REGIONS") == "1";
    ...
    private ReadOnlySpan<float> ForwardProfiled(int token, int position)
    {
        if (_tqEnabled)
            throw new NotSupportedException(
                "STINGRAY_CUDA_PROFILE per-phase profiling is not wired for the TurboQuant path. " +
    ...
    /// ffn / final so STINGRAY_CUDA_PROFILE=1 can decompose Gemma 4 kernel time.
        ...
        bool isSwa, int window, int appendCtx, int effLayerCtx)
    {
        ...
        else if (PrefillFlashTcEnabled && (layerHd & 15) == 0)
        {
            ...
            // #146 single-warp kernel. STINGRAY_PREFILL_FLASH_TC1=1 forces single-warp (A/B).
    ...
    /// per-token eviction the packed trunk doesn't run. Both fall back to the sequential
    /// <c>PrefillPackedMulti</c> <see cref="PrefillWithCache"/> loop — which IS SnapKV-aware — instead
    ...
    /// matvec, so a Q4_0 trunk/output weight would throw at the first batched decode step. Since
    /// #195 admits Gemma 4, a Q4_0 model (e.g. the Gemma 4 12B QAT) is kept off the batched path
    /// by THIS dtype check (its Q4_0 weights fail <see cref="DecodeBatchable"/>), not by the arch
    /// gate — note <see cref="RunBatchedTrunkGemma4"/> uses <see cref="GpuMatMulBatched"/>, which
    /// would happily route Q4_0, so the dtype gate is what keeps Q4_0 Gemma 4 in the single-user
    /// fallback.</summary>
    ...
    /// <summary>
    /// Whether the Gemma-4 batched decode (issue #195) can run this model: a Gemma-4-style
    /// transformer (per-layer head_dim) with every trunk + output + PLE weight in a GEMM-N-
    /// batchable dtype. The batched decode (<see cref="RunBatchedTrunkGemma4"/>) handles the
    /// per-layer head_dim / KV-head count, SWA-vs-global attention dispatch, shared-KV aliasing,
    /// k_eq_v, the PLE pre-pass + injection, sandwich post-norms, layer_output_scale, and the
    /// final-logit softcap — so unlike the dense gate this does NOT exclude softcap. TurboQuant
    /// stays out (the owned-cache ring state machine). SnapKV is applied by the caller.
    /// </summary>
    private bool Gemma4BatchedDecodeSupported()
    {
        if (!_isGemma4Like || _isMoE || _tqEnabled) return false;
        for (int i = 0; i < _hp.NumLayers; i++)
        {
            bool kvShared = _hp.KvSourceLayer is { } ksl && ksl[i] >= 0;
            if (!DecodeBatchable(_wq[i]) || !DecodeBatchable(_wo[i]) ||
                !DecodeBatchable(_wGate[i]) || !DecodeBatchable(_wUp[i]) || !DecodeBatchable(_wDown[i]))
                return false;
            if (!kvShared)
            {
                if (!DecodeBatchable(_wk[i])) return false;
                // k_eq_v global layers carry no attn_v (V = raw K projection); every other
                // KV-owning layer needs a batchable V. A null _wv on a non-k_eq_v model is
                // unexpected → not batchable.
                if (_wv[i] is { } wv) { if (!DecodeBatchable(wv)) return false; }
                else if (!_hp.AttentionKEqV) return false;
            }
            if (_hp.HasPerLayerTokenEmbd &&
                (!DecodeBatchable(_gpuInpGate![i]) || !DecodeBatchable(_gpuPleProj![i])))
                return false;
        }
        if (_hp.HasPerLayerTokenEmbd && !DecodeBatchable(_gpuPerLayerModelProj!))
            return false;
        return DecodeBatchable(_wOutput);
    }
    ...
    private void ThrowIfBatchingUnsupported(bool decodeOnly = false)
    {
        ...
        if (!DenseBatchedDecodeSupported() && !Gemma4BatchedDecodeSupported())
            throw new NotSupportedException(
                "CUDA continuous batching requires a dense or Gemma-4 model with every trunk + output " +
                "weight in a GEMM-N-batchable dtype (Q4_K/Q5_K/Q6_K/Q8_0/F32); a dense final-logit " +
                "softcap or a Q4_0 weight has no batched-decode path.");
    }

    /// <summary>
    /// Fill caller-provided per-layer K/V arrays with one cache's tensors, using the SAME
    /// per-layer geometry the owned cache uses: per-layer head_dim + KV-head count, SWA layers
    /// ring-sized (issue #162), and Gemma-4 shared-KV tail layers aliasing their source layer's
    /// tensor (returned in the aliased set so dispose skips the double-free). The single source
    /// of truth for KV-cache layout, shared by the constructor's owned-cache allocation and
    /// <see cref="CreateCache"/> (issue #195: Gemma 4 batching), so a per-sequence cache the
    /// batched decode / PrefillWithCache binds is byte-identical to the owned one. Frees its own
    /// partial (non-aliased) allocations on an OOM mid-loop and rethrows. Fills <paramref
    /// name="k"/>/<paramref name="v"/> in place (their identity is preserved — the constructor
    /// relies on this so <c>_ownedKCache</c> stays valid).
    /// </summary>
    private HashSet<int> FillKvCacheArrays(Tensor[] k, Tensor[] v)
    {
        int L = _hp.NumLayers;
        var aliased = new HashSet<int>();
        bool perLayerKv = _hp.LayerHeadDim is not null;
        int swaWindow = _hp.SlidingWindowSize > 0 ? _hp.SlidingWindowSize : _maxSeqLen;
        try
        {
            for (int i = 0; i < L; i++)
            {
                int kvSrc = _hp.KvSourceLayer is { } ksl ? ksl[i] : -1;
                if (kvSrc >= 0)
                {
                    // Alias the source layer's K/V handles (Gemma 4 shared-KV tail). The source
                    // is allocated first (the GGUF puts shared layers at the tail), so k[kvSrc]
                    // is already set. Never freed here — the source owns it. Fail loud on a
                    // forward reference (a malformed GGUF where the source isn't allocated yet)
                    // rather than aliasing a default(Tensor) and reading garbage at attention.
                    if (kvSrc >= i)
                        throw new NotSupportedException(
                ...
                int layerHd = perLayerKv ? _hp.LayerHeadDim![i] : _headDim;
                int layerKvHeads = _hp.LayerKvHeads is { } lkv ? lkv[i] : _numKvHeads;
                int layerKvDim = layerKvHeads * layerHd;
                int layerCtx = (perLayerKv && _hp.IsSwaLayer is { } swa && swa[i])
                    ? SwaRingSize(_maxSeqLen, swaWindow)
                    : _maxSeqLen;
                // q8_0 KV packs 32 elements/block; the store kernels' per-warp amax reduction
                // (and DTypeInfo.ByteSize's count/32 sizing) assume each layer's kvDim is a
                // multiple of 32 so blocks never straddle a KV row. Fail loud rather than
                // under-allocate + corrupt (#179).
                if (_kvDType == DType.Q8_0 && (layerKvDim & 31) != 0)
                    throw new NotSupportedException(
                        $"STINGRAY_KV_DTYPE=q8_0 requires every layer's kvDim to be a multiple of 32 " +
                        $"(block_q8_0 = 32 elements/block); layer {i} has kvDim={layerKvDim}. " +
                        "Use --kv-type bf16 or fp32 for this model.");
                k[i] = _gpu.Allocate(TensorShape.D1((long)layerCtx * layerKvDim), _kvDType);
                v[i] = _gpu.Allocate(TensorShape.D1((long)layerCtx * layerKvDim), _kvDType);
            }
        }
        catch
        {
            for (int i = 0; i < L; i++)
            {
                if (aliased.Contains(i)) continue; // source owns these handles
    ...
    /// <summary>
    /// Allocate a fresh, empty per-sequence GPU KV cache with the same per-layer geometry as the
    /// owned cache (dense: NumLayers full-context K/V at the model-wide head_dim; Gemma 4:
    /// per-layer head_dim + SWA ring sizing + shared-KV aliasing — issue #195). The active KV
    /// dtype (#179) applies throughout. OOM mid-loop frees the partial (non-aliased) allocations
    /// so the engine's AdmitPending catch doesn't strand VRAM no cache owns.
    /// </summary>
    internal CudaSequenceKvCache CreateCache()
    {
        ...
        var k = new Tensor[L];
        var v = new Tensor[L];
        var aliased = FillKvCacheArrays(k, v);
        return new CudaSequenceKvCache(_gpu, k, v, aliased.Count == 0 ? s_noAliasedLayers : aliased);
    }
    ...
    /// just not cross-prompt amortized) when the dense packed trunk can't run this model
    /// (<see cref="IsDensePackablePrefill"/> — e.g. Gemma 4, which batches its decode (#195) but
    /// keeps the Gemma-4-capable single-sequence batched trunk for prefill; or attn bias / L2
    /// QK-norm / non-NEOX RoPE), when any sub-sequence's attention range exceeds the kernel cap
    ...
    public ISequenceKvCache AllocateScratchSlot(int capacityTokens)
    {
        ...
        if (_kvDType == DType.Q8_0 && (kvDim & 31) != 0)
            throw new NotSupportedException(
                $"STINGRAY_KV_DTYPE=q8_0 requires kvDim % 32 == 0 (block_q8_0 = 32 elements/block); kvDim={kvDim}.");
    ...
    /// STINGRAY_BATCH_DECODE_WS=0: the #190 GEMM-N matvec. The #201/#206 int8 tensor-core
    ...
    /// bit-exact spec-verify path; STINGRAY_BATCH_DECODE_MMQ=0/1 forces it off/on everywhere.
    /// STINGRAY_BATCH_DECODE_GEMM=1: the compute-bound GEMM/MMQ path (the same routing
    ...
    /// the bit-exact WS path so greedy spec-decode output is byte-stable. STINGRAY_BATCH_DECODE_MMQ
                                 ...
                                 bool allowDecodeMmq)
    {
        ...
        // per-sequence loop (STINGRAY_BATCH_DECODE_RAGGED=0) needs them; the ragged kernels
        ...
        try
        {
            ...
            for (int layer = 0; layer < _hp.NumLayers; layer++)
            {
                ...
                // spec-verify path forces WS. STINGRAY_BATCH_DECODE_WS=0 / _GEMM=1 / _MMQ=0|1
                ...
                // Per-sequence (#190, STINGRAY_BATCH_DECODE_RAGGED=0; also forced when any cache is
    ...
    /// <summary>
    /// Gemma 4 batched decode (issue #195): the N-sequence analogue of the single-token
    /// <see cref="RunGemma4DeviceRegion"/>, structured like the batched-trunk prefill
    /// (<see cref="GpuLayerBatchedTrunk"/>) but with one decode token per sequence at its own
    /// position against its own per-sequence cache. Batched embed (+ EmbeddingScale) → batched
                                       ...
                                       bool allowDecodeMmq)
    {
        ...
        // 1. Embed each sequence's token, then the Gemma embedding scale.
        for (int n = 0; n < N; n++)
        {
            EmbedTokenGpu(tokens[n]);   // writes _hidden
            _gpu.CopyDeviceRegion(_bpHidden!, (long)n * embDim * sizeof(float),
        ...
        // 2. PLE pre-pass batched (builds _bpProjAll = [N × L*pleWidth] from the scaled hidden).
        if (_hp.HasPerLayerTokenEmbd)
            BuildPerLayerProjectionsBatched(tokens);
    ...
    /// <summary>
    /// One transformer layer of the Gemma 4 batched decode (issue #195). Mirrors the batched
    /// prefill layer <see cref="GpuLayerBatchedTrunk"/> for the batched parts (RmsNorm, QKV,
    /// QK-norm + V-norm, O-proj, sandwich norms, FFN, PLE injection, layer_output_scale — all
    /// position-independent or batched across N), but replaces the single batched attention with
    /// a per-sequence block: RoPE at positions[n] → KV-append into caches[n] → SWA-or-global
                                             ...
                                             bool allowDecodeMmq)
    {
        ...
        // Per-sequence: RoPE at positions[n] → KV-append into caches[n] → single-query attention.
        // Mirrors RunGemma4DeviceRegion's attention block on each sequence's slice.
        for (int n = 0; n < N; n++)
        {
            int pos = positions[n];
            ...
            try
            {
                ...
                if (isSwa)
                    AttentionSwaKv(qv, kc, vc, av, _attnScoresScratch,
                        pos, window, layerHd, _numHeads, layerKv, effLayerCtx, attnScale: _attnScale);
                else
                    AttentionKv(qv, kc, vc, av, _attnScoresScratch,
                        _numHeads, layerKv, layerHd, pos + 1, effLayerCtx, attnScale: _attnScale);
            }
            finally
            {
                _gpu.Free(qv);
        ...
        // PLE injection (batched, strided proj-slice read) + per-layer output scale.
        if (_hp.HasPerLayerTokenEmbd)
        {
            GpuMatMulBatched(_bpPleGate!, _gpuInpGate![layer], _bpHidden!, N);
            _gpu.GeluTanhMulStrided(_bpPleGate!, _bpProjAll!, _pleWidth,
    ...
    /// STINGRAY_BATCH_DECODE_MMQ=1 force-all override or STINGRAY_BATCH_DECODE_GEMM=1 route this
    ...
    public float[][] BatchVerify(int[] tokens, int startPos)
    {
        ...
        // parity oracle in CudaSpecBatchVerifyTests). STINGRAY_BATCH_DECODE_MMQ=1 still
    ...
    private void EnsureDecodeLogits(int n)
    {
        ...
        if (total > int.MaxValue)
            throw new NotSupportedException(
                $"Batched decode logits buffer ({n}×{_hp.VocabSize}) exceeds int.MaxValue; reduce STINGRAY_MAX_BATCH.");
    ...
    private Tensor UploadWeight(string name)
    {
        ...
        else if (info.DType == DType.Q4_0 || info.DType == DType.Q4_K || info.DType == DType.Q6_K || info.DType == DType.Q8_0)
        {
            ...
            // Gated on the same STINGRAY_MMQ_SOA flag as Q8_0 (#149); dense-only.
