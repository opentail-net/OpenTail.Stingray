...
/// <param name="IsBudgetExplicit">True iff <c>STINGRAY_SNAPKV_BUDGET</c> was set to
...
public readonly record struct SnapKvConfig(int Budget, int Window, int Recency, bool IsBudgetExplicit)
{
    ...
    /// <summary>
    /// Parse <c>STINGRAY_SNAPKV_BUDGET</c>, <c>STINGRAY_SNAPKV_WINDOW</c>,
    /// <c>STINGRAY_SNAPKV_RECENCY</c>. Budget=0 disables; budget unset leaves the
    /// decision to the backend (auto on CUDA hybrid GDN, opt-in elsewhere).
    /// </summary>
    public static SnapKvConfig FromEnvironment()
    {
        var budgetRaw = Environment.GetEnvironmentVariable("STINGRAY_SNAPKV_BUDGET");
        bool explicitBudget = int.TryParse(budgetRaw, out int budget);
        int window  = ParseInt("STINGRAY_SNAPKV_WINDOW",  SnapKvSelector.DefaultWindow);
        int recency = ParseInt("STINGRAY_SNAPKV_RECENCY", SnapKvSelector.DefaultRecency);
