...
public sealed unsafe class GpuForwardPass : IForwardPass
{
    ...
    /// <summary>
    /// Maximum k for the speculative-decode batched verify — how many draft tokens
    /// <see cref="BatchVerify"/> will fold into one pass.
    /// </summary>
    /// <remarks>
    /// <para>This used to double as the batched-prefill chunk size, because both were bounded by the
    /// same shader constant. They are separate concerns and are now separately expressed: prefill
    /// asks <see cref="MaxPrefillChunk"/>, which reports the active matmul path's per-dispatch token
    /// cap. Verify keeps this one, since draft length is a property of the drafter — the number of
    /// tokens it is worth speculating — and not of how wide a matmul tile happens to be.</para>
    ///
    /// <para>Both are 16 today, so the split changes nothing on its own. It exists so a Path 2
    /// kernel with a wider tile can be given a wider prefill chunk without silently altering
    /// speculative-decode behaviour, which would confound the very measurement it is being written
    /// for.</para>
    ///
    /// <para>Raised 8 → 16 historically: for prefill that was a straight halving of the passes over
    /// the weights (267 tokens went from 34 passes to 17). The ceiling is per-lane register
    /// pressure — <c>acc[MAX_NTOK]</c> is one VGPR per token — which is exactly why the way past it
    /// is a different memory hierarchy level rather than a bigger number here.</para>
    /// </remarks>
    private const int MaxBatchVerifyK = 16;

    /// <summary>
    /// Largest batched-prefill chunk the active Vulkan matmul path can serve in one dispatch.
    /// </summary>
    /// <remarks>
    /// Reads through to <see cref="VulkanMatMulPathConfig.MaxTokensPerDispatch"/> rather than
    /// caching, so flipping <c>STINGRAY_VULKAN_MM_PATH</c> at runtime between A/B measurements
    /// takes effect without reloading the model — the same property that made the CPU's
    /// <c>GemmPathConfig</c> usable in a harness.
    /// </remarks>
    private static int MaxPrefillChunk => VulkanMatMulPathConfig.MaxTokensPerDispatch;
    ...
    /// <summary>
    /// Forces <see cref="Prefill"/> onto the per-token <see cref="Forward"/> loop even when the
    /// batched path is available, so a parity test can run both over the same weights and compare.
    /// Also a kill-switch for the batched path: set <c>STINGRAY_VULKAN_NO_BATCHED_PREFILL=1</c>
    /// to fall back in the field without a rebuild.
    /// </summary>
    internal bool DisableBatchedPrefill { get; set; } =
        Environment.GetEnvironmentVariable("STINGRAY_VULKAN_NO_BATCHED_PREFILL") == "1";

    /// <summary>
    /// Kill-switch / A-B hook for the flash attention kernel on the prefill path. Set
    /// <c>STINGRAY_VULKAN_NO_FLASH_ATTN=1</c> to fall back to the per-(head,query) batched
    /// kernel without a rebuild.
    /// </summary>
    internal bool DisableFlashAttention { get; set; } =
        Environment.GetEnvironmentVariable("STINGRAY_VULKAN_NO_FLASH_ATTN") == "1";

    /// <summary>
    /// Tokens per batched-prefill chunk. 0 (the default) means "as many as the active path allows"
    /// — see <see cref="MaxPrefillChunk"/>. Any explicit value is still clamped to that cap, so
    /// neither a test nor the environment can ask for a chunk the kernels would reject.
    /// </summary>
    /// <remarks>
    /// <c>STINGRAY_VULKAN_PREFILL_CHUNK</c> exposes it without a rebuild, which is what makes the
    /// chunk-size sweep possible: chunk size is inversely proportional to how many times prefill
    /// streams the weights, so sweeping it measures how much of prefill time is actually
    /// weight-bandwidth — the entire premise of a tiled Path 2 — using only kernels that already
    /// exist. Distinct from the CPU-side <c>STINGRAY_PREFILL_CHUNK</c>, which sizes continuous
    /// batching's admission chunks.
    /// </remarks>
    internal int PrefillChunkTokens { get; set; } =
        int.TryParse(Environment.GetEnvironmentVariable("STINGRAY_VULKAN_PREFILL_CHUNK"), out int c) && c > 0
            ? c : 0;
    ...
    // Flash-decoding split-KV (issue #312): OPT-IN, DEFAULT-OFF via STINGRAY_VULKAN_SPLIT_DECODE.
    ...
    // LSE-merges the partials, mirroring CUDA's STINGRAY_SPLIT_DECODE. Default-ON (measured ~2×
    // decode at 10.5K ctx vs the VRAM score-spill; the win grows with context); the ≤4096 path
    // is untouched. STINGRAY_VULKAN_SPLIT_DECODE=0 reverts to the spill path (kill switch).
    private readonly bool _splitKvEnabled =
        Environment.GetEnvironmentVariable("STINGRAY_VULKAN_SPLIT_DECODE") != "0";

    /// <summary>
    /// Minimum decode position at which the split-KV (flash-decoding) attention path engages.
    ///
    /// <para>Was 4096, on the assumption that the single-workgroup-per-head scan only collapses
    /// past the 4096-slot shared-memory fast path. Measurement contradicted that: decode fell
    /// 17.8 -> 10.1 -> 6.4 t/s at 267 / 773 / 1621 tokens, entirely inside the supposedly-fast
    /// region, and engaging split-KV there gained +17% / +23% / +33% respectively. Every context
    /// measured — down to a 43-token prompt — was at worst neutral, so the threshold now sits low
    /// enough to actually reach the contexts where decode degrades.</para>
    ///
    /// <para>Note the split count is <c>ceil(seqLen/512)</c>, so below ~512 positions this selects
    /// a one-split partial+combine rather than a real split; the measured gain there is smaller and
    /// its mechanism (most likely avoiding the global score-spill buffer the single-query kernel
    /// uses) was not separately confirmed. Tunable via
    /// <c>STINGRAY_VULKAN_SPLIT_DECODE_MIN</c>; <c>STINGRAY_VULKAN_SPLIT_DECODE=0</c> still
    /// disables the path outright.</para>
    /// </summary>
    private static readonly int SplitKvMinPosition =
        int.TryParse(Environment.GetEnvironmentVariable("STINGRAY_VULKAN_SPLIT_DECODE_MIN"), out var v) && v > 0
            ? v : 256;
    ...
    // out the active rows. PLE (embedding_length_per_layer_input > 0) and shared-KV
    // layers (shared_kv_layers > 0) are wired in issue #351 — the E4B family enables
    // both (PLE injection per layer + a KV-share tail aliased to its source layer).
    ...
    // ── Gemma 4 PLE (per-layer token embeddings, issue #351) ─────────────────
    // The big per_layer_token_embd table stays CPU-resident (mmap, ~3–4 GB); one row is
    // gathered + dequant'd per token and uploaded. model_proj/proj_norm + the per-layer
    // inp_gate/proj/post_norm are GPU-resident. All non-null only when _hasPle.
    private readonly bool _hasPle;
    private readonly int _pleWidth;
    private readonly GgufTensorInfo _pleTokenEmbedInfo;  // CPU-resident raw table (mmap)
    private readonly float[]? _pleRowHost;               // [stackedDim] CPU gather+dequant scratch
    private readonly Tensor? _gpuPleRow;                 // [stackedDim] uploaded per token
    private readonly Tensor? _gpuPerLayerModelProj;      // [stackedDim, embDim] via UploadWeight
    private readonly Tensor? _gpuPerLayerProjNorm;       // [pleWidth]
    private readonly Tensor[]? _gpuInpGate;              // [L] blk.i.inp_gate.weight  ([pleWidth,embDim])
    private readonly Tensor[]? _gpuPleProj;              // [L] blk.i.proj.weight       ([embDim,pleWidth])
    private readonly Tensor[]? _gpuPlePostNorm;          // [L] blk.i.post_norm.weight  ([embDim])
    private readonly Tensor? _gpuProjPerLayer;           // [stackedDim] proj cache per token
    private readonly Tensor? _gpuPleSlice;               // [pleWidth] extracted slice for GeluTanhMul
    private readonly Tensor? _gpuPleX;                   // [pleWidth] inner gate scratch
    private readonly Tensor? _gpuPleY;                   // [embDim] inner proj scratch

    /// <summary>
    /// Choose the KV-cache dtype when the caller did not ask for one (perf-loop iteration 46).
    ///
    /// <para><b>bf16 is the Vulkan default.</b> Measured on this repo's reference part against the
    /// previous default (fp32, which lets SnapKV auto-enable) on a 3239-token prompt:
    /// prefill 47.6 → 52.2 t/s, decode 6.1 → 9.4 t/s, and it keeps the FULL context instead of
    /// letting SnapKV evict it to a 1024-position budget. Perplexity cost is +0.023%
    /// (19.2197 → 19.2241, iteration 45) with no concentration in the deep-context bucket.</para>
    ///
    /// <para>Every case below is one where the narrowed store would otherwise THROW. Because this
    /// is a default rather than a request, falling back to fp32 is the correct response — a user
    /// who never mentioned a KV dtype must not have <c>--tq</c> broken by one. An EXPLICIT
    /// <c>--kv-type bf16</c> still takes the strict path and throws, since silently ignoring it
    /// would be worse than failing.</para>
    /// </summary>
    /// <summary>
    /// The KV-cache element type this pass actually allocated. Since the dtype can now be CHOSEN
    /// (bf16 by default) and then downgraded on a conflict, callers that report or label the
    /// configuration must read it from here rather than re-deriving it from the environment.
    /// </summary>
    public DType KvCacheDType => _kvDType;

    /// <summary>
    /// Whether a narrowed KV store is actually usable for this model/config — the same conditions
    /// the constructor throws on, expressed as a predicate so the defaulted path can fall back
    /// instead of failing. Deliberately conservative: anything not clearly supported returns false.
    /// </summary>
    internal static bool CanNarrowKv(DType kv, ModelHyperparams hp, bool tqEnabled, SnapKvConfig snapKv)
    {
        if (kv is not (DType.BFloat16 or DType.Q8_0)) return false;
        if (tqEnabled) return false;
        if (snapKv.IsBudgetExplicit && snapKv.Budget > 0) return false;
        if (kv == DType.BFloat16 && (hp.HeadDim & 1) != 0) return false;
        if (kv == DType.Q8_0 && ((hp.NumKvHeads * hp.HeadDim) & 31) != 0) return false;
        return true;
    }
        ...
        DType? kvDtype = null)
    {
        ...
        // Read the SnapKV env config early — ChooseDefaultKvDType needs to know whether a budget was
        ...
        // Safety net for the defaulted path (iteration 46). ChooseDefaultKvDType already avoids
        // every rejection below, but those are TWO lists that must agree, and this codebase has
        // been bitten before by a constraint duplicated in two places drifting apart (the split-KV
        // CHUNK constant, iteration 36). So rather than trust the lists, re-check here: a narrowing
        ...
        if (kvNarrowed && _tqEnabled)
            throw new NotSupportedException(
                $"STINGRAY_KV_DTYPE={_kvDType} + TurboQuant is not supported on Vulkan " +
        ...
        // The Vulkan narrowed-KV store is IEEE FP16, not bfloat16, despite the selector being
        // STINGRAY_KV_DTYPE=bf16 and the internal DType being BFloat16. The shaders pack and
        // unpack with unpackHalf2x16 (Shaders.cs:2374, 2674, 2725), which is half-precision by
        // definition; Shaders.cs:2320 already records the discrepancy in passing.
        //
        // Reporting it as "bf16" is actively misleading rather than merely untidy: the two formats
        // differ in the property that matters for a KV cache. FP16 carries 10 mantissa bits but
        // saturates at 65504, so a large activation OVERFLOWS to infinity; BF16 carries 7 mantissa
        // bits with full F32 exponent range and cannot. Anyone diagnosing an inf/NaN in attention,
        // or porting this scheme to another backend on the assumption that it is range-safe, is
        // misled by the label. Diagnostics therefore state the physical format.
        //
        // The env var and DType spelling are left alone deliberately — renaming those is a public
        // API change and CUDA's bf16 path (CudaForwardPass.cs) genuinely is bfloat16, so the
        // selector is not simply wrong, it is overloaded across backends.
        string kvTag = _kvDType switch
        {
            DType.BFloat16 => " [KV fp16 (selector: bf16; Vulkan stores IEEE half)]",
            DType.Q8_0 => " [KV q8_0]",
            _ => "",
        };
        ...
        if (_tqEnabled && _snapKvCfg.IsBudgetExplicit && _snapKvCfg.Budget > 0)
            throw new NotSupportedException(
                ...
                "Set STINGRAY_SNAPKV_BUDGET=0 to disable or disable --tq.");
        ...
        if (kvNarrowed && kvExplicit && _snapKvCfg.IsBudgetExplicit && _snapKvCfg.Budget > 0)
            throw new NotSupportedException(
                $"STINGRAY_KV_DTYPE={_kvDType} + SnapKV is not yet implemented on Vulkan " +
                "(SnapKvScore/KvCompact read the cache as fp32, but the narrowed store packs it; " +
                "issue #325). Set STINGRAY_SNAPKV_BUDGET=0 to disable SnapKV.");
        if (_isGemma4)
        {
        ...
        else
        {
            ...
            if (_snapKvEffectiveBudget > 0)
            {
                Console.Error.WriteLine(
                    ...
                    "set STINGRAY_SNAPKV_BUDGET=0 to disable).");
            }
        }
        ...
        else if (_isGemma4)
        {
            ...
            for (int i = 0; i < hp.NumLayers; i++)
            {
                ...
                if (kvSrc >= 0)
                {
                    if (kvSrc >= i)
                        throw new NotSupportedException(
                            $"KvSourceLayer[{i}]={kvSrc} is a forward reference (>= {i}); shared-KV " +
                            "source layers must precede the layers that alias them.");
        ...
        // maxSplits <= 256 (combine shader bound) ⇔ maxSeqLen <= 65536 at CHUNK=256; bound it
        // directly so the split-count math can't overflow on a pathological ctx. NOTE: CHUNK is
        // duplicated between the three AttentionSplitKvPartial* shaders (const uint CHUNK /
        // sk_scores[CHUNK]), the nSplits formulas in VulkanBackend, and this sizing — all four must
        // move together.
        if (_splitKvEnabled && _maxSeqLen > SplitKvMinPosition && _maxSeqLen <= (int)VulkanBackend.SplitKvMaxSeqLen)
        {
            int maxSplits = (_maxSeqLen + (int)VulkanBackend.SplitKvChunk - 1) / (int)VulkanBackend.SplitKvChunk;
        ...
        // Gemma 4 PLE (issue #351): GPU-resident weight arrays. The big per_layer_token_embd
        // table stays CPU-resident (loaded below); these are the small per-layer projections.
        _hasPle = _isGemma4 && hp.HasPerLayerTokenEmbd;
        if (_hasPle)
        {
        ...
        for (int i = 0; i < L; i++)
        {
            ...
            // missing-tensor errors don't fire). attn_q / attn_q_norm are ALWAYS present (shared
            // layers still compute Q). Mirrors the CUDA upload probe.
            ...
            if (_hasAttnBias)
            {
                ...
                // at all, but guard for safety so a future biased shared-KV GGUF doesn't fault.
                if (!kvShared)
                {
            ...
            if (_hasQkNorm && !_hp.UseL2QkNorm)
            {
                if (_isGemma4)
                {
                    ...
                    // Shared-KV layers compute Q (q_norm present) but not K (k_norm absent).
        ...
        // are uploaded to VRAM (UploadWeight keeps raw or dequants per dtype). Per-token GPU scratch
        // is allocated here. Mirrors CudaForwardPass's PLE ctor wiring.
        if (_hasPle)
        {
            int stackedDim = L * _pleWidth;
            _pleTokenEmbedInfo = model.FindTensor("per_layer_token_embd.weight")!.Value;
            _gpuPerLayerModelProj = UploadWeight("per_layer_model_proj.weight");
            _gpuPerLayerProjNorm = UploadWeight("per_layer_proj_norm.weight");
            _pleRowHost = new float[stackedDim];
            _gpuPleRow = gpu.Allocate(TensorShape.D1(stackedDim));
            _gpuProjPerLayer = gpu.Allocate(TensorShape.D1(stackedDim));
            _gpuPleSlice = gpu.Allocate(TensorShape.D1(_pleWidth));
    ...
    public ReadOnlySpan<float> Prefill(IReadOnlyList<int> tokens, int startPos = 0)
    {
        ...
        // Weight-amortizing batched prefill: process the prompt in chunks of up to MaxPrefillChunk
        // tokens (the active matmul path's per-dispatch cap — NOT the spec-decode draft length),
        // reading each weight matrix from VRAM once per chunk instead of
        // once per token. The per-token loop below is otherwise the whole cost of GPU prefill — an
        // N-token prompt re-streams every weight N times, which is why GPU prefill measured SLOWER
        // than the CPU path before this existed.
        //
        // Excluded (fall through to the per-token loop):
        //  - !_canBatchedTrunk — MoE / Gemma-4 / QKV bias / L2 QK-norm / non-Q4_K-Q6_K weights.
        //  - TurboQuant — its KV ring bookkeeping breaks the contiguous [startPos, startPos+k)
        //    layout the batched KvAppend writes (same exclusion SupportsBatchVerify makes).
        //  - _kvEvicted — after an eviction physical slot != logical position, so the batched
        //    append's contiguous geometry no longer holds.
        //
        // SnapKV is NOT excluded: the trunk captures the trailing window's post-RoPE queries from
        // _qK itself (see RecordBatchedTrunk). It used to be excluded, which meant every prompt
        // above the SnapKV budget silently took the per-token path — measured at 6.4 vs 51.7 t/s
        // on a 3218-token prompt, an 8x cliff on default CLI settings.
        if (_canBatchedTrunk && !_tqEnabled && !_kvEvicted && N > 1
            && !DisableBatchedPrefill)
        {
            int cap = MaxPrefillChunk;
            int chunkSize = PrefillChunkTokens <= 0 ? cap : Math.Clamp(PrefillChunkTokens, 1, cap);
            ReadOnlySpan<float> batchedLogits = default;
            // Heap, not stackalloc: the chunk is now the active path's cap rather than a compile-time
            // 16, and a Path 2 tile wide enough to be worth writing would put a stack buffer into
            // four figures. One array per Prefill call is immaterial next to the GPU work.
            int[] chunkBuf = new int[chunkSize];
            for (int off = 0; off < N; off += chunkSize)
            {
                int take = Math.Min(chunkSize, N - off);
                bool isLast = off + take >= N;
                Span<int> chunk = chunkBuf[..take];
                for (int i = 0; i < take; i++) chunk[i] = tokens[off + i];
                batchedLogits = PrefillBatchedChunk(chunk, startPos + off, take, wantLogits: isLast,
                    promptBase: off,
                    snapWStart: snapKvActive ? wStart : -1,
                    snapW: snapKvActive ? W : 0);
            }

            if (snapKvActive)
                ApplySnapKvEviction(N, W, wStart);

            // STINGRAY_VULKAN_MM_STATS=1: report what the quantized matmul actually streamed.
            // Printed here rather than accumulated across the session because bytes/token is only
            // meaningful against a known token count, and this is where that count is known.
            if (VulkanMatMulPathConfig.StatsEnabled)
            {
                Console.WriteLine($"[GpuForwardPass] prefill N={N} chunk={chunkSize}");
                Console.WriteLine(VulkanMatMulStats.Report(N));
            }
    ...
    /// add), GELU-tanh FFN, per-layer output scale, and the final-logit softcap. PLE injection
    /// and shared-KV layer aliasing (issue #351) are wired for the E4B family. All dispatches are
    /// recorded into one command buffer and submitted once, like the dense Forward.
    /// </summary>
    /// <summary>Bisect probe for the Gemma4 Vulkan defect. Diagnostic only; unset in normal use.</summary>
    ...
    private ReadOnlySpan<float> ForwardGemma4(int token, int position)
    {
        ...
        // its own command buffer (submit+wait), so it cannot be nested inside an active record.
        if (_hasPle) BuildPerLayerRowUpload(token);

        _gpu.BeginRecord();
        // Make the _gpuPleRow transfer write visible to the compute reads in BuildPerLayerProjectionsGpu.
        ...
        // Bisect probe (STINGRAY_GEMMA4_PROBE=embed): submit here and report whether the state
        // is already non-finite BEFORE any layer runs. Splits the Gemma4 Vulkan defect search in
        // one shot — embedding/PLE-upload versus the 42-layer trunk. Diagnostic only; throws.
        if (s_gemma4Probe == "embed")
        {
            _gpu.RecordDownloadToStaging(_hidden, _embDim);
            _gpu.EndRecordAndSubmit();
            var probe = new float[_embDim];
            _gpu.ReadFromStaging(probe);
            int bad = 0; float amax = 0f;
            foreach (float v in probe)
            {
                if (!float.IsFinite(v)) bad++;
                else if (Math.Abs(v) > amax) amax = Math.Abs(v);
            }
            throw new InvalidOperationException(
                $"[gemma4-probe embed] token={token} embDim={_embDim} nonFinite={bad} maxAbsFinite={amax:E3} "
    ...
    private void RunGemma4Layers(int position)
    {
        ...
        // costs one fence wait each — irrelevant for a diagnostic, and far cheaper than 42 runs
        // of a 4 GB model. Device buffers persist across submissions, so re-BeginRecord resumes
        // the trunk exactly where it left off. Reporting every layer (not stopping at the first
        // bad one) shows the magnitude drift leading up to the blow-up, which is what separates
        // "one layer is broken" from "the state diverges gradually".
        System.Text.StringBuilder? probeLog = null;
        float[]? probeScratch = null;
        if (s_gemma4Probe == "layers")
        {
            probeLog = new System.Text.StringBuilder(
                $"[gemma4-probe layers] hidden after each layer (embDim={_embDim}, position={position})\n"
                + "layer   hd   kv  shared  swa   kEqV  nonFinite      maxAbs         rms\n");
            probeScratch = new float[_embDim];
        }

        // Bisect probe (STINGRAY_GEMMA4_PROBE=stage0): the same trick one level finer — dump
        // every intermediate inside layer 0 and throw at the end of it. Used once `layers` shows
        // the state is already dead at layer 0, to name the single dispatch that kills it.
        bool stageProbe = s_gemma4Probe == "stage0";
        System.Text.StringBuilder? stageLog = stageProbe
            ? new System.Text.StringBuilder(
                $"[gemma4-probe stage0] layer 0 intermediates (embDim={_embDim}, position={position})\n"
                + $"{"stage",-26} {"n",6} {"nonFinite",9} {"maxAbs",11} {"rms",11}\n")
            : null;
        float[] stageScratch = stageProbe ? new float[_embDim] : [];
        // Keep the GELU inputs so a non-finite OUTPUT can be attributed to a specific input
        // value rather than to "the activation" in general.
        float[]? savedGate = null, savedUp = null;

        void Stage(string tag, Tensor t, int n)
        {
            if (stageLog is null) return;
            _gpu.RecordBarrier();
            _gpu.RecordComputeToTransferBarrier();
            _gpu.RecordDownloadToStaging(t, n);
            _gpu.EndRecordAndSubmit();
            if (stageScratch.Length < n) stageScratch = new float[n];
            _gpu.ReadFromStaging(stageScratch.AsSpan(0, n));

            int bad = 0, firstBad = -1; float amax = 0f; double sumsq = 0;
            for (int i = 0; i < n; i++)
            {
                float v = stageScratch[i];
                if (!float.IsFinite(v)) { bad++; if (firstBad < 0) firstBad = i; continue; }
                float a = Math.Abs(v);
                if (a > amax) amax = a;
                sumsq += (double)v * v;
            }
            stageLog.Append($"{tag,-26} {n,6} {bad,9} {amax,11:E3} {Math.Sqrt(sumsq / n),11:E3}");
            if (firstBad >= 0) stageLog.Append($"   firstBad=[{firstBad}]={stageScratch[firstBad]}");
            stageLog.Append('\n');

            if (tag.StartsWith("16", StringComparison.Ordinal)) savedGate = stageScratch[..n];
            if (tag.StartsWith("17", StringComparison.Ordinal)) savedUp = stageScratch[..n];
            // Attribute a GELU blow-up to its exact inputs: g, up, and the tanh argument the
            // shader actually forms. Without this the finding is "the activation NaNs", which
            // does not distinguish an overflowing tanh from a bad input.
            if (tag.StartsWith("18", StringComparison.Ordinal) && firstBad >= 0
                && savedGate is not null && savedUp is not null)
            {
                float g = savedGate[firstBad];
                double inner = 0.7978845608028654 * (g + 0.044715 * (double)g * g * g);
                stageLog.Append(
                    $"    gelu input at [{firstBad}]: g={g:R} up={savedUp[firstBad]:R} "
                    + $"tanhArg={inner:R} exp(2*tanhArg)={Math.Exp(2 * inner):R}\n");
            }
            _gpu.BeginRecord();
        }
        ...
        for (int layer = 0; layer < L; layer++)
        {
            ...
            // Shared-KV tail layers (issue #351) reuse the source layer's K/V cache pages and skip
            // their own K/V projection + norm + RoPE + append. They still compute Q and run
            // attention against the source layer's (effLayer) cache.
            ...
            // (attention_k_eq_v). Only meaningful on KV-owning layers.
            bool kEqV = !kvShared && _hp.AttentionKEqV && !isSwa && _wv[layer] is null;
            ...
            Stage("00 hidden@entry", _hidden, _embDim);
            ...
            Stage("05 q_norm", qView, qDimL);
            Stage("06 k_norm", kView, kvDimL);
            ...
            //    KV-owning layers (shared layers' K was RoPE'd by the source layer).
            ...
            _gpu.RecordBarrier();
            Stage("08 q_rope", qView, qDimL);
            Stage("09 k_rope", kView, kvDimL);
            ...
            Stage("13 post_attn_norm", _hidden, _embDim);
            _gpu.AddInPlace(_hidden, _residual);
            _gpu.RecordBarrier();
            ...
            _gpu.RmsNorm(_normBuf, _hidden, _wFfnNorm[layer], _hp.RmsNormEps);
            _gpu.RecordBarrier();
            Stage("15 ffn_norm", _normBuf, _embDim);
            GpuMatMul(_ffnGate, _wGate[layer], _normBuf);
            GpuMatMul(_ffnUp, _wUp[layer], _normBuf);
            _gpu.RecordBarrier();
            Stage("16 ffn_gate", _ffnGate, _intermDim);
            Stage("17 ffn_up", _ffnUp, _intermDim);
            _gpu.GeluTanhMul(_ffnGate, _ffnUp);
            _gpu.RecordBarrier();
            Stage("18 gelu_mul", _ffnGate, _intermDim);
            GpuMatMul(_hidden, _wDown[layer], _ffnGate);
            _gpu.RecordBarrier();
            Stage("19 ffn_down", _hidden, _embDim);
            ...
            Stage("20 post_ffw_norm", _hidden, _embDim);
            _gpu.AddInPlace(_hidden, _residual);
            _gpu.RecordBarrier();
            ...
            if (probeLog is not null)
            {
                // Own barriers: the layer body's trailing barrier is conditional, so do not
                // rely on it. RecordBarrier also republishes the layer's writes for the NEXT
                // submission's shader reads — a fence wait alone is an execution dependency,
                // not a memory one.
                _gpu.RecordBarrier();
                _gpu.RecordComputeToTransferBarrier();
                _gpu.RecordDownloadToStaging(_hidden, _embDim);
                _gpu.EndRecordAndSubmit();
                _gpu.ReadFromStaging(probeScratch!);

                int bad = 0; float amax = 0f; double sumsq = 0;
                foreach (float v in probeScratch!)
                {
                ...
                probeLog.Append(
                    $"{layer,5}  {layerHd,3}  {layerKv,3}  {(kvShared ? "yes" : "no"),6}  "
                    + $"{(isSwa ? "yes" : "no"),3}  {(kEqV ? "yes" : "no"),5}  {bad,9}  "
                    + $"{amax,10:E3}  {Math.Sqrt(sumsq / _embDim),10:E3}\n");
        ...
        if (probeLog is not null)
            throw new InvalidOperationException(probeLog.ToString());
    }

    // ================================================================
    //  Gemma 4 Per-Layer-Embedding (PLE), issue #351
    // ================================================================

    /// <summary>
    /// CPU gather + dequant of the active token's PLE row, then upload it to <see cref="_gpuPleRow"/>.
    /// MUST run OUTSIDE the command-buffer record (UploadToExisting submits + waits its own buffer).
    /// Mirrors the CPU-dequant prologue of <c>CudaForwardPass.BuildPerLayerProjectionsGpu</c>.
    /// <para>The per_layer_token_embd table is ~3–4 GB (token-major: one stackedDim-wide row per
    /// token), so the row offset <c>token * bytesPerRow</c> exceeds int.MaxValue and a
    /// <c>Span.Slice(int,int)</c> can't address it. We take a raw <c>byte*</c> base pointer from the
    /// mmap (<see cref="GgufModel.GetTensorDataPtr"/>) and index it by a <c>long</c> — the same
    /// approach as the CPU/CUDA references.</para>
    /// </summary>
    private void BuildPerLayerRowUpload(int token)
    {
        int stackedDim = _hp.NumLayers * _pleWidth;
        int bytesPerRow = (stackedDim / DTypeInfo.BlockSize(_pleTokenEmbedInfo.DType))
                        * DTypeInfo.BytesPerBlock(_pleTokenEmbedInfo.DType);
        byte* rowPtr = _model.GetTensorDataPtr(_pleTokenEmbedInfo) + (long)token * bytesPerRow;
        var rowHost = _pleRowHost!.AsSpan(0, stackedDim);
        if (_pleTokenEmbedInfo.DType == DType.Float32)
            ...
            Dequantize.ToFloat32(new ReadOnlySpan<byte>(rowPtr, bytesPerRow),
                rowHost, _pleTokenEmbedInfo.DType, stackedDim);
        UploadToExisting(_gpuPleRow!, rowHost);
    }

    /// <summary>
    /// In-record GPU build of the per-layer projection cache (once per token). Mirrors CPU
    /// <c>ForwardPass.BuildPerLayerProjections</c> and <c>CudaForwardPass.BuildPerLayerProjectionsGpu</c>:
    /// scale the PLE row by sqrt(PleWidth), project through per_layer_model_proj, scale by
    /// 1/sqrt(EmbeddingDim), per-layer RmsNorm (per_layer_proj_norm) + add the same-slice PLE row,
    /// scale by 1/sqrt(2). Records into the in-progress command buffer.
    /// </summary>
    private void BuildPerLayerProjectionsGpu()
    {
        int L = _hp.NumLayers;
        ...
        _gpu.RecordBarrier();
        // Per-layer RmsNorm (one block per pleWidth row), then whole-buffer add + scale.
        _gpu.RmsNormBatched(_gpuProjPerLayer!, _gpuProjPerLayer!, _gpuPerLayerProjNorm!,
            _pleWidth, L, _hp.RmsNormEps);
        _gpu.RecordBarrier();
    ...
    /// → proj @ → post_norm → add to hidden</c>. Runs after the post-FFN residual add, before
    /// layer_output_scale. Mirrors CPU <c>ForwardPass.ApplyPerLayerEmbedding</c> /
    /// <c>CudaForwardPass.ApplyPerLayerEmbeddingGpu</c>. Vulkan has no offset views, so the proj
    /// slice is extracted into <see cref="_gpuPleSlice"/> via a region copy before GeluTanhMul
    /// (which writes gate = gelu_tanh(gate)*slice in place — gate is _gpuPleX, the FIRST arg).
    /// Records into the in-progress command buffer.
    /// </summary>
    private void ApplyPerLayerEmbedding(int layer)
    {
        ...
        _gpu.RecordBarrier();
        // Extract proj_per_layer[layer] (no offset views on Vulkan): copy the pleWidth-wide slice.
        CopyBufferRegion(_gpuPleSlice!, 0, _gpuProjPerLayer!,
            (long)layer * _pleWidth * sizeof(float), (long)_pleWidth * sizeof(float));
        _gpu.RecordBarrier();
        _gpu.GeluTanhMul(_gpuPleX!, _gpuPleSlice!);
        _gpu.RecordBarrier();
        // proj output → [embDim].
    ...
    {
        int k = tokens.Length;
        EnsureBatchVerifyScratch(k, withLogits: true);   // verify reads all k rows' logits
        ...
        // Spec-decode verify accepts on argmax, so the int8 activation path's small perturbation
        // costs at most a rejected draft token, never correctness — take the faster path.
        RecordBatchedTrunk(tokens, startPos, k, allowInt8: true, allowFlashAttention: false);
    ...
    /// <summary>
    /// Records the shared weight-amortizing trunk for <paramref name="k"/> tokens at contiguous
    /// positions [<paramref name="startPos"/>, <paramref name="startPos"/> + k): embed, then every
    /// layer's norm / QKV / QK-norm / RoPE / KvAppend+attention / O-projection / FFN, leaving the
    /// post-last-layer hidden states in <see cref="_hiddenK"/> with the command buffer STILL
    /// RECORDING. The caller opens nothing and must close it (final norm, whatever output
    /// projection it needs, then <c>EndRecordAndSubmit</c>).
    ///
    /// <para>Two callers with different tails: <see cref="BatchVerifyBatched"/> (speculative decode
    /// — needs all k rows' logits) and <see cref="PrefillBatchedChunk"/> (prefill — needs only the
    /// last row's, or none at all for a non-final chunk). The attention is causal with token i
    /// attending [0, startPos+i], which is simultaneously verify's semantics and prefill's, which
    /// is why one trunk serves both.</para>
    ///
    /// <para>Caller must have run <see cref="EnsureBatchVerifyScratch"/>(k) first, and must satisfy
    /// <see cref="_canBatchedTrunk"/>.</para>
    /// </summary>
    private void RecordBatchedTrunk(ReadOnlySpan<int> tokens, int startPos, int k, bool allowInt8,
        bool allowFlashAttention, int promptBase = 0, int snapWStart = -1, int snapW = 0)
    {
        int qDim = _numHeads * _headDim;
        int kvDim = _numKvHeads * _headDim;
        int embDim = _embDim;
        ...
        for (int layer = 0; layer < _hp.NumLayers; layer++)
        {
            ...
            _gpu.MatMulBatched(_qK, _wq[layer], _normK, k, WeightDType(_wq[layer]), allowInt8);
            _gpu.MatMulBatched(_kK, _wk[layer], _normK, k, WeightDType(_wk[layer]), allowInt8);
            _gpu.MatMulBatched(_vK, _wv[layer], _normK, k, WeightDType(_wv[layer]), allowInt8);
            _gpu.RecordBarrier();
            ...
            // SnapKV (issue #59): capture the post-RoPE / post-QK-norm queries for the trailing
            // scoring window, exactly what the per-token Forward path writes via
            // _snapKvCaptureSlot. _qK already holds all k queries in their final form, so this is
            // a strided copy, not recomputation — which is what lets SnapKV coexist with the
            // batched trunk instead of forcing the whole prefill onto the per-token loop
            // (previously an 8x cliff for any prompt above the SnapKV budget).
            if (snapW > 0 && _snapKvQCapture is { } capBuf)
            {
                for (int i = 0; i < k; i++)
                {
                    int slot = promptBase + i - snapWStart;
                    if (slot < 0 || slot >= snapW) continue;
                    long dstElems = ((long)layer * _snapKvQCaptureW + slot) * qDim;
                    _gpu.RecordComputeCopyRegion(capBuf, dstElems * sizeof(float),
                        _qK, (long)i * qDim * sizeof(float), (long)qDim * sizeof(float));
                }
                _gpu.RecordBarrier();
            }
            ...
            //
            // fp32 additionally prefers the FLASH kernel (one workgroup per head, online softmax,
            // K/V streamed once and reused across all k queries). It is the same computation but
            // NOT bit-identical — the running max/sum rescales as it goes — so it is used for
            // prefill, where the contract is FP-tolerance parity with the per-token path, and NOT
            // for spec-decode verify, which the argmax accept rule makes tolerant anyway but whose
            // established contract is exactness. It also has no 4096 ceiling, since it never
            // materialises a score array, which is why it is allowed to widen `batchedAttn`.
            // bf16 KV gets the flash kernel too (perf-loop iteration 44). It was previously gated to
            // fp32 only, which silently cost narrowed-KV prefill the whole iteration-31 win: bf16
            // fell back to AttentionBatchedBf16, whose per-(head, query) dispatch re-reads the
            ...
            if (batchedAttn)
            {
                switch (_kvDType)
                {
                    case DType.Float32:
                        ...
                        _gpu.RecordBarrier();
                        if (flashAttn)
                            _gpu.AttentionBatchedFlash(_qK, _gpuKCache[layer], _gpuVCache[layer], _attnOutK,
            ...
            _gpu.MatMulBatched(_hiddenK, _wo[layer], _attnOutK, k, WeightDType(_wo[layer]), allowInt8);
            _gpu.RecordBarrier();
    ...
    /// <summary>
    /// Runs one batched prefill chunk of <paramref name="k"/> tokens (k ≤
    /// <see cref="MaxBatchVerifyK"/>) at positions [<paramref name="startPos"/>,
    /// <paramref name="startPos"/> + k) through <see cref="RecordBatchedTrunk"/>, reading each
    /// weight matrix from VRAM once for all k tokens instead of once per token.
    ///
    /// <para>When <paramref name="wantLogits"/> is false (every chunk but the last) this skips the
    /// final norm, the output projection AND the download entirely: prefill discards intermediate
    /// positions' logits, and <c>output.weight</c> is the largest matmul in the model
    /// (vocab × embDim), so computing it for positions nobody reads is pure waste. The last chunk
    /// projects its final row only — one single-row matvec, not k.</para>
    /// </summary>
    // `scoped` on tokens: the returned span is the _logitsBuf field, never a view over the
    // caller's (stack-allocated) chunk buffer. Without it the compiler must assume the return
    // could alias the input and rejects the call site (CS9080).
    private ReadOnlySpan<float> PrefillBatchedChunk(scoped ReadOnlySpan<int> tokens, int startPos, int k, bool wantLogits,
        int promptBase = 0, int snapWStart = -1, int snapW = 0)
    {
        // withLogits: false — prefill returns only the LAST row's logits, and does so through the
        // single-token _logits tail below, never _logitsK.
        EnsureBatchVerifyScratch(k, withLogits: false);
        // allowInt8: false — prefill's logits SELECT the first generated token, so a flipped argmax
        // changes the whole completion (unlike spec-verify, where it only rejects a draft). The FP
        // batched shader still reads each weight row once per chunk, so the weight-bandwidth win —
        // which is the entire point of batching here — is unaffected; only the activation precision
        // differs. Measured: int8 flipped the argmax on 2 of 6 short prompts, where near-tied
        // out-of-distribution logits are within the ~0.5% activation noise.
        RecordBatchedTrunk(tokens, startPos, k, allowInt8: false, allowFlashAttention: !DisableFlashAttention,
            promptBase: promptBase, snapWStart: snapWStart, snapW: snapW);
        ...
        // Last row only: gather it into the single-token _hidden, then reuse the exact final-norm +
        // output-projection + download tail that Forward uses, so the returned logits are produced
        // by the same shaders (and are bit-identical to the per-token path's last step).
        const int f32 = sizeof(float);
    ...
    /// <summary>
    /// Lazily allocate the batched [K][dim] trunk scratch for <paramref name="k"/> tokens, and
    /// optionally the [K][vocab] logits pair. Freed in <see cref="Dispose"/>.
    /// </summary>
    /// <param name="k">Tokens in the pass. Bounded by <see cref="MaxBatchVerifyK"/> for verify, by
    /// <see cref="MaxPrefillChunk"/> for prefill.</param>
    /// <param name="withLogits">
    /// Whether this pass reads all k rows' logits. <b>True only for speculative-decode verify.</b>
    /// Batched prefill returns the last row's logits through the single-token <c>_logits</c> path,
    /// so it never touches <c>_logitsK</c> — and that buffer is <c>k × vocab</c>, by far the largest
    /// in the set. Allocating it unconditionally was harmless while k was pinned at 16; once the
    /// prefill chunk is free to follow a wider Path 2 tile it would dominate the scratch (at k=256
    /// and a 49k vocab, 50 MiB of VRAM plus 50 MiB of host array, per chunk, unused).
    /// </param>

    /// <summary>
    /// Fail loudly if the GPU produced non-finite logits, instead of letting them reach the sampler.
    /// </summary>
    /// <remarks>
    /// <para><b>Why this exists.</b> Gemma 4 E4B q4_0 on Vulkan returns 262144 non-finite logits and
    /// decodes to <c>&lt;pad&gt;</c> forever, while the CPU path on the same weights produces a real
    /// token. Nothing detected it: argmax over NaNs yields index 0, index 0 is a valid token id, and
    /// generation proceeds looking merely stupid rather than broken. Three tests sat green on this
    /// for as long as the model file happened to be absent.</para>
    ///
    /// <para><b>This is not a fix for that defect and does not claim to be.</b> The root cause is not
    /// yet identified — a headDim=512 hypothesis was raised and then weakened, because the attention
    /// shaders inspected use grid-stride loops and runtime head-dim bounds. What this does is convert
    /// a silent wrong answer into a diagnosable failure, which is worth having independently of the
    /// cause, and which will catch the next bug of the same class too.</para>
    ///
    /// <para>Cost is one pass over the logits per forward — negligible beside the matmul that
    /// produced them, and only on the GPU path. The expensive full count runs only once, on the
    /// failure path.</para>
    /// </remarks>
    private void GuardFiniteLogits()
    {
        var logits = _logitsBuf;
        for (int i = 0; i < logits.Length; i++)
        {
            if (float.IsFinite(logits[i])) continue;

            int bad = 0;
            for (int j = 0; j < logits.Length; j++)
                if (!float.IsFinite(logits[j])) bad++;
    ...
    private void EnsureBatchVerifyScratch(int k, bool withLogits)
    {
        ...
        if (_bvK == k)
        {
            // Trunk buffers are right. Add the logits pair if this caller needs it and a previous
            // (prefill) caller at the same k did not allocate it.
            if (withLogits && _logitsKBuf is null)
            {
                _logitsK = _gpu.Allocate(TensorShape.D1((long)k * _hp.VocabSize));
                _logitsKBuf = new float[k * _hp.VocabSize];
            }
            return;
        }
        ...
        try
        {
            ...
            if (withLogits)
            {
                ...
                _logitsKBuf = new float[k * _hp.VocabSize]; // verify k ≤ 16, vocab small → fits int
            }
            else
            {
                // Leave _logitsK holding the freed handle from the previous generation; _logitsKBuf
                // is the liveness flag FreeBatchVerifyScratch and the k-match path above both read.
                _logitsKBuf = null;
            }
            _bvK = k;
        }
    ...
    // Called only when fully allocated (_bvK > 0); the trunk tensors are non-null here. A partial
    // allocation that threw is freed in EnsureBatchVerifyScratch's catch, not here (the fields
    // would be null/freed). Free dereferences tensor.Handle, so it is NOT null-safe — which is why
    // _logitsK is freed only when _logitsKBuf proves the pair was actually allocated.
    ...
    public void Dispose()
    {
        ...
        // (InferenceEngineLoader adds it to owned[] when constructing the full-GPU-offload
        ...
        for (int i = 0; i < _hp.NumLayers; i++)
        {
            ...
            {
                _gpu.Free(_bq![i]);
                // Shared-KV layers carry no K/V bias (the upload is guarded the same way) — slots
                // are null. Guard symmetrically so a future biased shared-KV GGUF won't NRE here.
                if (_bk![i] is not null) _gpu.Free(_bk[i]);
                if (_bv![i] is not null) _gpu.Free(_bv[i]);
            ...
            if (_hasQkNorm && !_hp.UseL2QkNorm)
            {
                if (_isGemma4)
                {
                    _gpu.Free(_wQNormG4![i]);
                    // Shared-KV layers carry no attn_k_norm (slot is null).
                    if (!kvShared) _gpu.Free(_wKNormG4![i]);
                }
                else
                {
            ...
            // Gemma 4 PLE per-layer weights (issue #351).
            if (_hasPle)
            {
        ...
        // Gemma 4 PLE shared buffers (issue #351), null unless _hasPle. The per-layer PLE weight
        // arrays were freed in the layer loop above; these are the model-wide proj/norm + scratch.
        if (_gpuPleRow is { } pleRow) _gpu.Free(pleRow);
        if (_gpuPerLayerModelProj is { } plProj) _gpu.Free(plProj);
        if (_gpuPerLayerProjNorm is { } plProjNorm) _gpu.Free(plProjNorm);
        if (_gpuProjPerLayer is { } projPerLayer) _gpu.Free(projPerLayer);
        if (_gpuPleSlice is { } pleSlice) _gpu.Free(pleSlice);
