...
public static class Sampler
{
    ...
    public static int Sample(ReadOnlySpan<float> logits, SamplingParams p, Random? rng = null)
    {
        ...
        float[]? rented = null;
        Span<float> probs = vocabSize <= 256
    ...
    private static int SampleTopK(ReadOnlySpan<float> logits, SamplingParams p, Random rng, bool hasPenalty)
    {
        ...
        int[]? rentedIdx = null;
        float[]? rentedVals = null;
        float[]? rentedProbs = null;

        Span<int> idx = sel <= 256
            ? stackalloc int[sel]
            : (rentedIdx = ArrayPool<int>.Shared.Rent(sel)).AsSpan(0, sel);
        Span<float> vals = sel <= 256
            ? stackalloc float[sel]
            : (rentedVals = ArrayPool<float>.Shared.Rent(sel)).AsSpan(0, sel);
        ...
        try
        {
            ...
            Span<float> probs = k <= 256
                ? stackalloc float[k]
                : (rentedProbs = ArrayPool<float>.Shared.Rent(k)).AsSpan(0, k);
    ...
    private static void Softmax(Span<float> x)
    {
        ...
        {
            // All logits are -Infinity (fully masked vocabulary): assign uniform probabilities
            float uniform = x.Length > 0 ? 1.0f / x.Length : 0f;
            x.Fill(uniform);
            return;
        }
...
public sealed record SamplingParams
{
    ...
    /// (<c>STINGRAY_MAX_BATCH</c>, issue #377): under batching the instance is carried per sequence and
    ...
    ///         <c>STINGRAY_DISABLE_MTP=1</c>.</item>
