...
public sealed unsafe class CudaHybridGdnForwardPass : IForwardPass
{
    ...
    // at ctx=4096 to admit more dense-FFN layers on GPU. `STINGRAY_KV_DTYPE=fp32`
    ...
    // Used by the CPU GDN block path (STINGRAY_CPU_GDN=1).
    ...
    // Diagnostic: per-layer activation trace (env: STINGRAY_TRACE_LAYERS=1).
    private static readonly bool _traceLayers =
        Environment.GetEnvironmentVariable("STINGRAY_TRACE_LAYERS") == "1";
    ...
    //
    // STINGRAY_CPU_MOE override values:
    ...
    // STINGRAY_CPU_GDN=1 forces the legacy CPU GDN block path (Phase 7a baseline).
    ...
    private readonly bool _cpuGdn =
        Environment.GetEnvironmentVariable("STINGRAY_CPU_GDN") == "1";
    ...
    // legacy FP kernels are the decode bottleneck. STINGRAY_Q3K_Q8K=1 / =0 and
    // STINGRAY_Q8_0_Q8K=1 / =0 force the latch on / off respectively, overriding
    ...
    // the STINGRAY_*_Q8K gates above (which control the Q8_KS scheme for Q3_K/Q4_K/Q8_0) —
    ...
    // BatchForward2 (STINGRAY_CPU_GDN=1 debug trunk) snapshots t1's pre-norm hidden into
    ...
    // _batchSnapshotBuf above serves the STINGRAY_CPU_GDN=1 debug trunk only.
    ...
    // Prefill profiling (STINGRAY_PREFILL_PROFILE=1): accumulate synchronous CPU-MoE
    ...
    // Decode profiling (STINGRAY_DECODE_PROFILE=1): per-token GPU-trunk vs CPU-MoE split. The
    ...
    private static readonly bool _decodeProfile =
        Environment.GetEnvironmentVariable("STINGRAY_DECODE_PROFILE") == "1";
    ...
    // Gated STINGRAY_DECODE_CUDA_GRAPH (default OFF). Falls back to direct launches on any capture failure.
    private static readonly bool _decodeCudaGraph =
        Environment.GetEnvironmentVariable("STINGRAY_DECODE_CUDA_GRAPH") == "1";
    ...
    // STINGRAY_PREFILL_PROFILE=1. Establishes whether routed-MoE prefill cost is in the
    ...
    // STINGRAY_BATCHED_PREFILL=0 (falls back to the sequential Forward loop).
    ...
    // STINGRAY_GDN_PREFILL_COMPUTE=0 reverts to the byte-exact per-token matvec.
    internal static bool GdnPrefillComputeEnabled =
        Environment.GetEnvironmentVariable("STINGRAY_GDN_PREFILL_COMPUTE") != "0";
    ...
    // shares the same contract as GdnPrefillComputeEnabled; STINGRAY_GDN_RAW_Q8_0=0 reverts
    // to the F32-dequant upload (the byte-exact reference the bit-parity oracle pins).
    internal static bool RawQ80WeightsEnabled =
        Environment.GetEnvironmentVariable("STINGRAY_GDN_RAW_Q8_0") != "0";
    ...
    // original byte-exact kernel. Default OFF; STINGRAY_GDN_DECODE_FAST=1 enables.
    internal static bool GdnDecodeFastEnabled =
        Environment.GetEnvironmentVariable("STINGRAY_GDN_DECODE_FAST") == "1";
    ...
    // a Q8_0 matvec). Default OFF; STINGRAY_TRUNK_MATVEC_FAST=1 enables.
    internal static bool TrunkMatVecFastEnabled =
        Environment.GetEnvironmentVariable("STINGRAY_TRUNK_MATVEC_FAST") == "1";
    ...
    // STINGRAY_*_Q8K); Q6_K uses the plain Q8_K scheme its single dot uses. Allocated only when
    ...
    // STINGRAY_BATCHED_TRUNK=0 (falls back to the per-token trunk loop).
    internal static bool BatchedTrunkEnabled =
        Environment.GetEnvironmentVariable("STINGRAY_BATCHED_TRUNK") != "0";
    private int _btCap;
    ...
    // OPT-IN, default OFF (STINGRAY_MOE_GPU_PREFILL=1; CLI --gpu-moe-prefill true / server
    ...
    //   (STINGRAY_MOE_PIN_MODE=register, now default) cudaHostRegisters the expert mmap pages instead
    ...
    // as BatchedGpuMoeFfn's already-default GPU router). STINGRAY_MOE_GPU_ROUTER=0 forces CPU.
    private readonly bool _gpuRouterPrefill = ResolveGate("STINGRAY_MOE_GPU_ROUTER", true);
    ...
    // Env override STINGRAY_MOE_GPU_PREFILL_MIN_TOKENS (0 = always engage when op-offload is on).
    private readonly int _gpuMoePrefillMinTokens = ResolveIntGate("STINGRAY_MOE_GPU_PREFILL_MIN_TOKENS", 64);
    ...
    // GPU-offload per-phase profiling (printed once per chunk under STINGRAY_PREFILL_PROFILE).
    ...
    // per-token loop. Default on; STINGRAY_BATCHED_FFN=0 forces the per-token FFN path
    // (the bit-exact reference). A/B-toggleable like STINGRAY_BATCHED_TRUNK.
    internal static bool BatchedFfnEnabled =
        Environment.GetEnvironmentVariable("STINGRAY_BATCHED_FFN") != "0";
    ...
    // STINGRAY_BATCHED_GDN_SCAN=0 falls back to the per-position View loop inside
    // GdnBlockBatched (the pre-#114-B path). A/B-toggleable like STINGRAY_BATCHED_TRUNK.
    internal static bool BatchedGdnScanEnabled =
        Environment.GetEnvironmentVariable("STINGRAY_BATCHED_GDN_SCAN") != "0";
    ...
    //   scan is kept. Tri-state STINGRAY_GDN_CHUNKED_PREFILL: "1" forces chunked even with
    ...
    internal static bool? GdnChunkedPrefillOverride =
        Environment.GetEnvironmentVariable("STINGRAY_GDN_CHUNKED_PREFILL") switch
    ...
    // STINGRAY_BATCHED_ATTN=0 forces the per-position path.
    internal static bool BatchedAttnEnabled =
        Environment.GetEnvironmentVariable("STINGRAY_BATCHED_ATTN") != "0";
    ...
    // Kill-switch: STINGRAY_BATCHED_CPU_FFN=0.
    internal static bool BatchedCpuDenseFfnEnabled =
        ResolveGate("STINGRAY_BATCHED_CPU_FFN", true);
        ...
        LayerPlacement placement, int maxContextLength = 0)
    {
        ...
        if (hp.IsMoE)
        {
            ...
            else
            {
                ...
                _cpuMoe = ratio < 0.5;
                Console.Error.WriteLine(
                    $"[CudaHybridGdnForwardPass] MoE auto-select: SLRU capacity ≈ {predictedSlots}/{totalExperts} ({ratio:P0}) → {(_cpuMoe ? "CPU" : "GPU SLRU")} MoE.  Override with STINGRAY_CPU_MOE=0|1.");
            }
        }
        else
        {
        ...
        // Carnice). STINGRAY_Q3K_Q8K / STINGRAY_Q8_0_Q8K = "1" or "0" override.
        ...
        _q3kQ8KEnabled  = ResolveGate("STINGRAY_Q3K_Q8K",  hasQ3KRouted);
        _q8_0Q8KEnabled = ResolveGate("STINGRAY_Q8_0_Q8K", hasQ8_0Routed);
        ...
        // the int8 dot can win); on AVX-512 default OFF. Still forceable via STINGRAY_Q4K_Q8K=1.
        // (Q3_K/Q8_0 int8 have no f32-AVX512 competitor, so they stay auto-on.)
        _q4kQ8KEnabled  = ResolveGate("STINGRAY_Q4K_Q8K",
        ...
        if (_cpuMoe && (_q3kQ8KEnabled || _q8_0Q8KEnabled || _q4kQ8KEnabled))
        {
            ...
            Console.Error.WriteLine(
                $"[CudaHybridGdnForwardPass] Routed-MoE Q8_K-input kernels enabled: {string.Join(", ", enabled)}. Override with STINGRAY_Q3K_Q8K=0 / STINGRAY_Q8_0_Q8K=0 / STINGRAY_Q4K_Q8K=0.");
        }
        ...
        for (int i = 0; i < L; i++)
        {
            ...
            if (hp.IsMoE)
            {
                ...
                else
                {
                    ...
                    // prefill router matvec can run on-GPU (STINGRAY_MOE_GPU_ROUTER); the routed
                    // expert weights still stream from CPU mmap. _cpuFfnGateInp stays resolved for
                    // the CPU-router fallback (STINGRAY_MOE_GPU_ROUTER=0 / sequential trunk).
        ...
        // and 5-10× every verify phase. STINGRAY_DISABLE_MTP=1 skips the ring so
        ...
        if (_hasMtp)
        {
            ...
            if (_mtpIsMoE && !_cpuMoe)
                throw new NotSupportedException(
                    "MoE MTP head requires STINGRAY_CPU_MOE=1. GPU MoE path (SLRU expert cache) " +
            ...
            // the STINGRAY_CPU_GDN=1 debug trunk uses it (the default GPU trunk's
        ...
        // path, ~5× slower than warm. MmapPrefault honours STINGRAY_PREFAULT and the
        ...
        // Op-offload (STINGRAY_MOE_GPU_PREFILL): build ALL op-offload scratch at LOAD — the
        ...
        if (_gpuMoePrefill)
        {
            ...
            catch (Exception ex)
            {
                Console.Error.WriteLine(
                    ...
                    "falling back to the CPU MoE prefill. Set STINGRAY_MOE_GPU_PREFILL=0 to silence.");
    ...
    public ReadOnlySpan<float> Prefill(IReadOnlyList<int> tokens, int startPos = 0)
    {
        ...
        // STINGRAY_SNAPKV_BUDGET=0 to disable" intent (value is 0 in that case).
        ...
        //     chunk is enormous (STINGRAY_PREFILL_CHUNK set very large).
        ...
        // toggle (the whole value here is the batched trunk; STINGRAY_BATCHED_TRUNK=0 falls
    ...
    /// CPU-GDN debug path and when STINGRAY_BATCHED_TRUNK=0. Produces the host
    ...
    /// View loops under <c>STINGRAY_BATCHED_GDN_SCAN=0</c> / <c>STINGRAY_BATCHED_ATTN=0</c>
    ...
    /// per-position View loop under <c>STINGRAY_BATCHED_GDN_SCAN=0</c>.
    ...
    /// the <c>STINGRAY_BATCHED_GDN_SCAN=0</c> fallback and keeps its CopyDeviceRegion
    ...
    /// <c>STINGRAY_BATCHED_ATTN=0</c> takes the per-position KV-append + SDPA loop (which
    ...
    /// Qwen3-Coder-30B forced with <c>STINGRAY_CPU_MOE=0</c>). The trunk (attention / GDN)
    ...
    private void BatchedRoutedExperts(int layer, int N)
    {
        ...
        // the STINGRAY_*_Q8K gates (useQ8KDown, which is the distinct Q8_KS scheme).
        bool downQ6K = downDt == DType.Q6_K;
    ...
    private void EnsureGpuOffloadScratch(int N)
    {
        ...
        // in-place cudaHostRegister of the mmap pages, per STINGRAY_MOE_PIN_MODE) + the whole-layer
        ...
        if (!_goStaticAllocated)
        {
            ...
            // Pin the expert weights as the DMA source for UploadRawIntoAsyncDirect. STINGRAY_MOE_PIN_MODE
    ...
    /// weights, in one of two modes selected by <c>STINGRAY_MOE_PIN_MODE</c>: <c>register</c>
    ...
    private void EnsureExpertWeightsPinned()
    {
        ...
        string pinMode = (Environment.GetEnvironmentVariable("STINGRAY_MOE_PIN_MODE") ?? "register")
    ...
    /// (copy-mode ~14 GB buffer or the in-place mmap registration, per <c>STINGRAY_MOE_PIN_MODE</c>),
    ...
    private void FreeGpuOffloadScratch(bool freeStatic = true)
    {
        ...
        //    buffer or the in-place mmap registration, per STINGRAY_MOE_PIN_MODE): only on full
    ...
    /// during stream capture — so STINGRAY_TRACE_LAYERS + STINGRAY_DECODE_CUDA_GRAPH together make the
    ...
    /// <inheritdoc />
    /// Issue #45: MoE MTP is supported via the CPU-MoE path (STINGRAY_CPU_MOE=1 or
    ...
    /// (slots + 1, reserved at construction — STINGRAY_MTP_BATCH_MAX). The
    /// STINGRAY_CPU_GDN=1 debug trunk keeps the legacy 2-token path.
    ...
    /// is bookkept). STINGRAY_CPU_GDN=1 trunk: host ring → host state.
    /// </summary>
    ...
    public float[][] BatchVerify(int[] tokens, int startPos)
    {
        ...
        if (k > MaxBatchVerifyTokens)
            throw new ArgumentOutOfRangeException(nameof(tokens), k,
                ...
                "raise STINGRAY_MTP_BATCH_MAX (ring slots are reserved at construction).");
        if (k == 1)
        {
        ...
        if (_cpuGdn)
        {
            // STINGRAY_CPU_GDN=1 debug trunk: the host-side GDN state is live, so the
        ...
        for (int layer = 0; layer < _hp.NumLayers; layer++)
        {
            ...
            else
            {
                ...
                // STINGRAY_MTP_BATCHED_MOE_VERIFY=0 forces this path for MoE too).
    ...
    // Driven by STINGRAY_CPU_MOE or auto-selected from SLRU capacity. Exposed so
    ...
    private void BatchedGpuMoeFfn(int layer, int N, Tensor normAll, Tensor hiddenAll)
    {
        ...
        //    on-GPU-experts (STINGRAY_CPU_MOE=0) path.
    ...
    private void TryUploadDenseFfnLayers(CudaBackend gpu, ModelHyperparams hp, int L)
    {
        ...
        // Override via STINGRAY_DENSE_FFN_GPU_MARGIN_MB env var (set 0 to push to the wall).
        long safetyMarginBytes = 64L * 1024 * 1024;
        var marginOverride = Environment.GetEnvironmentVariable("STINGRAY_DENSE_FFN_GPU_MARGIN_MB");
    ...
    // =================================================================
    //  CPU MoE FFN (STINGRAY_CPU_MOE=1) — mirror of HybridGdnForwardPass.MoeFfn
        ...
        float* cpuNormIn, float* cpuMoeOut)
    {
        ...
        // STINGRAY_Q3K_Q8K=1 / STINGRAY_Q8_0_Q8K=1: hoist a single Q8_K prepack of
        ...
        // STINGRAY_Q3K_Q8K=1 / STINGRAY_Q8_0_Q8K=1 Phase C prepack: each routed
    ...
    // logical processor count, but STINGRAY_MOE_THREADS overrides it: the int8
    ...
    // DispatchDot* kernels) so the STINGRAY_MOE_THREADS resolver has one owner per backend.
    internal static ParallelOptions MoeParallelOpts => s_moeParallelOpts;

    private static int ResolveMoeThreads()
    {
        var v = Environment.GetEnvironmentVariable("STINGRAY_MOE_THREADS");
    ...
    internal static bool ResolveGate(string envName, bool autoDetect)
    {
        ...
        // types STINGRAY_..=true isn't silently defaulted — the CLI/server plumbing writes "1"/"0",
    ...
    private void GpuMatMulBatched(Tensor outputAll, Tensor matrix, Tensor inputAll, int nTok)
    {
        ...
        // N. At decode-sized N — the #30 batched verify (k ≤ 8 by STINGRAY_MTP_BATCH_MAX)
    ...
    // weight, same class as the Q5_K/Q6_K dequant-GEMM). STINGRAY_Q3K_DEQUANT_GEMM=0 → GEMM-N.
    private static readonly bool Q3kDequantGemmEnabled =
        Environment.GetEnvironmentVariable("STINGRAY_Q3K_DEQUANT_GEMM") != "0";
    ...
    /// STINGRAY_CPU_GDN debug GDN weights, and the MoE-MTP head experts. Arrays that
    ...
    private List<(nint Ptr, long Bytes)> BuildCpuPrefaultRegions()
    {
        ...
        // STINGRAY_CPU_GDN=1 debug path (arrays always allocated, populated only then).
    ...
    private Tensor UploadWeight(string name)
    {
        ...
        {
            // Q3_K, etc. (and Q8_0 under STINGRAY_GDN_RAW_Q8_0=0) — no raw GPU matvec used
    ...
    // STINGRAY_KV_DTYPE — issue #27. Default Bf16 on this forward pass; fp32 is
    ...
    private static DType ParseKvDType(string? envValue) => envValue?.Trim().ToLowerInvariant() switch
    {
        ...
        var other     => throw new ArgumentException(
            $"STINGRAY_KV_DTYPE must be 'bf16' or 'fp32' (got '{other}').", nameof(envValue)),
    };
    ...
    // =================================================================
    //  Tracing (STINGRAY_TRACE_LAYERS=1)
    // =================================================================
    ...
    public void Dispose()
    {
        ...
        {
            // STINGRAY_EXPERT_STATS=<path>: dump SLRU hit rate, per-layer hit rate, and
