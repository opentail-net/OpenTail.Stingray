...
/// cache, issue #130) or under <c>STINGRAY_DISABLE_BATCH_VERIFY=1</c>. Per iter:</para>
///
/// <list type="number">
...
public sealed class MtpDecoder
{
    ...
    /// to <c>STINGRAY_MTP_DRAFT_N</c>, then to the built-in default of 3 (a 4-token verify
    ...
    /// (raise <c>STINGRAY_MTP_BATCH_MAX</c> together for k &gt; 4). The result is further
                       ...
                       CancellationToken ct = default)
    {
        ...
        if (batched && draftN > _fwd.MaxBatchVerifyTokens - 1)
            Console.Error.WriteLine(
                ...
                "(raise STINGRAY_MTP_BATCH_MAX to reserve more ring slots).");
        if (batched)
        {
    ...
    {
        bool trace = Environment.GetEnvironmentVariable("STINGRAY_TRACE_MTP") == "1";
        int generated = 0;
