...
public sealed unsafe class VulkanHybridGdnForwardPass : IForwardPass
{
    ...
    // per-token launch overhead. Gated by STINGRAY_VULKAN_BATCHED_PREFILL (default ON; =0 forces
    ...
    // (STINGRAY_VULKAN_GDN_CHUNKED_PREFILL=1 to enable) + requires the device to fit the ~34 KB
        ...
        LayerPlacement placement, int maxContextLength = 0)
    {
        ...
        if (hp.IsMoE)
        {
            ...
            else
            {
                ...
                Console.Error.WriteLine(
                    $"[VulkanHybridGdnForwardPass] MoE auto-select: SLRU capacity ≈ {predictedSlots}/{totalExperts} ({ratio:P0}) → {(_cpuMoe ? "CPU" : "GPU SLRU")} MoE.  Override with STINGRAY_CPU_MOE=0|1.");
            }
        }
        else
        {
        ...
        // Batched prefill gate (issue #356 PR5b): default ON; STINGRAY_VULKAN_BATCHED_PREFILL=0
        // forces the byte-identical sequential per-token Prefill loop so regressions isolate.
        _batchedPrefillEnabled =
            Environment.GetEnvironmentVariable("STINGRAY_VULKAN_BATCHED_PREFILL") != "0";
        ...
        bool chunkedRequested =
            Environment.GetEnvironmentVariable("STINGRAY_VULKAN_GDN_CHUNKED_PREFILL") == "1";
        _chunkedPrefillEnabled = chunkedRequested && gpu.SupportsGdnChunkedPrefill;
        if (chunkedRequested && !gpu.SupportsGdnChunkedPrefill)
            Console.Error.WriteLine(
                "[VulkanHybridGdnForwardPass] STINGRAY_VULKAN_GDN_CHUNKED_PREFILL=1 requested but the device's " +
    ...
    public float[][] BatchVerify(int[] tokens, int startPos)
    {
        ...
        if (_gpuGdnRingScan is null || _gdnRingSlots < 1)
            throw new InvalidOperationException(
                ...
                "head and STINGRAY_DISABLE_MTP must be unset).");
    ...
    private void LoadMtpHead(VulkanBackend gpu)
    {
        ...
        if (_mtpIsMoE && !_cpuMoe)
            throw new NotSupportedException(
                "MoE MTP head requires CPU MoE mode (STINGRAY_CPU_MOE=1). The GPU-SLRU expert cache " +
    ...
    // ================================================================
    //  CPU-MoE FFN (STINGRAY_CPU_MOE=1 / auto on small VRAM) — the headline
