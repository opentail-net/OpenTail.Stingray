...
public static class OpenAiEndpoints
{
        ...
        IOptions<OpenTailStingrayServerOptions> options)
    {
        ...
        try
        {
            req = await ctx.Request.ReadFromJsonAsync(OpenTailStingrayJsonContext.Default.ChatCompletionRequest, ctx.RequestAborted);
        }
        ...
        // (STINGRAY_NO_THINKING) forces reasoning off regardless — for agentic clients that never
        ...
        {
            await HandleNonStreaming(ctx, engine, metrics, adapter, toolsActive, prompt, canonicalHistoryPrefix, images, sp, requestId, created, req.OpentailTiming == true);
        }
    }
        ...
        bool includeTiming = false)
    {
        ...
        ctx.Response.ContentType = "application/json";
        await ctx.Response.WriteAsync(
            JsonSerializer.Serialize(response, OpenTailStingrayJsonContext.Default.ChatCompletionResponse),
            ctx.RequestAborted);
    }
        ...
        bool toolsActive, string prompt, string? canonicalHistoryPrefix, IReadOnlyList<byte[]> images, SamplingParams sp, string requestId, long created)
    {
        ...
        await WriteEvent(ctx.Response, JsonSerializer.Serialize(firstChunk, OpenTailStingrayJsonContext.Default.ChatCompletionChunk));
        ...
        async Task WriteContentDelta(string text)
        {
            ...
            await WriteEvent(ctx.Response, JsonSerializer.Serialize(chunk, OpenTailStingrayJsonContext.Default.ChatCompletionChunk));
        }
        ...
        try
        {
            await foreach (var c in ImageContent.Generate(engine, prompt, canonicalHistoryPrefix, images, sp, ctx.RequestAborted))
            {
                ...
                if (c.Kind == GenerateChunkKind.Usage) { timing.MarkAdmitted(); continue; }
        ...
        // Excludes the trailing finish-reason chunk and "[DONE]" frame below — those are
        // delivery, not generation. Per-token WriteEvent calls above still happen inline with
        // enumeration, matching what a streaming client actually experiences chunk to chunk.
        timing.MarkGenerationComplete();
        ...
        await WriteEvent(ctx.Response, JsonSerializer.Serialize(finalChunk, OpenTailStingrayJsonContext.Default.ChatCompletionChunk));
...
public sealed record ChatCompletionRequest(
    ...
    // block instead of being stripped (OpenTailStingrayServerOptions.PreserveThinking is the
...
public sealed record ChatCompletionResponse(
    ...
    ChatUsage Usage,
    [property: JsonPropertyName("opentail_timing")] OpenTailResponseTiming? OpenTailTiming = null);
...
/// present, constrains the whole response via <see cref="OpenTail.Stingray.Core.Grammar.JsonSchemaOutputConstraint"/>.
/// </summary>
...
/// <see cref="Ordered"/> (OpenTail.Stingray extension, issue #425: require properties in declaration
