...
internal static class ToolGrammarHelper
{
    ...
    /// <see cref="OpenTailStingrayServerOptions.ToolGrammar"/> option, the single source of truth.
    /// The runnable host still honours <c>STINGRAY_TOOL_GRAMMAR=1</c> by mapping that environment
    /// variable onto the option at startup (mirrors the STINGRAY_* env pattern), so the library layer
    /// doesn't re-read the environment here.
    /// </summary>
    public static bool Enabled(OpenTailStingrayServerOptions opts) => opts.ToolGrammar;
