...
public static class AnthropicEndpoints
{
        ...
        IOptions<OpenTailStingrayServerOptions> options)
    {
        ...
        try
        {
            req = await ctx.Request.ReadFromJsonAsync(OpenTailStingrayJsonContext.Default.AnthropicMessageRequest, ctx.RequestAborted);
        }
        ...
        // Server-level DisableThinking (STINGRAY_NO_THINKING) forces reasoning off regardless of
        ...
        // preserve_thinking (per-request) or STINGRAY_PRESERVE_THINKING (server-wide) keeps prior
        // assistant turns' thinking content blocks in the rendered history instead of the default
        // ChatTemplate.ScrubAssistantThinking strip — see OpenTailStingrayServerOptions.PreserveThinking.
        ...
        if (images.Count > 0 && !engine.SupportsImageInput)
        {
            ...
            await ctx.Response.WriteAsync(
                JsonSerializer.Serialize(new AErrorResponse("invalid_request_error",
                        ...
                        "STINGRAY_MMPROJ pointing at its gemma4uv mmproj, on a CPU (NGpuLayers=0) or full-CUDA " +
        ...
        {
            await HandleNonStreaming(ctx, engine, metrics, adapter, prompt, canonicalHistoryPrefix, images, sp, msgId, modelId, req.OpentailTiming == true);
        }
    }
    ...
    {
        using var timing = metrics.BeginServingRequest();
        ...
        var response = new AnthropicMessageResponse(
            ...
            includeTiming ? new OpenTailResponseTiming(timing.TimeToFirstTokenMs, timing.ElapsedMs) : null);

        ctx.Response.ContentType = "application/json";
        await ctx.Response.WriteAsync(
            JsonSerializer.Serialize(response, OpenTailStingrayJsonContext.Default.AnthropicMessageResponse),
            ctx.RequestAborted);
    }
        ...
        string prompt, string? canonicalHistoryPrefix, IReadOnlyList<byte[]> images, SamplingParams sp, string msgId, string modelId)
    {
        ...
        var startMsg = new AMessageStartEvent("message_start",
            new AMessageStartInner(msgId, "message", "assistant", modelId, null, new AUsage(0, 0)));
        await WriteAnthropicEvent(ctx.Response, "message_start",
            JsonSerializer.Serialize(startMsg, OpenTailStingrayJsonContext.Default.AMessageStartEvent));
        ...
        async Task FlushTextDelta(string text)
        {
            ...
            if (!textOpen)
            {
                ...
                if (thinkingOpen && !thinkingClosed)
                {
                    ...
                    await WriteAnthropicEvent(ctx.Response, "content_block_delta",
                        JsonSerializer.Serialize(sigDelta, OpenTailStingrayJsonContext.Default.AContentBlockDeltaEvent));
                    thinkingClosed = true;
                    await WriteAnthropicEvent(ctx.Response, "content_block_stop",
                        JsonSerializer.Serialize(new AContentBlockStopEvent("content_block_stop", 0),
                            OpenTailStingrayJsonContext.Default.AContentBlockStopEvent));
                    nextBlockIndex = 1;
                }
                ...
                await WriteAnthropicEvent(ctx.Response, "content_block_start",
                    JsonSerializer.Serialize(textStart, OpenTailStingrayJsonContext.Default.AContentBlockStartEvent));
                textOpen = true;
            }
            ...
            await WriteAnthropicEvent(ctx.Response, "content_block_delta",
                JsonSerializer.Serialize(delta, OpenTailStingrayJsonContext.Default.AContentBlockDeltaEvent));
        }
        ...
        async Task EmitToolCallsFromBlock(string blockContent)
        {
            ...
            foreach (var tc in calls)
            {
                ...
                await WriteAnthropicEvent(ctx.Response, "content_block_start",
                    JsonSerializer.Serialize(toolStart, OpenTailStingrayJsonContext.Default.AContentBlockStartEvent));
                ...
                await WriteAnthropicEvent(ctx.Response, "content_block_delta",
                    JsonSerializer.Serialize(inputDelta, OpenTailStingrayJsonContext.Default.AContentBlockDeltaEvent));
            ...
            await foreach (var chunk in ImageContent.Generate(engine, prompt, canonicalHistoryPrefix, images, sp, ctx.RequestAborted))
            {
                ...
                if (chunk.Kind == GenerateChunkKind.Thinking)
                {
                    ...
                    if (!thinkingOpen)
                    {
                        ...
                        await WriteAnthropicEvent(ctx.Response, "content_block_start",
                            JsonSerializer.Serialize(thinkingStart, OpenTailStingrayJsonContext.Default.AContentBlockStartEvent));
                        thinkingOpen = true;
                        nextBlockIndex = 1;
                    }
        ...
        catch (Exception ex) when (ex is not OperationCanceledException)
        {
            ...
            ctx.RequestServices.GetService<ILoggerFactory>()?
                .CreateLogger("OpenTail.Stingray.Server.Endpoints")
        ...
        // Excludes the terminal text block, message_delta, and message_stop frames below —
        // those are delivery, not generation. Per-token WriteAnthropicEvent calls above still
        // happen inline with enumeration, matching what a streaming client actually experiences.
        timing.MarkGenerationComplete();
        ...
        await WriteAnthropicEvent(ctx.Response, "message_delta",
            JsonSerializer.Serialize(msgDelta, OpenTailStingrayJsonContext.Default.AMessageDeltaEvent));

        // message_stop
        await WriteAnthropicEvent(ctx.Response, "message_stop",
            JsonSerializer.Serialize(new ATypeOnly("message_stop"), OpenTailStingrayJsonContext.Default.ATypeOnly));
    ...
    private static string MakeSignatureStub(string thinking)
    {
        ...
        var bytes = Encoding.UTF8.GetBytes($"opentail-llm-v1:{thinking.Length}:{thinking.GetHashCode()}");
...
public sealed record AnthropicMessageRequest(
    ...
    // <think> block instead of being dropped (OpenTailStingrayServerOptions.PreserveThinking is
...
public sealed record AnthropicMessageResponse(
    ...
    string StopReason,
    AUsage Usage,
    OpenTailResponseTiming? OpentailTiming = null);
...
/// <see cref="OpenTailStingrayJsonContext"/>'s <c>WhenWritingNull</c> setting), so the wire format is
...
// StopReason is null on message_start — Anthropic only populates it on the terminal
// message_delta, so the field must be nullable to represent the opening event.
