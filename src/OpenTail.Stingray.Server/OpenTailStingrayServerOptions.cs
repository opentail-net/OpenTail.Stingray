...
/// <summary>
/// Configuration for <see cref="ServiceCollectionExtensions.AddOpenTailStingray"/>. Mirrors the
/// OpenTail.Stingray CLI surface (<c>opentail-llm-cli run</c>) so an operator can express any tuning
...
public sealed class OpenTailStingrayServerOptions
{
    ...
    /// <c>STINGRAY_NO_THINKING</c>). For agentic clients that never send the per-request opt-out.
    /// </summary>
    ...
    /// <c>STINGRAY_PRESERVE_THINKING</c>). Off by default: reasoning-model chat templates (Qwen3,
    ...
    /// unconstrained decoding. Also turned on by the <c>STINGRAY_TOOL_GRAMMAR=1</c> environment variable.
    /// </summary>
    ...
    /// that fits VRAM). Set via this property, the <c>STINGRAY_MMPROJ</c> environment variable, or
    /// the <c>OpenTail.Stingray:MmprojPath</c> configuration key. When unset, image content blocks
    ...
    /// property or the <c>STINGRAY_DSPARK_MODEL</c> environment variable. When set, greedy
    ...
    /// this property or the <c>STINGRAY_DSPARK_PLACE</c> environment variable.
    /// </summary>
    ...
    /// <c>STINGRAY_KV_DTYPE</c>: <c>"fp32"</c> (default), <c>"bf16"</c> (half the KV VRAM →
    ...
    /// loader forwards this to <c>STINGRAY_KV_DTYPE</c> before the forward pass is built; the
    /// value is validated then (an unrecognized string fails model load). <c>null</c>/empty
    /// leaves any externally-set <c>STINGRAY_KV_DTYPE</c> untouched. Has effect only on the
    ...
    /// <summary>
    /// Minimum batch size before <see cref="OpenTail.Stingray.Cpu.SimdKernels.MinBatchForBlas"/>
    /// promotes the inner loop to OpenBLAS SGEMM. Mirrors <c>--min-batch-blas</c> /
    /// <c>STINGRAY_MIN_BATCH_BLAS</c>. <c>0</c> = leave the engine default.
    /// </summary>
    public int MinBatchBlas { get; set; } = 0;

    /// <summary>
    /// Worker threads for CPU SIMD matrix-vector kernels. <c>0</c> = use
    /// <see cref="Environment.ProcessorCount"/> (or <c>STINGRAY_CPU_THREADS</c> when set).
    /// Set this below the logical processor count when the server shares CPU or memory bandwidth
    /// with other work; it is applied before the model is loaded.
    /// </summary>
    ...
    /// an immediate HTTP 429 rather than consuming an unbounded number of HTTP connections
    /// and prompt buffers. Set to <c>0</c> to disable this bounded-queue policy. An explicitly
    /// configured <see cref="MaxConcurrentRequests"/> takes precedence for backward
    /// compatibility. Defaults to 16. Mirrors
    /// <c>STINGRAY_MAX_QUEUE</c>.
    /// </summary>
    ...
    /// <c>STINGRAY_MAX_BATCH</c> instead of queuing. Leave unset (or use a value matching
    /// <see cref="MaxBatchSize"/>) when running <see cref="ContinuousBatchingEngine"/>, which
    /// serves concurrency itself. Mirrors <c>STINGRAY_MAX_CONCURRENT</c>.
    /// </summary>
    ...
    /// MiB. Only meaningful for the CPU forward pass. Mirrors <c>STINGRAY_PREFILL_DEQUANT_MB</c>.
    /// </summary>
    ...
    /// <c>STINGRAY_KV_BUDGET_MB</c>.
    /// </summary>
    public long KvBudgetMb { get; set; } = 0;

    /// <summary>
    /// Retained KV-prefix cache budget in MiB for CPU continuous batching. Prefixes are exact
    /// canonical token prefixes rounded down to 16-token pages, shared copy-on-write with new
    /// requests, and evicted LRU when this budget is reached. <c>0</c> disables retention,
    /// negative is unlimited. CUDA batching currently leaves this disabled because its device KV
    /// pages do not yet support safe sharing. Mirrors <c>STINGRAY_PREFIX_CACHE_MB</c>.
    /// </summary>
    public long PrefixCacheMb { get; set; } = 256;

    // ── MoE expert-cache tuning ──────────────────────────────────────────────
    //
    // These knobs only have effect on Vulkan-hybrid MoE today; the engine reads them from
    // the STINGRAY_* environment, so the loader translates the options into env vars before
    ...
    /// <c>--moe-warmpin</c> / <c>STINGRAY_MOE_WARMPIN</c>.
    /// </summary>
    ...
    /// <c>STINGRAY_MOE_GPU_PREFILL=1</c>; <c>false</c> → force the CPU MoE prefill; <c>null</c>
    ...
    /// pin mode (<c>STINGRAY_MOE_PIN_MODE</c>, default <c>register</c>) cudaHostRegisters the expert
    /// mmap pages instead of a ~14 GB copy (no RAM duplicate, no page-cache eviction), and a token
    /// gate (<c>STINGRAY_MOE_GPU_PREFILL_MIN_TOKENS</c>, default 64) keeps tiny prefills + decode on
...
public enum ServerBackend
{
    ...
    Auto = 0,
    /// <summary>Force CPU; ignore <see cref="OpenTailStingrayServerOptions.NGpuLayers"/>.</summary>
    Cpu = 1,
...
/// <summary>
/// Bundle returned by <see cref="OpenTailStingrayServerOptions.EngineFactory"/> and by the
...
/// <see cref="OpenTail.Stingray.Core.IToolCallAdapter.ToolBoundaryStopMarkers"/> against the vocab.
