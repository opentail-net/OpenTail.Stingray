...
public sealed unsafe class CpuBackend : IComputeBackend
{
    ...
    {
        var count = (int)gate.ElementCount;
        float* g = (float*)gate.Handle;
        SimdKernels.GeluTanhMul(g, (float*)up.Handle, g, count);
    }

    public void SoftcapInPlace(Tensor x, float cap)
    {
        var count = (int)x.ElementCount;
        SimdKernels.SoftcapInPlace((float*)x.Handle, count, cap);
    }

    public void AttentionSwa(Tensor q, Tensor kCache, Tensor vCache, Tensor output,
                      Tensor? scoresScratch, int position, int windowSize,
                      int headDim, int numHeads, int numKvHeads, int maxSeqLen)
    {
        SimdKernels.AttentionSwa(
            (float*)q.Handle,
            (float*)kCache.Handle,
            (float*)vCache.Handle,
            (float*)output.Handle,
            scoresScratch != null ? (float*)scoresScratch.Handle : null,
