...
public static class GdnKernels
{
    ...
    //  (env: STINGRAY_TRACE_GDN_INTERNAL=1). Dumps per-head-0 GDN internals
    //  — decay scalar, state-l2-after-decay, state-l2-after-rank1, p-readout
    //  (q · S, pre-RMSNorm/SiLU) — at layers selected by STINGRAY_TRACE_GDN_LAYERS
    //  (default "0,20,40,60") and position selected by STINGRAY_TRACE_GDN_POS
    //  (default 12). Only fires when the caller threads layer/position through.
    // ────────────────────────────────────────────────────────────────────
    private static readonly bool _traceGdnInternal =
        Environment.GetEnvironmentVariable("STINGRAY_TRACE_GDN_INTERNAL") == "1";
    ...
    {
        var s = Environment.GetEnvironmentVariable("STINGRAY_TRACE_GDN_LAYERS");
    ...
    {
        var s = Environment.GetEnvironmentVariable("STINGRAY_TRACE_GDN_POS");
        ...
        int layer = -1, int position = -1)
    {
        ...
        for (int h = 0; h < hv; h++)
        {
            ...
            // state magnitudes are larger; see STINGRAY_TRACE_GDN_INTERNAL.
            p.Clear();
            for (int i = 0; i < d; i++)
            {
                float ki = kh[i];
    ...
    // STINGRAY_TRACE_GDN_INTERNAL emission. Format mirrors EmitBufTrace in
