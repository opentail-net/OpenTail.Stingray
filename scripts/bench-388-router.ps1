...
# Op-offload stays ON (the #390 default) throughout; we toggle STINGRAY_MOE_GPU_ROUTER.
...
function Run-One([string]$name, [string]$model, [string]$router) {
    if ($router -eq "off") { $env:STINGRAY_MOE_GPU_ROUTER = "0" } else { Remove-Item env:STINGRAY_MOE_GPU_ROUTER -ErrorAction SilentlyContinue }
...
} finally { Remove-Item env:STINGRAY_MOE_GPU_ROUTER -ErrorAction SilentlyContinue }
