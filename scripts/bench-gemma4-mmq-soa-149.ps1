# A/B: Q8_0 SoA weight layout (#149, STINGRAY_MMQ_SOA) off vs on, both with the TC
...
function Run($tag, $on) {
    try {
        if ($on) { $env:STINGRAY_MMQ_SOA = "1" } else { Remove-Item env:STINGRAY_MMQ_SOA -ErrorAction SilentlyContinue }
    ...
    } finally { Remove-Item env:STINGRAY_MMQ_SOA -ErrorAction SilentlyContinue }
}
Run "soa-off" $false
Run "soa-on"  $true
