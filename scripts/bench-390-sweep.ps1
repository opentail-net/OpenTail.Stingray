...
param(
    ...
    [string]$PromptDir = "C:\Users\pekka\AppData\Local\Temp\claude\C--p-opentail-llm\e585495f-574a-494e-820c-3cfea3d513e6\scratchpad",
...
function Run-One([string]$cfg, [string]$pf) {
    if ($cfg -eq "off") { $env:STINGRAY_MOE_GPU_PREFILL = "0"; Remove-Item env:STINGRAY_MOE_PIN_MODE -ErrorAction SilentlyContinue }
    else { $env:STINGRAY_MOE_GPU_PREFILL = "1"; $env:STINGRAY_MOE_PIN_MODE = $cfg }
