...
# and MTP-off (STINGRAY_DISABLE_MTP=1) on CPU and CUDA-hybrid for one or more quants,
...
# STINGRAY_MTP_BATCH_MAX=4 / STINGRAY_MTP_DRAFT_N=3) — the 4-input CPU-FFN MatVec4In
...
# vs MTP-off 6.5 t/s. Set STINGRAY_MTP_BATCH_MAX / STINGRAY_MTP_DRAFT_N to sweep other k.
param(
...
foreach ($q in $Quants) {
    ...
    # MTP-off (STINGRAY_DISABLE_MTP=1)
    $env:STINGRAY_DISABLE_MTP = "1"
    try {
        ...
        Remove-Item Env:\STINGRAY_DISABLE_MTP -ErrorAction SilentlyContinue
    }
}
