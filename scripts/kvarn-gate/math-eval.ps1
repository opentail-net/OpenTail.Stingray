...
run greedily through opentail-llm-cli per KV-cache config (fp32 / lloydmax / kvarn),
...
param(
    ...
    [string]$CliExe = (Join-Path $PSScriptRoot '..\..\src\OpenTail.Stingray.Cli\bin\Release\net10.0\opentail-llm-cli.exe'),
    [int]$NPredict = 256,
    [string]$OutDir = (Join-Path $PSScriptRoot 'results')
)

$ErrorActionPreference = 'Stop'
if (-not (Test-Path $CliExe)) { throw "CLI not found at $CliExe — build first: dotnet build OpenTail.Stingray.slnx -c Release" }
...
$noisePattern = '^(Loading model:|Backend:|TurboQuant:|Hardware:|Prefill: \d+ tokens,|Warning:|Note:|Gemma 4 defaults|\[ForwardPass\]|\[OpenTail.Stingray\]|Model loaded in )'
