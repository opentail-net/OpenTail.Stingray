<#
.SYNOPSIS
    Downloads GGUF models for OpenTail.Stingray development.
.DESCRIPTION
...
$Models = @{
    ...
    # `opentail-llm -m models/Qwen3-4B-Q4_K_M.gguf --dspark-model models/dspark_qwen3_4b_block7 --temp 0 -g 0`.
    ...
    # we pull it alongside the text GGUF. The mmproj is only consumed once opentail-llm
    ...
    #   Use with: opentail-llm image ... --upscaler models/RealESRGAN_x4plus.safetensors
    "realesrgan-x4" = @{
...
function Download-Model {
    ...
    if ($info.IsImage) {
        ...
        if ($key -like "z-image-turbo*") {
            ...
            Write-Host "  dotnet run --project src/OpenTail.Stingray.Cli -c Release -- image \"
