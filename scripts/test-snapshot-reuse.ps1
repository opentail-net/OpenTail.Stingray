...
# confirm opentail-llm_prefill_tokens_reused_total > 0 — the metric that stayed at 0
...
function Get-ReusedTokens {
    ...
    foreach ($line in $metrics -split "`n") {
        if ($line -match '^opentail-llm_prefill_tokens_reused_total\s+(\d+)') {
...
Write-Host "      baseline opentail-llm_prefill_tokens_reused_total = $baseline"
if ($baseline -lt 0) {
...
if ($delta -gt 0) {
    Write-Host "      PASS — opentail-llm_prefill_tokens_reused_total advanced by $delta on turn 2." -ForegroundColor Green
