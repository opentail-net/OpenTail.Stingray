# Issue #129 A/B: GPU-SLRU MoE prefill (STINGRAY_CPU_MOE=0) — this branch vs master.
...
$dlls = [ordered]@{
  "master"   = "C:\p\opentail-llm-master\src\OpenTail.Stingray.Cli\bin\Release\net10.0\opentail-llm-cli.dll"
  "branch129" = "C:\p\opentail-llm\src\OpenTail.Stingray.Cli\bin\Release\net10.0\opentail-llm-cli.dll"
}

$env:STINGRAY_CPU_MOE = "0"   # force GPU-SLRU on-GPU routed experts (#129 path)
$dotnet = (Get-Command dotnet).Source
$outDir = "C:\p\opentail-llm\tools\bench"
...
foreach ($kv in $dlls.GetEnumerator()) {
  Write-Host "=== Running $($kv.Key) (STINGRAY_CPU_MOE=0, GPU-SLRU) ===" -ForegroundColor Cyan
