...
# STINGRAY_PREFILL_FLASH_TC=1). Same ~1K prompt + warm-discard as bench-gemma4-136.ps1,
...
function Run($tag, $variant) {
    try {
        Remove-Item env:STINGRAY_PREFILL_FLASH_TC  -ErrorAction SilentlyContinue
        Remove-Item env:STINGRAY_PREFILL_FLASH_TC1 -ErrorAction SilentlyContinue
        if ($variant -eq "tc1") { $env:STINGRAY_PREFILL_FLASH_TC = "1"; $env:STINGRAY_PREFILL_FLASH_TC1 = "1" }
        elseif ($variant -eq "tc2") { $env:STINGRAY_PREFILL_FLASH_TC = "1" }
