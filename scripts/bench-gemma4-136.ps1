...
function Run($tag, $a, $envOff) {
    try {
        if ($envOff) { $env:STINGRAY_BATCHED_PREFILL = "0" } else { Remove-Item env:STINGRAY_BATCHED_PREFILL -ErrorAction SilentlyContinue }
        ...
        Remove-Item env:STINGRAY_BATCHED_PREFILL -ErrorAction SilentlyContinue
    }
}
