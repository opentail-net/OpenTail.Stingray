...
#   off      — STINGRAY_MOE_GPU_PREFILL=0 (CPU MoE prefill; the decode-friendly baseline)
#   copy     — op-offload ON, 14 GB cudaMallocHost copy   (STINGRAY_MOE_PIN_MODE=copy)     [#387 behavior]
#   register — op-offload ON, cudaHostRegister mmap in place (STINGRAY_MOE_PIN_MODE=register) [#390 fix]
...
param(
    ...
    [string]$PromptFile = "C:\Users\pekka\AppData\Local\Temp\claude\C--p-opentail-llm\e585495f-574a-494e-820c-3cfea3d513e6\scratchpad\prefill_prompt.txt",
    [int]$NTokens = 80,
    [int]$TimeoutSec = 900,
...
$cliDll  = ".\src\OpenTail.Stingray.Cli\bin\Release\net10.0\opentail-llm-cli.dll"
$dotnet  = (Get-Command dotnet).Source
$prompt  = Get-Content -Raw $PromptFile
$env:STINGRAY_CPU_MOE = "1"

function Run-One([string]$cfg, [string]$tag) {
    if ($cfg -eq "off") {
        $env:STINGRAY_MOE_GPU_PREFILL = "0"
        Remove-Item env:STINGRAY_MOE_PIN_MODE -ErrorAction SilentlyContinue
    } else {
        $env:STINGRAY_MOE_GPU_PREFILL = "1"
        $env:STINGRAY_MOE_PIN_MODE = $cfg   # copy | register
    }
...
try {
    ...
    Remove-Item env:STINGRAY_MOE_GPU_PREFILL -ErrorAction SilentlyContinue
    Remove-Item env:STINGRAY_MOE_PIN_MODE    -ErrorAction SilentlyContinue
}
