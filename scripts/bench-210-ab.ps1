...
#   baseline  — STINGRAY_DISABLE_MTP=1                     (MTP off; acceptance denominator)
#   pertoken  — MTP on, STINGRAY_MTP_BATCHED_MOE_VERIFY=0  (old per-token verify FFN)
...
$cells = [ordered]@{
    baseline = @{ STINGRAY_DISABLE_MTP = "1" }
    pertoken = @{ STINGRAY_MTP_BATCHED_MOE_VERIFY = "0" }
    batched  = @{}
}
...
try {
    ...
    Remove-Item Env:\STINGRAY_CPU_MOE -ErrorAction SilentlyContinue
}
