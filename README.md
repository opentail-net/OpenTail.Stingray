...
| LTX-Video | 🔴 | 🔬 | CPU | **2026-09-01**: real, checkpoint-driven port (was a structural placeholder) — DiT transformer, VAE decoder, T5-v1.1-XXL text encoder, and T5 tokenizer (real Viterbi-optimal Unigram segmentation, exact match against HuggingFace's `T5TokenizerFast`) each independently golden-verified against their real reference implementations (diffusers/the official `ltx-video` PyPI package/`transformers`), and the full pipeline runs end-to-end with no exceptions — but a real generated output has been looked at and is NOT yet visually correct. Syntactically/structurally sound; the scheduler/CFG trajectory itself is only smoke-tested (not golden-verified against a real multi-step run), which is the leading suspect. See `docs/055-ltx-video-implementation-plan.md` |
...
| FLUX.1 | 🔴 | 👂 | CPU | **Run for the first time 2026-09-01** (`FLUX.1-schnell`, real checkpoint): found and fixed FOUR real bugs blocking execution/correctness (a GGUF tensor-name prefix mismatch, two missing `.weight` suffixes, a genuine architecture bug in the single-block MLP — gated GEGLU instead of real FLUX's plain full-width GELU — and a full rewrite of the 2D RoPE, which had both the wrong axis split and the wrong rotation convention, silently ignoring column position entirely). Pipeline runs to completion with no crash and the RoPE fix produces byte-different output, but a periodic tiling artifact still dominates the image — surviving a real positional-signal fix points at something more mechanical (VAE conditioning or a Q2_K-specific kernel artifact) rather than attention/positional wiring. Not yet root-caused; paused — see `docs/056-flux-tiling-artifact-handoff.md`. Same stage as Wan/LTX-Video: real port, wrong output |
