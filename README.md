...
| RVC (Retrieval-based Voice Conversion) | 🟡 | 🔬 | CPU | **In progress, 2026-09-06**: real HuBERT-base content encoder golden-verified (ours -0.0049/0.3318 vs reference -0.0054/0.3400). RMVPE pitch extractor (5-level ResNet U-Net + bidirectional GRU + sigmoid head) fully implemented with real audio-padding preprocessing; three real structural bugs found+fixed via per-stage tracing against the C++ reference (missing `audio_pad_duration_sec` preprocessing, a U-Net H/W-axis orientation swap, and a `conv_block_res` ReLU-placement bug) — end-to-end salience now closely tracks the reference (max=0.812 vs reference's 0.970; a small remaining gap was root-caused to a latent off-by-one in the *reference's own* ConvTranspose2d graph wiring, not this port, and is not practically reproducible). k-NN retrieval index and synthesizer (confirmed to reuse this codebase's existing HiFiGAN/NSF `HiFTVocoderKernels`) not yet ported — not yet runnable end-to-end. See `docs/audio-review-progress.md`'s RVC section |
