...
| LTX-Video | 🔴 | 🔬 | CPU | **2026-09-01**: real, checkpoint-driven port — DiT transformer (both 32-token test scale AND real 64-token generation scale), VAE decoder (F=1 and F=2 multi-frame), the real multi-step scheduler+CFG denoising LOOP itself (not just the formula), and T5-v1.1-XXL encoder+tokenizer are each independently golden-verified at effectively machine precision (>0.999999 cosine similarity) against their real reference implementations. One real bug found and fixed this pass (missing per-channel VAE latent normalization, `vae.per_channel_statistics.*`, real pipeline default). Despite all of that, real end-to-end generations still produce heavy per-pixel visual corruption — and critically, a from-scratch Python assembly of the SAME independently-verified real components (diffusers transformer + real `ltx_video` VAE/scheduler + real T5) produces the SAME corruption, ruling out the C# port as the sole cause. Root cause still open; see `docs/055-ltx-video-implementation-plan.md` for the full investigation and concrete next steps (diff against the real native `pipeline_ltx_video.LTXVideoPipeline` class itself, not an ad-hoc assembly) |
...
| Vision (17+ architectures implemented; 11 covered by the real-weight suite) | 🟡 | 👂 | CPU / CUDA / Vulkan | Extensively implemented (see below); `MultimodalRealWeightsTests.cs` real-weight-tests 11 architectures with a differentiation/sanity check (real, non-degenerate embeddings; two genuinely different images must NOT produce near-identical output) — this is not a golden numeric-correctness check, only "not obviously broken." Confirmed with local checkpoints, 2026-09-02: **`Llava`, `Pixtral`, `Exaone4`, `MiMoVl`, `Qwen2.5-VL`, `GLM-4.6V` pass**; **`YoutuVl`, `HunyuanVl` return all-zero embeddings, `KimiVl` returns `NaN` cosine similarity, `MiniCpmV` returns near-identical embeddings for distinct images** (none of the 4 root-caused yet; `YoutuVl`/`HunyuanVl` are lowest priority to fix); **`Step3-VL` has no local checkpoint on this machine, so it is genuinely untested**, not passing. `Gemma4UV` (`gemma4uv`) is the one architecture in the whole suite with real GOLDEN numeric verification (`VisionEmbedderParityTests.cs`, min per-token cosine > 0.9995 against a real numpy port of llama.cpp's `gemma4uv.cpp`, `scripts/gemma4uv_ref.py`) — everything else in this row is sanity-checked only, architecture-agnostic and not bit-exact. The remaining named architectures beyond these 11 (Llama 4, Gemma 3, etc. — see the feature list below) are implemented but have no real-weight test coverage in this suite at all |
