...
| LTX-Video | 🟡 | 🔬 | CPU | **2026-09-01**: real, checkpoint-driven port (was a structural placeholder) — DiT transformer, VAE decoder, and T5-v1.1-XXL text encoder each independently golden-verified against their real reference implementations (diffusers/the official `ltx-video` PyPI package/`transformers`) at >0.95/>0.999/>0.999 cosine similarity respectively; real end-to-end CLI smoke test completes with no exceptions. Open gaps: the scheduler/CFG trajectory is only smoke-tested (not golden-verified against a real multi-step run), the T5 tokenizer uses a greedy (not Viterbi-optimal) approximation, and no real full-quality visual output has been judged by eye yet — see `docs/055-ltx-video-implementation-plan.md` |
