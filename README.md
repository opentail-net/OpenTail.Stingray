...
| LTX-Video | 🟡 | 🔬 | CPU | **2026-09-01**: real, checkpoint-driven port — DiT transformer, VAE decoder, the real multi-step scheduler+CFG denoising loop, and T5-v1.1-XXL encoder+tokenizer are each independently golden-verified at effectively machine precision (>0.999999 cosine similarity) against their real reference implementations; one real bug found and fixed (missing per-channel VAE latent normalization). Real end-to-end generations show heavy per-pixel corruption — but this was DEFINITIVELY traced to the checkpoint/settings, not this port: the actual unmodified official `LTXVideoPipeline` class (real native transformer, real everything, real default settings) produces the identical corruption from the same checkpoint. Three independent implementations (this C# port, a from-scratch Python assembly, and the real official pipeline itself) all converge on the same output — ruling this port out as the cause. Likely needs a different resolution/checkpoint/recipe than what was tried; see `docs/055-ltx-video-implementation-plan.md` for the full investigation |
...
| Vision (17+ architectures implemented; 11 covered by the real-weight suite) | 🟡 | 👂 | CPU / CUDA / Vulkan | Extensively implemented (see below); `MultimodalRealWeightsTests.cs` real-weight-tests 11 architectures with a differentiation/sanity check (real, non-degenerate embeddings; two genuinely different images must NOT produce near-identical output) — this is not a golden numeric-correctness check on its own, only "not obviously broken." Confirmed with local checkpoints, 2026-09-02: **`Llava`, `Pixtral`, `Exaone4`, `MiMoVl`, `Qwen2.5-VL`, `GLM-4.6V` pass**; **`YoutuVl`, `HunyuanVl` return all-zero embeddings, `KimiVl` returns `NaN` cosine similarity, `MiniCpmV` returns near-identical embeddings for distinct images** (none of the 4 root-caused yet; `YoutuVl`/`HunyuanVl` are lowest priority to fix); **`Step3-VL` has no local checkpoint on this machine, so it is genuinely untested**, not passing. **Four architectures now have real GOLDEN numeric verification** (min per-token cosine thresholds against a hand-written numpy port of the real llama.cpp mtmd C++ reference, reading the same local mmproj GGUF): `Gemma4UV` (`VisionEmbedderParityTests.cs`, cosine > 0.9995, `scripts/gemma4uv_ref.py`), `Llava` (`LlavaVisionEmbedderParityTests.cs`, cosine > 0.999, `scripts/llava_ref.py` — 2 real bugs found/fixed, see `docs/00-current-work.md`), `Pixtral` (`PixtralVisionEmbedderParityTests.cs`, cosine > 0.97, `scripts/pixtral_ref.py` — 3 real bugs found/fixed), and `GLM-4.6V` (`Glm4VisionEmbedderParityTests.cs`, cosine > 0.97, `scripts/glm4v_ref.py` — **6 real bugs found/fixed**: fused-QKV tensor never read (Q/K/V were silently zero on every layer), attention was a non-softmaxing stub, the second dual-conv patch embedding was fetched but never summed, the strided-conv2d patch merger was fetched but never applied (a plain concat was used instead), the FC/post-norm/gated-FFN projector tail used wrong tensor names or was missing outright, and the 4-section M-RoPE only rotated half of `head_dim` using the wrong (column-only) position axis). `Exaone4`/`MiMoVl`/`Qwen2.5-VL` are differentiation-passing only — golden verification is blocked on a real, unimplemented windowed/local-attention feature (`n_wa_pattern` present in their checkpoints, ignored by the current encoders), deliberately deferred as its own scoped task rather than fixed opportunistically. The remaining named architectures beyond these 11 (Llama 4, Gemma 3, etc. — see the feature list below) are implemented but have no real-weight test coverage in this suite at all |
