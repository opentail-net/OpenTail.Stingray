...
| CosyVoice 2 / 3 | 🟡 | 👂 | CPU | Produces real, intelligible, non-buzzing speech with real end-to-end zero-shot conditioning (mel + prompt tokens + CFG, re-verified 2026-09-01 — a prior handoff doc describing this as missing was stale); whether speaker identity actually transfers is a listening judgment not yet made — see `docs/qwentts-cosyvoice3-handoff.md` |
...
| FLUX.1, SD3/3.5 | 🔴 | 🔵 | — | `FluxDiT.cs` (784 lines) and `SD3/Sd3Pipeline.cs` are real, non-stubbed ports — genuine double/single-stream MMDiT blocks reading real named tensors via `IWeightLoader`, not hardcoded formulas — but no local checkpoint has ever been downloaded, so neither has ever actually been run once. `Sd3ConformanceTests.cs` doesn't help settle this either — it never instantiates the real pipeline, only checks array-concatenation arithmetic. Confirmed 2026-09-02 |
