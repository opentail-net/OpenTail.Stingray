...
| MusicGen (text-to-music) | 🟡 | 👂 | CPU | **2026-09-02: first real end-to-end generation.** `facebook/musicgen-small` (T5 text conditioning + 24-layer delayed-pattern decoder with classifier-free guidance + EnCodec 32kHz decoder) runs against real weights and produces non-degenerate, audibly musical output (user-confirmed "I can hear music in the sample"), but is **not yet golden-verified** — no numeric reference run exists (no local Python/torch this session) and the CFG null-condition convention is a best-recollection guess, not confirmed against real HF `transformers` behavior. Two real bugs found and fixed while wiring against real weights: a delay-pattern input/target off-by-one, and a missing `enc_to_dec_proj` (768→1024) projection only surfaced by inspecting the checkpoint's own tensor names. See `docs/062-musicgen-implementation-plan.md` |
