...
| LTX-Video | 🟡 | 🔬 | CPU | **2026-09-01**: real, checkpoint-driven port (was a structural placeholder) — DiT transformer, VAE decoder, T5-v1.1-XXL text encoder, and T5 tokenizer (real Viterbi-optimal Unigram segmentation, exact match against HuggingFace's `T5TokenizerFast`) each independently golden-verified against their real reference implementations (diffusers/the official `ltx-video` PyPI package/`transformers`); real end-to-end CLI smoke test completes with no exceptions. Only open gap: the scheduler/CFG trajectory is smoke-tested but not golden-verified against a real multi-step run, and no real full-quality visual output has been judged by eye yet — see `docs/055-ltx-video-implementation-plan.md` |
