...
| SD3/3.5 | 🔴 | 👂 | CPU / Vulkan / CUDA | **Run for the first time ever, 2026-09-02.** The CLI could previously only load one specific gated HuggingFace checkpoint layout; added `Sd3Pipeline.LoadSeparate` + a new `--clip-g` flag to load the standard, freely-available multi-file layout instead. Once loadable, found and fixed two real bugs that had made every real checkpoint unloadable or wrong: `MMDiTModel` assumed three separate `qkv.0/1/2` weight matrices per block when the real checkpoint stores one fused `qkv` matrix (exactly why it had never run once), and QK-RMSNorm (`ln_q`/`ln_k`, real tensors, confirmed present) was never applied at all. Fixed both. Progresses much further into real denoising now, but hits a native `AccessViolationException` in `DiffusionOps.Linear` deeper in the block loop — a further real, not-yet-root-caused bug (likely dimension/buffer-size mismatch). Same stage as Wan/LTX-Video/FLUX: real port, not yet producing correct output — see `docs/00-current-work.md` |
