...
**Local LLM inference and image generation, written in C#.** No Python, no P/Invoke to llama.cpp, no
...
|---|---|
| [`OpenTail.Stingray`](https://www.nuget.org/packages/OpenTail.Stingray) | The engine — all nine assemblies in one package |
...
> **This project is a fork of [SharpInference](https://github.com/pekkah/SharpInference)** by Pekka
> Heikura (MIT), which remains actively developed upstream. OpenTail.Stingray has since diverged —
> SafeTensors model packages, session persistence, a dependency-free CLI parser with llama.cpp flag
> compatibility, and its own packaging — and does not track upstream releases. See
> [Acknowledgements](#acknowledgements).

---

## GGUF tensor-format support

GGUF type recognition is not the same as executable model support. OpenTail validates
the model architecture separately and only advertises formats that its portable CPU
path can execute. The following current llama.cpp types are supported for **CPU
serving** (`-g 0`):

| GGML / GGUF type | ID | CPU status | Accelerator-native status |
|---|---:|---|---|
| MXFP4 | 39 | Supported | No dedicated CUDA/Vulkan raw-quant kernel promised |
| NVFP4 | 40 | Supported | No dedicated CUDA/Vulkan raw-quant kernel promised |
| Q1_0 | 41 | Supported | No dedicated CUDA/Vulkan raw-quant kernel promised |
| Q2_0 | 42 | Supported | No dedicated CUDA/Vulkan raw-quant kernel promised |

These formats currently use the portable scalar decoder and the existing
dequantize-to-F32 matvec fallback. They are correctness/compatibility support, not a
throughput claim; a specialised AVX2/AVX-512 or GPU kernel will be added only after a
real model demonstrates a material benefit. GPU and hybrid loaders may use their own
fallback or upload strategy, but only formats explicitly benchmarked for a backend are
performance-supported. Q1_0 and Q2_0 are particularly aggressive quantizations, so a
model may load correctly while still being unsuitable for quality-sensitive use.

Optional: OpenBLAS in `tools/openblas/` for faster batched GEMM — auto-detected, silently skipped
when absent.

## SafeTensors model packages

GGUF remains the **preferred local deployment format**: its tokenizer and configuration metadata
travel with the weights, and it is the only route for OpenTail's block-quantized fast paths.
SafeTensors is supported as an *original-weights* package format for high-precision models.

There is no global "SafeTensors support". Support is published per profile, and anything absent from
a row is unsupported:

| Profile | Architectures | Source dtypes | Tokenizer | Backends | Batching | Sessions |
|---|---|---|---|---|---|---|
| dense-llama-cpu | llama/mistral | F32/F16/BF16 | HuggingFaceJson | Cpu | no | no |

Explicitly excluded from `dense-llama-cpu`: attention/MLP projection bias,
non-SiLU activations, RoPE scaling, quantized SafeTensors weights, and the CUDA/Vulkan routes.

The input is a model **directory** (not a bare `.safetensors` file): `config.json`, the weight shards
with an optional `model.safetensors.index.json`, and a tokenizer asset. Ask whether a package is
supported without loading it:

```bash
stingray capabilities              # the published rows above
stingray inspect -m ./my-llama-package
```

`inspect` reads only the configuration, tokenizer assets and SafeTensors headers — it never loads
tensor data and never constructs a forward pass. It exits non-zero for an unsupported package and
names every reason at once: the missing shard, the unsupported setting, the tokenizer family, the
dtype. A package that is not fully understood is **refused rather than run**, because a model that
executes and is subtly wrong is a worse outcome than one that declines to start.
...
> indicative of the shared engine lineage, not as verified claims about this build — the CUDA rows in
> particular cannot be reproduced by this project's maintainers. Independent numbers are welcome.
...
upstream / llama.cpp defaults). Commands use the published `stingray` binary — from source, swap it for
...
```bash
stingray -m models/SmolLM2-1.7B-Instruct-Q4_K_M.gguf -g -1 \
...
stingray -m models/OLMoE-1B-7B-0924-Instruct-Q4_K_M.gguf -g -1 \
...
stingray -m models/gemma-4-E4B_q4_0-it.gguf -g -1 -c 2048 \
...
stingray -m models/Qwen3-8B-Q4_K_M.gguf -g -1 \
...
```bash
stingray -m models/gemma-4-E4B-it-Q8_0.gguf -g -1 -c 2048 \
  --temp 1.0 --top-k 64 --top-p 0.95 --min-p 0 -p "Summarize the water cycle"
```
...
| **CUDA** `-g 22 -c 2048` (hybrid) | 6.8 | 7.0 | 22 GPU + 20 CPU layers. `-g ≤ 22` required so the CPU shared-KV tail can read its own-KV source layers; CPU dense-FFN dominates decode (bandwidth-bound). `STINGRAY_CUDA_PROFILE=1` for per-phase breakdown |
...
```bash
stingray -m models/gemma-4-12b-it-qat-q4_0.gguf -g -1 -c 2048 \
...
```bash
stingray -m models/Qwen3-Coder-30B-A3B-Instruct-Q4_K_M.gguf -g -1 \
  --temp 0.7 --top-p 0.8 --top-k 20 --repeat-penalty 1.05 -p "Implement a binary search tree in C#"
```

| Backend | Prefill t/s | Decode t/s | Notes |
|---|---:|---:|---|
| **CUDA** `-g -1` (hybrid) | **102.6** | **28.0** | 48-layer trunk on GPU; routed experts auto-select to CPU mmap (expert cache too small for full SLRU offload — `STINGRAY_CPU_MOE=0` forces GPU SLRU streaming). Prefill: batched CPU-MoE (#416/#440, bit-identical, default-on) — **3.5× over the per-token path** (29.4); the Q6_K down-projection no longer re-quantizes its input per weight row. `STINGRAY_HYBRID_BATCHED_MOE=0` to bisect; `STINGRAY_EXPERT_STATS=path` for hit rates |
...
stingray -m models/Carnice-Qwen3.6-MoE-35B-A3B-APEX-MTP-I-Compact.gguf -g -1 \
  --temp 0 --no-thinking -p "List three uses for a paperclip"
```

| Backend | Prefill t/s | Decode t/s | Notes |
|---|---:|---:|---|
| **CUDA** `-g -1 --no-thinking` (hybrid) | **522.0** | **26.5** | agentic finetune; 80% acceptance (`bench-carnice.ps1`). APEX mixed-precision (76% Q3_K + Q4_K/Q8_0 experts); Q8_KS per-32 int dots auto-enable. Prefill: GPU MoE op-offload (#390, register-in-place pin) + on-GPU router + Q3_K dequant→cuBLAS expert GEMM + **chunked FlashQLA GDN scan** (#388), all default-on with op-offload — **3.6× over the CPU MoE path** (144→522), decode within noise. (`--gpu-moe-prefill false` / `STINGRAY_MOE_GPU_ROUTER=0` / `STINGRAY_Q3K_DEQUANT_GEMM=0` / `STINGRAY_GDN_CHUNKED_PREFILL=0` force the CPU/byte-exact paths) |
...
```bash
stingray -m models/Qwen3.6-35B-A3B-UD-Q4_K_M.gguf -g -1 \
  --temp 0.6 --top-p 0.95 --top-k 20 -p "Prove that the square root of 2 is irrational"
```

| Backend | Prefill t/s | Decode t/s | Notes |
|---|---:|---:|---|
| **CUDA** `-g -1` (hybrid) | **475.4** | **24.5** | 10 attn + 30 GDN on GPU; routed MoE on CPU, shared expert GPU-overlapped. `STINGRAY_CPU_MOE=0` forces on-GPU experts. Prefill: GPU MoE op-offload (#390) + on-GPU router (#388) + **raw-Q8_0 trunk** (the all-Q8_0 attn/GDN/shared-expert projections run int8 MMQ instead of a dequant-to-F32 GEMM — **4.2×** prefill over the prior F32-trunk path, `STINGRAY_GDN_RAW_Q8_0=0` reverts). Argmax-stable, not byte-exact |
...
| CPU | **11.3** | 9.3 | hybrid GDN/attn, 256 experts / 8 active. Chunk-parallel (FlashQLA) GDN prefill default-on (1.35× over the 8.4 t/s per-token scan; `STINGRAY_GDN_CHUNKED_PREFILL=0` to disable). MTP variants keep the byte-exact scan (FP-reorder flips the thinking-boundary token) |

#### Qwen3.6-35B-A3B-MTP (GDN+MoE) — [unsloth](https://huggingface.co/unsloth/Qwen3.6-35B-A3B-MTP-GGUF) · 22 GB

```bash
# STINGRAY_CPU_MOE=1 keeps GDN/attn + shared expert on GPU, routed experts on CPU
STINGRAY_CPU_MOE=1 stingray -m models/Qwen3.6-35B-A3B-MTP-UD-Q4_K_M.gguf -g -1 \
  --temp 0 --no-thinking -p "Write a binary search in Rust"
```

| Backend | Prefill t/s | Decode t/s | Notes |
|---|---:|---:|---|
| **CUDA** `-g -1 --no-thinking` (hybrid) | **480.2** | **33.3** | needs `STINGRAY_CPU_MOE=1`: 30 GDN + 10 attn + shared expert on GPU, routed experts CPU mmap; MTP self-spec (~74% accept, this prompt). Prefill: GPU MoE op-offload (#390) + on-GPU router (#388) + **raw-Q8_0 trunk** (int8 MMQ over the all-Q8_0 attn/GDN/shared-expert projections — **4.2×** prefill over the prior F32-trunk dequant-GEMM, `STINGRAY_GDN_RAW_Q8_0=0` reverts). Decode +15% (faster int8 trunk matvec in MTP verify). Argmax-stable. Plain per-token decode is ~80% of llama.cpp's speculation-free `tg128` on the same card (measured warm, back-to-back, same thermal — both throttle together so the ratio holds): the routed experts are RAM-bandwidth-floored (≈ llama.cpp, ~43 GB/s of scattered Q4_K/Q5_K expert weights) and the GPU trunk runs in their shadow, so this is near the practical CPU-MoE decode ceiling; MTP self-spec closes the rest on high-acceptance content. (Faster trunk decode kernels — `STINGRAY_GDN_DECODE_FAST`, `STINGRAY_TRUNK_MATVEC_FAST` — are opt-in but don't move e2e here: the trunk is shadowed, so they're dormant building blocks) |
| Vulkan `-g -1 --no-thinking` (hybrid) | 15.4 | 9.3 | needs `STINGRAY_CPU_MOE=1`; runs on Vulkan (#357). 61% acceptance, but MTP self-spec **regresses** vs ~22 t/s plain MoE decode — routed-expert verify un-amortized on Vulkan (#370). Use `--spec-type none` for speed |
...
```bash
stingray -m models/Qwen3.6-27B-MTP-Q4_K_M.gguf -g -1 \
...
stingray -m models/Llama-4-Scout-17B-16E-Instruct-Q4_K_M-00001-of-00002.gguf \
...
11→45 t/s @32K, up to ~2× on the hybrids). `STINGRAY_SPLIT_DECODE=0` reverts.
...
in VRAM; routed experts auto-select SLRU GPU cache vs CPU mmap by fit (`STINGRAY_CPU_MOE=0|1`). On Ampere+ it
uses bf16 cuBLAS GEMM (`STINGRAY_CUDA_PRECISION` to bisect; NVRTC kernels keep fp32 accumulators). GDN paths
store bf16 KV by default (`STINGRAY_KV_DTYPE=fp32`).

MoE expert-cache knobs (`--moe-warmpin`, `--moe-warmpin-after`, `--no-moe-predict-prefetch`,
`--expert-stats`) are CLI-only; the server reads the equivalent `STINGRAY_MOE_WARMPIN*`,
`STINGRAY_MOE_PREDICT_PREFETCH=0`, `STINGRAY_EXPERT_STATS=<path>` env vars.

MoE expert *placement* matches llama.cpp: `--cpu-moe` (alias `--cmoe`) keeps **all** routed
experts on the CPU, equivalent to `STINGRAY_CPU_MOE=1` (omit for VRAM-fit auto-select;
`STINGRAY_CPU_MOE=0` forces the on-GPU SLRU cache). On the server set `OpenTail.Stingray:CpuMoe`
(`true`/`false`) in `appsettings.json`, or export `STINGRAY_CPU_MOE` directly. llama.cpp's
`--n-cpu-moe` (per-layer split) is **not yet supported** — OpenTail.Stingray's override is
...
paths auto-enable above ~256 MiB KV (budget `min(maxSeqLen/4, 4096)` floored at 1024); `STINGRAY_SNAPKV_BUDGET=N`
...
`STINGRAY_MTP_BATCH_MAX≥N+1`, ~150 MiB VRAM/slot), `--spec-draft-p-min`. Off: `STINGRAY_DISABLE_MTP=1`.
...
`opentail-llm_prefill_tokens_reused_total`.
...
### Tool-call argument grammar (`STINGRAY_TOOL_GRAMMAR`, issue #374)
...
`{"location":"Berlin"}` and `{"query":"…"}`. Enable on the server with `STINGRAY_TOOL_GRAMMAR=1` (or
`OpenTail.Stingray:ToolGrammar`); default off is byte-identical to unconstrained decoding. Honored by the
...
# Read-only compatibility and placement report (no model weights loaded or inference run)
dotnet run --project src/OpenTail.Stingray.Cli -c Release -- \
  inspect -m models/SmolLM2-1.7B-Instruct-Q4_K_M.gguf --json

# Same report without initializing a GPU runtime (availability is reported as not_probed)
dotnet run --project src/OpenTail.Stingray.Cli -c Release -- \
  inspect -m models/SmolLM2-1.7B-Instruct-Q4_K_M.gguf --no-gpu-probe --json

# Fast local diagnostic (optional model validation; no inference)
dotnet run --project src/OpenTail.Stingray.Cli -c Release -- \
...
# Multi-user: STINGRAY_MAX_BATCH=8 enables continuous batching (CPU or CUDA backend). Set
# STINGRAY_MAX_QUEUE to bound waiting requests (default: 16) and receive 429 when full. On CUDA,
# batched decode uses int8 tensor-core matmuls by default at N>=5 (Qwen3-8B Q4_K_M @ 4070 Ti:
# +11% to +28% aggregate t/s from N=5 to N=8); STINGRAY_BATCH_DECODE_MMQ=0 forces the bit-exact
# path. Long-prompt admission prefills in STINGRAY_PREFILL_CHUNK-token slices (default 256,
# 0 = blocking) interleaved with decode, packed across prompts; STINGRAY_KV_BUDGET_MB caps total
# KV memory for admission backpressure (default: half of RAM). CPU continuous batching also
# retains up to 256 MiB of exact canonical, page-aligned KV prefixes by default; set
# STINGRAY_PREFIX_CACHE_MB=0 to disable it. See issues #183 / #206.
...
this engine implements the same quantization block layouts (Q4_K, Q6_K, Q8_0 and friends) so that the
files interoperate. Several kernels were written against its published algorithms — the int8 `mul_mat_q`
approach, the `ggml_vec_dot_q*` reduction orders — and a number of correctness gates in this repo are
byte- or argmax-parity checks *against* llama.cpp output. CLI flag names deliberately follow
`llama-cli` where the meaning matches, so a command copied from its documentation generally just runs.

**[LLamaSharp](https://github.com/SciSharp/LLamaSharp)** — SciSharp, MIT. Studied as the reference for
what a .NET-native inference API should feel like. **No LLamaSharp code is used here**, and the
approach differs fundamentally: LLamaSharp binds to native llama.cpp via P/Invoke, whereas this engine
is managed C# end to end.

Model architectures, quantization schemes and the SafeTensors format belong to their respective
authors and publishers; this project implements readers for published formats and claims no rights
over them.
