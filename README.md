...
| RVC (Retrieval-based Voice Conversion) | 🟡 | 🔬 | CPU | **New port, in progress, 2026-09-06**: real HuBERT-base content encoder (conv feature extractor + weight-normalized positional conv + 12-layer transformer) written and golden-verified — hidden-state mean/std closely match the real `examples/audio.cpp` reference on identical audio (ours -0.0049/0.3318 vs reference -0.0054/0.3400). RMVPE pitch extraction (a 5-level ResNet-style U-Net + bidirectional GRU) precisely scoped but not yet implemented; the k-NN retrieval index and synthesizer (confirmed to reuse this codebase's existing HiFiGAN/NSF `HiFTVocoderKernels`) also not yet ported. Not yet runnable end-to-end. See `docs/audio-review-progress.md`'s RVC section |
