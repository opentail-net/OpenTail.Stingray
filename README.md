...
| Stable Audio 3 | 🟢 | 👂 | CPU | **2026-09-02: fully ported and confirmed.** `stabilityai/stable-audio-3-small-music-base` is ungated and self-contained (real DiT, VAE, and T5Gemma text-conditioner weights bundled together) — all three components (`T5GemmaEncoder`, `StableAudioDiT`, `AcousticVae`) are real, weight-driven, and golden-verified against the real HF `transformers`/`stable_audio_3` Python package run directly as the oracle, including a full end-to-end pipeline golden test (tokenizer → T5Gemma → DiT Euler+CFG → VAE decode). Real classifier-free guidance (Adaptive Projected Guidance, matching the shipped Gradio demo default). Two real T5Gemma-tokenizer bugs found and fixed (missing SentencePiece space-normalizer detection; wrong merge algorithm — leftmost-tie instead of rank-priority). A real listening check at a realistic (non-chaotic) 6s duration/seed — deliberately different from the golden fixture's numerically unstable 0.5s case — was **user-confirmed "100% good"** (2026-09-02) |
| MusicGen (text-to-music) | 🟡 | 👂 | CPU | **2026-09-02: first real end-to-end generation.** `facebook/musicgen-small` (T5 text conditioning + 24-layer delayed-pattern decoder with classifier-free guidance + EnCodec 32kHz decoder) runs against real weights and produces non-degenerate, audibly musical output (user-confirmed "I can hear music in the sample"), but is **not yet golden-verified** — no numeric reference run exists. Two real bugs found and fixed: a delay-pattern input/target off-by-one, and a missing `enc_to_dec_proj` (768→1024) projection only surfaced by inspecting the checkpoint's own tensor names. **Same day, CFG null-condition convention independently CONFIRMED** (was a guess at first) by reading AudioGen's real `audiocraft` source (see below): the real mechanism always converges to the same all-zero embedding this implementation already used. See `docs/062-musicgen-implementation-plan.md` |
| AudioGen (text-to-sound) | 🟡 | 👂 | CPU | **2026-09-02, same day as MusicGen: first real end-to-end generation.** `facebook/audiogen-medium` (48-layer delayed-pattern decoder, same architecture family as MusicGen, + external `t5-large` conditioning + a separately-trained 16kHz environmental-sound EnCodec) runs against real weights and produces non-degenerate audio. No official HF `transformers` release exists for this model — converted the real native AudioCraft `.bin` checkpoints to safetensors via this environment's Python/torch rather than a hand-rolled pickle parser; every hyperparameter came from the checkpoint's own embedded training config, cross-checked against the real `audiocraft` package's source (pip-installed specifically for this). Proved MusicGen's delay-pattern and CFG-formula code genuinely reusable unchanged; extracted two new shared kernels (`Primitives/T5EncoderKernels.cs`, `Primitives/EncodecDecoderKernels.cs`) now used by both models. Not yet golden-verified. See `docs/063-audiogen-implementation-plan.md` |
