...
| MiniMax-Music3 | 🟡 | 🔬 | CPU | **Corrected 2026-09-04** — all six real components (condition encoder, Qwen3-based language model, RVQ depth decoder, flow-matching DiT transformer, vocoder, tokenizer) are individually golden-verified against the real `diffusers` reference (8/8 real-weight test classes pass: component golden-parity ×6, KV-cache consistency, AR-generator smoke test). Not yet done: a real listening check of full end-to-end generated audio, and numeric golden-parity of the composed multi-stage pipeline (only verified stage-by-stage so far). See `docs/066-minimax-music3-future-plan.md` |
