...
OpenTail.Stingray is a high-performance LLM inference engine, image/video diffusion pipeline, multimodal vision understanding runtime, and native neural speech synthesis (TTS/ASR) engine in C# 14 / .NET 10. Reads GGUF model files and executes on CPU (AVX2/AVX-512 SIMD), Vulkan compute shaders, and CUDA/cuBLAS. Targets NativeAOT single-binary deployment.
...
3. **Test Filter Syntax**:
   * Through `dotnet test`: use `--filter-class <Name>` or `--filter-method <Name>` (e.g., `dotnet test tests/OpenTail.Stingray.Tests.Audio -- --filter-class CosyVoiceTests`).
...
4. **Domain Pipelines**:
   * `OpenTail.Stingray.Audio` â Native TTS/ASR (`CosyVoice 3`, `Qwen3-TTS 12Hz`, `Kokoro-82M`, `F5-TTS`, `MeloTTS`, `Piper`, `OpenAI Whisper`, `Silero VAD`).
