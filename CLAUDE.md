...
9. **Don't commit scratch/debug output to the repo root**:
   * Investigation artifacts (one-off debug scripts, raw tensor/error dumps, tuning-iteration audio/image outputs) belong in the OS temp scratchpad while you work, not the repo root — a 2026-08-31 review found ~25 such files had been committed there over time (`chatterbox_lunch_darling_*.wav`, `*_tensors.txt`, `scratch_*.cs`, etc.), which the root-level `.gitignore` patterns now block going forward. If a generated sample is worth keeping as a real fixture (a benchmark result, a bug-repro reference), it goes in `docs/audio-samples/` or `docs/diffusion-samples/` with a descriptive filename, per the existing convention there — not the repo root.
