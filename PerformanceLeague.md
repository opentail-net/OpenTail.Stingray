...
| LTX-Video-2B v0.9.1 (Lightricks Video DiT, T5-XXL text encoder + VAE) | 256×256, 1 frame, 25 steps, CFG 3.0 | CPU | 100.5s | 2026-09-11 | new coverage; `stingray image` CLI (LTX-Video auto-routed by filename), real weights, real 181KB PNG output saved to `docs/diffusion-samples/ltx-video-perfleague-check2.png`. Much faster than Z-Image-Turbo/Wan2.1 above at the same resolution — a smaller 2B DiT and single-frame/no-video-attention path. First real timing number recorded anywhere for this checkpoint (README claims 🟢 golden-verified correctness; no perf number existed before this) |
