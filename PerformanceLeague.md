...
| Granite-4.0-3B-Vision (real image input) | vision-encode correctness | CPU | **ROOT-CAUSED 2026-09-11**: `Granite4VisionEncoder.cs`'s projector is a simplified single-linear-layer approximation of the real reference's multi-block windowed QFormer (`granite4-vision.cpp`), and isn't even wired to the real GGUF tensor names — real inference silently falls back to feeding raw, untrained SigLIP hidden states as "image tokens," producing fluent but non-image-grounded output regardless of prompt/image content. Confirmed against llama.cpp's real reference source, not guessed | A substantial projector rewrite (per-block feature-layer selection, window gather/scatter, self-attn+cross-attn QFormer sub-layer, correct tensor names) — a real architecture-port task, not a quick fix |
