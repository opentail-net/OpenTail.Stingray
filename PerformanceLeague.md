...
| FunASR Paraformer (ONNX path) | structural smoke test, non-Chinese audio | CPU | 1567ms (single run) | — (not RTF-comparable, no real Chinese test audio available locally) | — | — | 2026-09-11 | **NEW independent path, implemented this session** (`src/OpenTail.Stingray.Audio/ParaformerOnnx/`, separate from the broken native GGUF path — real single-graph ONNX forward + real fbank/LFR/CMVN + real per-position-argmax decode). Real weights load, real non-crashing decode, real timing — but not golden-verified (no real Chinese speech clip exists in this repo to check against a ground-truth transcript; this checkpoint is Chinese-specific and the only available test audio is English). Gives this project a working Paraformer route independent of the confirmed-broken GGUF conversion, once golden-verified with a real Chinese clip |
...
| Granite-4.0-3B-Vision (real image input) | vision-encode correctness | CPU | **ROOT-CAUSED and PARTIALLY FIXED 2026-09-11**: the real multi-block windowed QFormer projector (8 blocks, real window gather/scatter, real self+cross attention) has been ported from `granite4-vision.cpp`, replacing the old single-linear-layer stand-in that fed raw untrained hidden states to the LLM. Verified: no more crash, soft-token count now architecturally correct (1152 = 8×144). **Still not correct**: real `--image` runs still produce non-image-grounded output — two open leads (an unverified LayerNorm `eps` value, unimplemented `image_newline`/`add_newline` handling) not yet confirmed as the cause. Needs real numerical debugging against a reference trace | Compare per-block intermediate tensors against a captured reference trace to find the remaining bug |
...
| ~~SDXL-Turbo at `--steps 1`~~ | any | CPU / Vulkan | **FIXED 2026-09-11.** `EulerDiscreteScheduler` divided by zero at `numInferenceSteps == 1`, producing NaN sigmas that rendered as a solid black 843-byte PNG on both backends. Fixed to match diffusers' real `linspace(start,stop,num=1)==[start]` semantics. **Still open, separate, deeper gap**: post-fix output quality at 1-2 steps is still poor (needs "trailing" timestep spacing for turbo/few-step models, not implemented) — 4+ steps produce real, coherent output | Implement "trailing" timestep spacing as a scheduler option for turbo-distilled checkpoints |
