...
public static class Program
{
    public static int Main(string[] args)
    {
        ...
        if (modelPaths.Count == 0)
        {
            ...
            Console.WriteLine("C:\\p\\opentail-llm\\models\\ or E:\\models\\.");
            return 0;
        }
        ...
        string position, Options opts)
    {
        ...
        Environment.SetEnvironmentVariable(
            "STINGRAY_SNAPKV_BUDGET", budget.ToString(CultureInfo.InvariantCulture));
        try
        {
    ...
    /// standard OpenTail.Stingray model directories for both SmolLM2 and Qwen3-8B.
    /// </summary>
    private static List<string> ResolveModels(string? explicitPath)
    {
        ...
        var candidates = new[]
        {
            @"C:\p\opentail-llm\models\SmolLM2-1.7B-Instruct-Q4_K_M.gguf",
            @"C:\p\opentail-llm\models\Qwen3-8B-Q4_K_M.gguf",
...
Options:
  --model <path>         Path to a Q4_K_M GGUF. If omitted, probes the standard
                         OpenTail.Stingray model directories for SmolLM2-1.7B
